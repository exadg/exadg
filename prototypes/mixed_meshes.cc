 ...
 *
 * Does not work in 3D out of the box, need to adopt hp_vertex_dof_identities() and
 * hp_quad_dof_identities() for FE_Q and FE_SimplexP elements.
 *
         ...
         const bool                         ones_on_diagonal = false)
  {
    ...
    if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
    {
      std::cout << "Sizes shape info: "
                << matrix_free.get_shape_info().data[0].shape_values.memory_consumption() << " "
                << matrix_free.get_shape_info().data[0].shape_gradients.memory_consumption() << " "
      ...
      std::cout << "Mapping info: "
                << matrix_free.get_mapping_info().cell_data[0].data_index_offsets.size()
                << " n_jacobians "
                << matrix_free.get_mapping_info().cell_data[0].jacobians[0].size() << std::endl;
    }
...
do_test(const unsigned int fe_degree)
{
  ...
  const unsigned int subdivisions = dim == 2 ? 25 : 4;
  ...
  // set up operator
  op.reinit(mappings, dof_handler, quads, constraints, numbers::invalid_unsigned_int, true);
  ...
  else
  {
    ...
    for(unsigned int r = 0; r < 5; ++r)
    {
      ...
      pcout << "n_dofs " << dof_handler.n_dofs() << "  time " << run_time / 100 << "  GDoFs/s "
