  ...
  const unsigned int n_global_refinements = 3;
    ...
    class InitialSolutionVelocity : public Function<dim> {
        ...
        InitialSolutionVelocity (
            const unsigned int n_components = dim, const double time = 0.) :
        ...
        public:
            ...
            void run() {
                ...
                LinearAlgebra::distributed::Vector<double> velocity_double(dof_handler_u.locally_owned_dofs(),
                                                           MPI_COMM_WORLD);
                VectorTools::interpolate(mapping, dof_handler_u, InitialSolutionVelocity<dim>(), velocity_double);
