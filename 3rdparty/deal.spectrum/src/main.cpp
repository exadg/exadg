...
int main(int argc, char **argv){
    ...
    // init MPI...
    MPI_Init(&argc, &argv); fftw_mpi_init();
    // ... and get info
    int rank, size;
    ...
    bool writeRowWise     = false;
    double lower_limit    = std::numeric_limits<double>::min();
    // ... overwrite default configurations with command line provided arguments
    if (map.count("e")) evaluation_points = stoi(map["e"]);
    if (map.count("r")) writeRowWise      = true;
    if (map.count("l")) lower_limit       = stof(map["l"]);
    ...
    for (int i = 1; i < argc; i++){
        ...
        if(s.type == 0){
            ...
            h.init(); fftw.init(); ipol.init(h); fftc.init(h, fftw);
            // ... read dofs from file
            ipol.deserialize(filename); 
            // ... interpolate values
            ipol.interpolate();
            // ... permute to be row wise
            fftc.ipermute(ipol.dst, fftw.u_real); fftc.iwait();
            
            if(writeRowWise){
                // write permuted data to file: create new file name...
                std::string a(filename); a.append("_converted");
                // ... write header
                Setup s_ = s; s_.type = 1; s_.writeHeader(a.c_str());
                // ... write data
