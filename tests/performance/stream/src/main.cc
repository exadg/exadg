...
//#include
//"../../../../applications/incompressible_navier_stokes_test_cases/deformed_cube_manifold.h"
...
public:
  ...
  run(ConvergenceTable & convergence_table, bool curv, MPI_Comm comm)
  {
...

    {
      ...
      repeat<dim, fe_degree>(convergence_table, "poisson-1-face-based", curv, [&]() mutable {
        ns.run();
      });
    }

    {
      ...
      repeat<dim, fe_degree>(convergence_table, "poisson-1-cell-based", curv, [&]() mutable {
        ns.run();
      });
    }

    {
      ...
      repeat<dim, fe_degree>(convergence_table, "poisson-d-face-based", curv, [&]() mutable {
        ns.run();
      });
    }

    {
      ...
      repeat<dim, fe_degree>(convergence_table, "poisson-d-cell-based", curv, [&]() mutable {
        ns.run();
      });
    }
    {
    ...
    }
    {
      CompNS::CombinedWrapper<dim, fe_degree, fe_degree + 1, fe_degree + 1, Number> ns(
        triangulation, true);
      repeat<dim, fe_degree>(convergence_table, "comp-cell-based", curv, [&]() mutable {
        ns.run();
      });
    }
  }
