...
private:
  void
  local_diagonal_cell(const MF & data, int &, const int &, const Range & cell_range) const
  {
  ...
  void
  local_diagonal_face(const MF & data, int &, const int &, const Range & cell_range) const
  {
...
public:
  ...
  run(ConvergenceTable & convergence_table)
  {
    ...
    //        triangulation.refine_global(log(std::pow(dim==2 ? 5e7 : 2e7, 1.0 / dim) / (fe_degree +
    //        1)) / log(2));
    ...
    additional_data.mapping_update_flags_inner_faces =
      update_gradients | update_JxW_values | update_values | update_normal_vectors;
