...
public:
  OperatorWrapper(parallel::distributed::Triangulation<dim> const & triangulation, bool do_faces)
    : fe(degree), mapping(1 /*TODO*/), dof_handler(triangulation)
  {
    ...
    laplace_additional_data.face_evaluate.value &= do_faces;
    laplace_additional_data.face_evaluate.gradient &= do_faces;
