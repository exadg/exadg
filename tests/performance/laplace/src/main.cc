...
public:
  ...
  run(ConvergenceTable & convergence_table, bool curv, MPI_Comm comm)
  {
    ...
    {
      Poisson::OperatorWrapper<dim, fe_degree, Number> ns(triangulation, false);
      repeat<dim, fe_degree>(convergence_table, "poisson-cell", curv, [&]() mutable { ns.run(); });
    }

    {
      Poisson::OperatorWrapper<dim, fe_degree, Number> ns(triangulation, true);
      repeat<dim, fe_degree>(convergence_table, "poisson-all", curv, [&]() mutable { ns.run(); });
    }
  }
};
