...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::do_face_integral(FEEvalFace & p_n, FEEvalFace & p_p) const
{
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::do_face_int_integral(FEEvalFace & p_n, FEEvalFace & /*p_p*/) const
{
  VectorizedArray<Number> sigmaF;
  sigmaF = 100.0; // dummy value :)
  ...
  for(unsigned int q = 0; q < p_n.n_q_points; ++q)
  {
    ...
    VectorizedArray<Number> average_gradient = (p_n.get_normal_gradient(q)) * 0.5;
    average_gradient                         = average_gradient - jump_value * sigmaF;
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::do_face_ext_integral(FEEvalFace & /*p_n*/, FEEvalFace & p_p) const
{
  VectorizedArray<Number> sigmaF;
  sigmaF = 100.0; // dummy value :)
  ...
  for(unsigned int q = 0; q < p_p.n_q_points; ++q)
  {
    ...
    VectorizedArray<Number> average_gradient = (p_p.get_normal_gradient(q)) * 0.5;
    average_gradient                         = average_gradient - jump_value * sigmaF;
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::do_boundary_integral(FEEvalFace & phi,
                                                           OperatorType const & /**/,
