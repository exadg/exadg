...
{
public:
  Runner(ConvergenceTable & convergence_table, const int approx_system_size, std::string label)
    : comm(MPI_COMM_WORLD),
      rank(get_rank(comm)),
      triangulation(comm,
...
private:
      ...
      unsigned int                                  mg_level = numbers::invalid_unsigned_int)
  {
    ...
    // ... matrix-free VMult:
    repeat<dim, fe_degree>(convergence_table, label, [&]() mutable {
      laplace.vmult_add(vec_dst_1, vec_src);
    });
  }
...
public:
  ...
  {
    Runner<dim, fe_degree, FE_DGQ<dim>, 0> run_dg_0(convergence_table_1,
                                                    approx_system_size,
                                                    "dg-cart");
    run_dg_0.run();
    Runner<dim, fe_degree, FE_DGQ<dim>, 1> run_dg_1(convergence_table_3,
                                                    approx_system_size,
                                                    "dg-curv");
    run_dg_1.run();

    Runner<dim, fe_degree, FE_Q<dim>, 0> run_cg_0(convergence_table_2,
                                                  approx_system_size,
                                                  "cg-cart");
    run_cg_0.run();
    Runner<dim, fe_degree, FE_Q<dim>, 1> run_cg_1(convergence_table_4,
                                                  approx_system_size,
                                                  "cg-curv");
    run_cg_1.run();
  }
    ...
    {
      {
        ...
        std::string   file_name =
          "operator-dg-cart-" + std::to_string(dim) + "-" + std::to_string(i) + ".csv";
      ...
      }
      {
        std::ofstream outfile;
        std::string   file_name =
          "operator-cg-cart-" + std::to_string(dim) + "-" + std::to_string(i) + ".csv";
        outfile.open(file_name.c_str());
      ...
      }
      {
        std::ofstream outfile;
        std::string   file_name =
          "operator-dg-curv-" + std::to_string(dim) + "-" + std::to_string(i) + ".csv";
        outfile.open(file_name.c_str());
      ...
      }
      {
        std::ofstream outfile;
        std::string   file_name =
          "operator-cg-curv-" + std::to_string(dim) + "-" + std::to_string(i) + ".csv";
        outfile.open(file_name.c_str());
