...
GridToGridProjector<dim, n_components>::project()
{
  ...
  if constexpr(export_vectors)
  {
    ...
    {
      write_vector(dof_handler_source_continuous,
                   *mapping,
                   vectors_source_continuous[i],
                   "./",
                   "source_continuous_" + std::to_string(i) + "_" + elem_str,
                   n_subdivisions,
                   n_components);
    }
  }
  ...
  data.solver_data.compute_performance_metrics = false;
  data.solver_data.use_preconditioner          = true;
  ...
  if(all_dg)
  {
    data.preconditioner = GridToGridProjection::Preconditioner::InverseMass;
  }
  else
  {
    data.preconditioner = GridToGridProjection::Preconditioner::PointJacobi;
  }

  GridToGridProjection::grid_to_grid_projection<dim, VectorType>(source_vectors_per_dof_handler,
                                                                 source_dof_handlers,
                                                                 mapping,
...
GridToGridProjector<dim, n_components>::check()
{
  ...
  if constexpr(export_vectors)
  {
    ...
    for(unsigned int i = 0; i < vectors_target.size(); ++i)
    {
      write_vector(dof_handler_target,
                   *mapping,
                   vectors_target[i],
                   "./",
    ...
    for(unsigned int i = 0; i < vectors_target_continuous.size(); ++i)
    {
      write_vector(dof_handler_target_continuous,
                   *mapping,
                   vectors_target_continuous[i],
                   "./",
                   "target_continuous_" + std::to_string(i) + "_" + elem_str,
                   n_subdivisions,
                   n_components);
    }
  }
