...
public:
  Runner(ConvergenceTable& convergence_table)
      : convergence_table(convergence_table), comm(MPI_COMM_WORLD), rank(get_rank(comm)),
...
//    additional_data_cg.level_mg_handler = global_refinements;
    ...
    mg_constrained_dofs_cg.make_zero_boundary_constraints(dof_handler_cg,
                                                       dirichlet_boundary);

//    if (fe_dgq.dofs_per_vertex > 0)
      dummy_cg.add_lines(
          mg_constrained_dofs_cg.get_boundary_indices(global_refinements));
  ...
  void run(LaplaceOperator<dim, fe_degree, value_type> &laplace_dg,
    ...
    CGToDGTransfer<dim, value_type> transfer(
        laplace_dg.get_data(), laplace_cg.get_data(), mg_level, fe_degree);
    
    
    
    repeat<dim, fe_degree>(convergence_table, "toCG",
               [&]() mutable { transfer.toCG(vec_rhs_cg, vec_rhs_dg); });
    
    repeat<dim, fe_degree>(convergence_table, "toDG",
               [&]() mutable { transfer.toDG(vec_rhs_dg, vec_rhs_cg); });
