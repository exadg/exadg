...
#ifdef LIKWID_PERFMON
    std::string likwid_label = label + "#" + std::to_string(dim) + "#" + std::to_string(fe_degree_1) + "#" +
                               std::to_string(fe_degree_2);
...
public:
  ...
  run()
  {
    ...
    {
      laplace_1.reinit(
        dof_handler_1, mapping_1, (void *)&laplace_additional_data, mg_constrained_dofs, level);
      laplace_2.reinit(
        dof_handler_2, mapping_2, (void *)&laplace_additional_data, mg_constrained_dofs, level);
...
public:
  ...
  {
    Runner<dim, fe_degree_1, fe_degree_1 / 2> run_cg(convergence_table);
    run_cg.run();
  }
};
