...
#include "poisson_test_cases/lung/lung_grid.h"
    ...
    void run(int generations, int refinements1, int refinements2, std::vector<std::string> & files) {
        ...
        auto tree_factory = [](std::vector<Node*>& roots, unsigned int generations){
            ...
            cells[2].vertices[0] = 1; cells[2].vertices[1] = 3;
        ...
        
        {
            ...
            LinearAlgebra::distributed::Vector<double> ranks(dofhanlder2.locally_owned_dofs (),MPI_COMM_WORLD);
        ...
        printf("### Generations: %d; refs_1: %d; refs_2: %d; cells: %d\n", generations, refinements1, refinements2, tria.n_active_cells());
