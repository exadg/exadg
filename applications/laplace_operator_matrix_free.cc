...
WallTimeCalculation const WALL_TIME_CALCULATION = WallTimeCalculation::Average;
...
template<int dim>
void
create_grid(parallel::distributed::Triangulation<dim> &     triangulation,
...
template<int dim>
void
print_grid_data(ConditionalOStream &                        pcout,
...
public:
  ...
  void
  reinit(const MatrixFree<dim, Number> &  mf_data,
...
LaplaceOperator<dim, degree, Number>::LaplaceOperator() : data(0)
{
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::compute_array_penalty_parameter(const Mapping<dim> & mapping)
{
  ...
  for(unsigned int i = 0; i < data->n_cell_batches() + data->n_ghost_cell_batches(); ++i)
  {
    for(unsigned int v = 0; v < data->n_components_filled(i); ++v)
    {
      ...
      for(unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
      {
        ...
        const double factor =
          (cell->at_boundary(f) && periodic_boundary_ids.find(cell->face(f)->boundary_id()) ==
  ...
  LinearAlgebra::distributed::Vector<Number> const & src) const
{
  ...
  if(this->operator_data.compute_face_integrals == false)
    data->cell_loop(&LaplaceOperator<dim, degree, Number>::cell_loop, this, dst, src);
  else
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  if(this->operator_data.compute_face_integrals == true)
  {
    ...
    FEFaceEvaluation<dim, degree, degree + 1, 1, Number> fe_eval_neighbor(
      data, false, operator_data.laplace_dof_index, operator_data.laplace_quad_index);
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  if(this->operator_data.compute_face_integrals == true)
  {
    ...
    for(unsigned int face = face_range.first; face < face_range.second; face++)
    {
      ...
      fe_eval.evaluate(true, true);
      VectorizedArray<Number> sigmaF =
        fe_eval.read_cell_data(array_penalty_parameter) * get_penalty_factor();
  ...
  const LinearAlgebra::distributed::Vector<Number> & src) const
{
  ...
  typedef internal::EvaluatorTensorProduct<internal::evaluate_evenodd,
                                           dim,
                                           degree + 1,
                                           degree + 1,
  ...
  for(unsigned int cell = 0; cell < data->n_macro_cells(); ++cell)
  {
    ...
    const unsigned int * dof_indices =
      &data->get_dof_info()
         .dof_indices_contiguous[2][cell * VectorizedArray<Number>::n_array_elements];
    const unsigned int vectorization_populated = data->n_components_filled(cell);
    // transform array-of-struct to struct-of-array
    if(vectorization_populated == VectorizedArray<Number>::n_array_elements)
      vectorized_load_and_transpose(dofs_per_cell, src.begin(), dof_indices, data_ptr);
    else
    {
    ...
    else if(dim == 2)
    {
      ...
      eval_val.template values<1, true, false>(data_ptr + 2 * dofs_per_cell,
                                               data_ptr + dofs_per_cell);
    }
    else if(dim == 3)
    {
      eval_val.template values<0, true, false>(data_ptr, data_ptr + dofs_per_cell);
      eval_val.template values<1, true, false>(data_ptr + dofs_per_cell,
                                               data_ptr + 2 * dofs_per_cell);
      eval_val.template values<2, true, false>(data_ptr + 2 * dofs_per_cell,
                                               data_ptr + dofs_per_cell);
    }
    eval.template gradients<0, true, false>(data_ptr + dofs_per_cell, data_ptr + 2 * dofs_per_cell);
    if(dim > 1)
      eval.template gradients<1, true, false>(data_ptr + dofs_per_cell,
                                              data_ptr + 3 * dofs_per_cell);
    if(dim > 2)
      eval.template gradients<2, true, false>(data_ptr + dofs_per_cell,
                                              data_ptr + 4 * dofs_per_cell);
    ...
    if(data->get_mapping_info().cell_type[cell] == internal::MatrixFreeFunctions::cartesian)
      for(unsigned int q = 0; q < dofs_per_cell; ++q)
        for(unsigned int d = 0; d < dim; ++d)
          data_ptr[(2 + d) * dofs_per_cell + q] *=
            jac_ptr[0][d][d] * jac_ptr[0][d][d] * (jxw_ptr[0] * quadrature_weights[q]);
    // affine cell case
    ...
    // apply tensor product kernels
    eval.template gradients<0, false, false>(data_ptr + 2 * dofs_per_cell,
                                             data_ptr + dofs_per_cell);
    if(dim > 1)
      eval.template gradients<1, false, true>(data_ptr + 3 * dofs_per_cell,
                                              data_ptr + dofs_per_cell);
    if(dim > 2)
      eval.template gradients<2, false, true>(data_ptr + 4 * dofs_per_cell,
                                              data_ptr + dofs_per_cell);
    ...
    {
      eval_val.template values<0, false, false>(data_ptr + dofs_per_cell,
                                                data_ptr + 2 * dofs_per_cell);
    ...
    else if(dim == 3)
    {
      eval_val.template values<0, false, false>(data_ptr + dofs_per_cell,
                                                data_ptr + 2 * dofs_per_cell);
      eval_val.template values<1, false, false>(data_ptr + 2 * dofs_per_cell,
                                                data_ptr + dofs_per_cell);
    ...
    if(vectorization_populated == VectorizedArray<Number>::n_array_elements)
      ...
      vectorized_transpose_and_store(false, dofs_per_cell, data_ptr, dof_indices, dst.begin());
    else
    {
      for(unsigned int v = 0; v < vectorization_populated; ++v)
        for(unsigned int i = 0; i < dofs_per_cell; ++i)
  ...
  const LinearAlgebra::distributed::Vector<Number> & src) const
{
  ...
  const VectorizedArray<Number> * __restrict shape_vals =
    data->get_shape_info().shape_values_eo.begin();
  const VectorizedArray<Number> * __restrict shape_grads =
  ...
  const LinearAlgebra::distributed::Vector<Number> & src) const
{
  ...
  VectorizedArray<Number> * __restrict data_ptr =
    degree < 9 ? my_array : scratch_data_array->begin();
      ...
      for(unsigned int i1 = 0; i1 < nn; ++i1) // loop over y layers
      {
        ...
        if(cell_type == internal::MatrixFreeFunctions::cartesian)
          ...
          for(unsigned int i = 0; i < nn; ++i)
          {
            ...
            outy[i1 * nn + i] =
              jxw_ptr[q] * (jac_ptr[q][0][1] * grad[0] + jac_ptr[q][1][1] * grad[1] +
...
public:
  ...
  apply_laplace_operator() const
  {
    ...
    pcout << std::endl
          << std::scientific << std::setprecision(4) << "Wall time / dof [s]: " << wall_time
          << std::endl;
    ...
    if(COMPUTE_FACE_INTEGRALS == false)
    {
      ...
      dst2 -= dst;
      pcout << std::scientific << std::setprecision(4) << "Wall time / dof [s]: " << wall_time
            << ", error to nice code: " << dst2.linfty_norm() << std::endl;
      times[1] = wall_time;
...
public:
  static void
  run(unsigned int const refine_steps_space)
  {
...
void
print_wall_times(std::vector<std::pair<unsigned int, std::array<double, 4>>> const & wall_times,
                 unsigned int const refine_steps_space)
{
  if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
  {
    std::cout << std::endl
              ...
              << std::endl
              << std::endl
              << "Wall times for refine level l = " << refine_steps_space << ":" << std::endl
              << std::endl
              << "       "
              << "wall time / dof [s]" << std::endl
              << "  k    "
              << "standard    manual_1    manual_2    manual_3" << std::endl;

    typedef
      typename std::vector<std::pair<unsigned int, std::array<double, 4>>>::const_iterator ITERATOR;
    for(ITERATOR it = wall_times.begin(); it != wall_times.end(); ++it)
    {
      std::cout << "  " << std::setw(5) << std::left << it->first;
      for(unsigned int i = 0; i < 4; ++i)
        std::cout << std::setw(12) << std::left << std::scientific << std::setprecision(4)
                  << it->second[i];
      std::cout << std::endl;
    }
