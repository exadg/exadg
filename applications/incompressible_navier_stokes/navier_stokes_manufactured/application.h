...
private:
  ...
  set_parameters() final
  {
    ...
    this->param.solver_data_pressure_poisson    = SolverData(1000, 1.e-12, 1.e-8, "cg");
    ...
    {
      std::string const solver_name = treatment_of_convective_term == TreatmentOfConvectiveTerm::Explicit ?
          "cg" : "fgmres";
      this->param.solver_data_momentum = SolverData(1000, 1e-12, 1e-8, solver_name);
    ...
    if(this->param.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
    {
      ...
      // linear solver
      this->param.solver_data_momentum           = SolverData(1e4, 1.e-12, 1.e-8, "gmres", 100);
    ...
    if(this->param.preconditioner_momentum == MomentumPreconditioner::Multigrid)
    {
      ...
      this->param.multigrid_data_momentum.coarse_problem.solver_data    = SolverData(1000, 1e-12, 1e-8, "undefined", 30);
    ...
    // linear solver
    this->param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-8, "fgmres", 100);
