...
do_set_parameters(Parameters & param, bool const is_precursor = false)
{
  ...
  param.solver_data_pressure_poisson         = SolverData(1000, 1.e-12, 1.e-3, "cg", 100);
  ...
  // projection step
  param.solver_data_projection           = SolverData(1000, 1.e-12, 1.e-3, "cg");
  ...
  {
    param.solver_data_momentum    = SolverData(1000, 1.e-12, 1.e-3, "cg");
  ...
  if(param.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
  {
    ...
    if(param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
      param.solver_data_momentum = SolverData(1e4, 1.e-12, 1.e-1, "gmres", 100);
    else
      param.solver_data_momentum = SolverData(1e4, 1.e-12, 1.e-3, "gmres", 100);
  ...
  // linear solver
  if(param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
    param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-1, "gmres", 100);
  else
    param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-3, "gmres", 100);
