...
private:
  ...
  set_parameters() final
  {
    ...
    if(this->param.temporal_discretization == TemporalDiscretization::BDFDualSplitting)
    {
      ...
      else
      {
        ...
        this->param.solver_data_momentum        = SolverData(1000, 1.e-12, 1.e-6, "gmres");
      }
    ...
    if(this->param.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
    {
      ...
      // linear solver
      this->param.solver_data_momentum           = SolverData(1e4, 1.e-12, 1.e-6, "fgmres", 100);
      this->param.update_preconditioner_momentum = false;
      this->param.preconditioner_momentum = MomentumPreconditioner::InverseMassMatrix; // Multigrid;
    ...
    // linear solver
    this->param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-6, "fgmres", 100);
  ...
  set_parameters_poisson() final
  {
    ...
    // SOLVER
    this->poisson_param.solver_data               = SolverData(1e4, 1e-20, 1e-10, "cg");
