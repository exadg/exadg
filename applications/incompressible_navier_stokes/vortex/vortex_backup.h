...
public:
  ...
  double const viscosity = 0.01; // 2.5e-2; // 1.e-2; //2.5e-2;
  ...
  set_input_parameters(InputParameters & param)
  {
    ...
    param.calculation_of_time_step_size =
      TimeStepCalculation::MaxEfficiency;          // UserSpecified; //CFL;
    param.time_step_size                  = 5.e-5; // end_time;
    param.max_velocity                    = 1.4 * u_x_max;
    param.cfl                             = 0.1; // TODO 0.2; // 0.4;
    ...
    if(param.temporal_discretization == TemporalDiscretization::BDFCoupledSolution)
      param.apply_penalty_terms_in_postprocessing_step = true; // TODO false;
    else
    ...
    // formulation
    param.order_pressure_extrapolation = std::min(2, (int)param.order_time_integrator) - 1;
    ...
    param.newton_solver_data_coupled = Newton::SolverData(100, 1.e-12, 1.e-4);
