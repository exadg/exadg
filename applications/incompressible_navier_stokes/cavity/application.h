...
private:
  ...
  set_parameters() final
  {
    ...
    this->param.solver_data_block_diagonal = SolverData(1000, 1.e-12, 1.e-2, "undefined", 1000);
    this->param.quad_rule_linearization    = QuadratureRuleLinearization::Overintegration32k;

    // PROJECTION METHODS

    // pressure Poisson equation
    this->param.solver_data_pressure_poisson    = SolverData(1000, 1.e-12, 1.e-8, "cg", 100);
    this->param.preconditioner_pressure_poisson = PreconditionerPressurePoisson::Multigrid;

    // projection step
    this->param.solver_data_projection    = SolverData(1000, 1.e-12, 1.e-8, "cg");
    ...
    {
      this->param.solver_data_momentum    = SolverData(1000, 1.e-12, 1.e-8, "cg");
    ...
    if(this->param.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
    {
      ...
      if(this->param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
        this->param.solver_data_momentum = SolverData(1e4, 1.e-12, 1.e-2, "gmres", 100);
      else
        this->param.solver_data_momentum = SolverData(1e4, 1.e-12, 1.e-6, "gmres", 100);
      this->param.preconditioner_momentum        = MomentumPreconditioner::InverseMassMatrix;
    ...
    // linear solver
    if(this->param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
      this->param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-2, "fgmres", 30);
    else
      this->param.solver_data_coupled = SolverData(1e4, 1.e-12, 1.e-6, "fgmres", 30);
    ...
    this->param.solver_data_velocity_block        = SolverData(1e4, 1.e-12, 1.e-6, "fgmres", 100);
    ...
    this->param.solver_data_pressure_block        = SolverData(1e4, 1.e-12, 1.e-6, "cg", 100);
  }
