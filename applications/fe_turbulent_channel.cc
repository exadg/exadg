...
{
public:
  InitialChannel(const double time = 0.) : Function<dim>(dim, time)
  {
  }
  ...
  vector_value(const Point<dim> & p, Vector<double> & values) const
  {
    ...
    if(dim == 3)
      values(2) = 6. * (1 - p[1] * p[1] * p[1] * p[1] * p[1] * p[1]) *
                  std::sin(p[dim - 1] * 3 + 1. / 2. * p[0]);
  }
};
...
run_channel_flow()
{
  ...
  {
    std::cout << "Running on " << Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD) << " MPI ranks"
              << std::endl;
    std::cout << "Mesh: " << elements_in_y_dir << " elements in wall-normal direction, "
              << triangulation.n_global_active_cells() << " elements in total" << std::endl;
    std::cout << "Finite element pair: " << solver.fe_u.base_element(0).get_name() << "^" << dim
              << " x " << solver.fe_p.get_name() << std::endl
              << std::endl;
  }
  ...
  body_force[0]           = 1.;
  std::string output_base = "output/cha-180-" + Utilities::to_string(elements_in_y_dir) + "-q" +
                            Utilities::to_string(u_degree) + "-";
