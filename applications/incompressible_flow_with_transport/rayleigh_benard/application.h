...
private:
  ...
  set_parameters() final
  {
    ...
    this->param.nonlinear_solver_data_coupled =
      NonlinearSolver::SolverData(100, 1.e-12, 1.e-6, NonlinearSolver::SolverType::Newton);
  ...
  create_grid() final
  {
    ...
    dealii::GridTools::collect_periodic_faces(
      *this->grid->triangulation, 10, 11, 0, this->grid->periodic_face_pairs);
    if(dim == 3)
      dealii::GridTools::collect_periodic_faces(
        *this->grid->triangulation, 12, 13, 2, this->grid->periodic_face_pairs);
