...
public:
  ExactSolutionU(const double viscosity = 1., const double time = 0.)
    : Function<dim>(dim, time), nu(viscosity)
  {
  }
...
{
public:
  ExactSolutionP() : Function<dim>(1, 0)
  {
  }
...
bool
is_at_tick(const double time, const double time_step, const double tick)
{
...
template<int dim>
void
compute_error(FENavierStokesSolver<dim> & solver)
{
  ...
  VectorTools::integrate_difference(solver.dof_handler_u,
                                    ...
                                    ExactSolutionU<dim>(solver.get_viscosity(), solver.time),
                                    cellwise_errors,
                                    quadrature,
                                    VectorTools::L2_norm);
  ...
  if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
    std::cout << "  L2-Errors: ||e_p||_L2 = " << p_l2_error << ",   ||e_u||_L2 = " << u_l2_error
              << std::endl;
}
...
ChannelFlowProblem<dim>::run()
{
  // create mesh and solver
  {
    ...
    for(typename Triangulation<dim>::active_cell_iterator it = triangulation.begin();
      ...
      for(unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
        if(it->face(face)->at_boundary() && std::abs(it->face(face)->center()[0] - 2) < 1e-13)
          it->face(face)->set_boundary_id(1);
        else if(it->face(face)->at_boundary() && std::abs(it->face(face)->center()[0] + 2) < 1e-13)
          it->face(face)->set_boundary_id(2);
        else if(it->face(face)->at_boundary() && std::abs(it->face(face)->center()[1]) < 1e-13)
  ...
  solver.set_open_boundary_with_normal_flux(
    1, std::shared_ptr<Function<dim>>(new ExactSolutionP<dim>()));
  solver.set_velocity_dirichlet_boundary(
    2, std::shared_ptr<Function<dim>>(new ExactSolutionU<dim>(0.1)));
  // solver.set_open_boundary_with_normal_flux(2, std::shared_ptr<Function<dim> > (new
  // ExactSolutionP<dim>()));
  ...
  for(; solver.time < end_time;)
  {
    ...
    if(is_at_tick(solver.time, solver.get_time_step(), out_tick) || solver.time >= end_time)
    {
