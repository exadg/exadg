...
private:
  ...
  set_parameters() final
  {
    ...
    if(this->param.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
    {
      ...
      if(param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
        param.solver_data_momentum =
          SolverData(1e4, ABS_TOL_LINEARIZED, REL_TOL_LINEARIZED, "fgmres", 100);
      else
        param.solver_data_momentum = SolverData(1e4, ABS_TOL, REL_TOL, "fgmres", 100);
    ...
    if(param.treatment_of_convective_term == TreatmentOfConvectiveTerm::Implicit)
      ...
      param.solver_data_coupled = SolverData(1e4, ABS_TOL, REL_TOL, "fgmres", 100);
  ...
  set_parameters_ale_poisson() final
  {
    ...
    // SOLVER
    param.solver_data    = SolverData(1e4, ABS_TOL, REL_TOL, "fgmres", 100);
  ...
  set_parameters_ale_elasticity() final
  {
    ...
    if(param.large_deformation)
      param.solver_data = SolverData(1e4, ABS_TOL_LINEARIZED, REL_TOL_LINEARIZED, "fgmres", 100);
    else
      param.solver_data = SolverData(1e4, ABS_TOL, REL_TOL, "fgmres", 100);
    param.preconditioner                               = Preconditioner::Multigrid;
    param.multigrid_data.type                          = MultigridType::phMG;
