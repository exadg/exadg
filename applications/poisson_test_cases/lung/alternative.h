...
public:
  ...
  {
    return std_cxx14::make_unique<EllipseManifold<dim>>
      (center, direction, main_radius, cut_radius);
  }
                                            ...
                                            const double w) const
  {
    ...
    const double a1 = std::atan2((p1[2]-center[2])/cut_radius,
                                 (p1[1]-center[1])/main_radius);
    const double a2 = std::atan2((p2[2]-center[2])/cut_radius,
                                 (p2[1]-center[1])/main_radius);
...
template<int dim>
void create_lung(dealii::parallel::distributed::Triangulation<dim> & triat, int n_refine_space, int use_manifolds)
{
  ...
  const double ycord = 0.55*radius * std::cos(numbers::PI/12)/(std::sin(numbers::PI/12)+std::cos(numbers::PI/12));
  ...
  for (unsigned int i=0; i<vertices_up.size(); ++i)
    {
      ...
      vertices_3d[vertices_low.size()+i][2] = length-std::tan(numbers::PI/4)*std::abs(tria_2d.get_vertices()[i][0])/radius;
    }
  ...
  transform[2][2] =  std::sqrt(0.5);
  for (unsigned int i=0; i<8; ++i)
    vertices_3d[2*vertices_low.size()+8+i] = Point<dim>(offset+transform*auxiliary_points[i]);
      ...
      if (cell->at_boundary(f))
        {
          ...
          subcell_data.boundary_quads[count].vertices[2] = cell->face(f)->vertex_index(0)+tria_2d.n_vertices();
          subcell_data.boundary_quads[count].vertices[3] = cell->face(f)->vertex_index(1)+tria_2d.n_vertices();
  ...
  CylindricalManifold<dim> cm2 (Point<dim>{std::sqrt(0.5),0,std::sqrt(0.5)},Point<dim>{0,0,length});
  CylindricalManifold<dim> cm3 (Point<dim>{-std::sqrt(0.5),0,std::sqrt(0.5)},Point<dim>{0,0,length});
  EllipseManifold<dim> ellipse1(Point<dim>(0,0,length), Point<dim>(1,0,0),
                                radius, radius/std::sin(numbers::PI/3.));
