...
namespace Step7
{
  ...
  private:
    ...
    std_cxx11::shared_ptr<MappingQEulerian<dim,Vector<double> > >  mapping;
  ...
  void HelmholtzProblem<dim>::setup_system ()
  {
    ...
    */
    mapping.reset(new MappingQEulerian<dim,Vector<double> > (fe->degree, dof_handler, eulerian_vector));
  }
  ...
  {
    unsigned int sizes [] = {1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 14, 16, 20, 24, 28, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128, 144, 160, 176, 192, 208, 224, 240, 256};
    const unsigned int n_cycles = (refinement_mode==global_refinement)?sizeof(sizes)/sizeof(unsigned int):9;
    for (unsigned int cycle=0; cycle<n_cycles; ++cycle)
      {
        if (cycle == 0 || refinement_mode == global_refinement)
          {
            triangulation.clear();
            const unsigned int subdiv = refinement_mode == global_refinement ?
              sizes[cycle] : 1;
