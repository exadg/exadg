...
unsigned int const REFINE_TIME_MAX = 0; //12;
...
IncNS::FormulationViscousTerm const FORMULATION_VISCOUS_TERM = IncNS::FormulationViscousTerm::LaplaceFormulation;
...
//  param.multigrid_data_momentum.coarse_problem.solver = MultigridCoarseGridSolver::GMRES;
  ...
  param.newton_solver_data_coupled = NewtonSolverData(100,1.e-10,1.e-6); // TODO did not converge with 1.e-12
...
set_boundary_conditions(std::shared_ptr<ConvDiff::BoundaryDescriptor<1,dim>> boundary_descriptor)
{
  ...
  std::shared_ptr<Function<dim>> bc = set_mesh_movement_function<dim>();
...
{
public:
  NeumannBoundaryVelocityALE(const double time = 0.)
    ...
    std::shared_ptr<BoundaryDescriptorP<dim> > boundary_descriptor_pressure)
{
  ...
  if(ALE)
    boundary_descriptor_velocity->neumann_bc.insert(pair(1,new NeumannBoundaryVelocityALE<dim>()));
  else
...
construct_postprocessor(InputParameters const &param, MPI_Comm const &mpi_comm)
{
  ...
  pp_data.output_data.output_name = "fsi_test3";
