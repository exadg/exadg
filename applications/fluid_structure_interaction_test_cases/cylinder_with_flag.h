...
void
merge_triangulations(const std::vector<Triangulation<dim, spacedim>> &tria_vector,
                     ...
                     const bool copy_manifold_ids = false)
{
  ...
  for(auto tria = tria_vector.begin(); tria != tria_vector.end()-1; ++tria, ++alternating)
  {
    ...
    {
      AssertThrow(tria_vector.size() > 0, ExcMessage("A vector with at least two triangulations has to be provided."));
    }
  }
}
...
void create_triangulation(Triangulation<2> &tria)
{
  ...
  GridGenerator::general_cell(tria_vec[0], {Point<2>(X_0, 0.0),
...
function_space(double const x)
{
  ...
  if(x > X_C+R)
  {
    return AMPLITUDE * ( std::pow((x - (X_C+R))/L_FLAG, 2.0) - (1-std::cos((x - (X_C+R))/L_FLAG * 2.0 * numbers::PI))/4.0);
  }
  else
    return 0.0;
}
...
set_boundary_conditions(std::shared_ptr<ConvDiff::BoundaryDescriptor<1,dim>> boundary_descriptor)
{
  ...
  boundary_descriptor->dirichlet_bc.insert(pair(4, new MeshMotion<dim>(dim)));
}
    ...
    std::shared_ptr<BoundaryDescriptorP<dim> > boundary_descriptor_pressure)
{
  ...
  boundary_descriptor_velocity->dirichlet_bc.insert(pair(3,new Functions::ZeroFunction<dim>(dim)));
  boundary_descriptor_velocity->dirichlet_bc.insert(pair(4,new VelocityBendingWall<dim>(dim)));
...
construct_postprocessor(InputParameters const &param, MPI_Comm const &mpi_comm)
{
  ...
  pp_data.output_data.output_folder = "output/cylinder_with_flag/vtu/";
