...
private:
  ...
  std::shared_ptr<ConvDiff::DriverSteadyProblems<dim, fe_degree, value_type,
      ConvDiff::DGOperation<dim,fe_degree, value_type> > > driver_steady;
};
  ...
  n_refine_space(n_refine_space_in)
{
  ...
  driver_steady.reset(new ConvDiff::DriverSteadyProblems<dim, fe_degree, value_type,
      ConvDiff::DGOperation<dim,fe_degree, value_type> >(conv_diff_operation,postprocessor,param));
}
