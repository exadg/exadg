                                   ...
                                   Triangulation<dim>::cell_iterator> >        &periodic_faces)
{
 ...
 for(;cell!=endc;++cell)
 {
   for(unsigned int face_number=0;face_number < GeometryInfo<dim>::faces_per_cell;++face_number)
   {
    ...
    // after extrude triangulation to create the 3d mesh, the boundary IDs on the z-orthogonal boundaries are not 0!
    if(dim == 3)
    {
      if ((std::fabs(cell->face(face_number)->center()(2))< 1e-12) || (std::fabs(cell->face(face_number)->center()(2) - (L))< 1e-12))
