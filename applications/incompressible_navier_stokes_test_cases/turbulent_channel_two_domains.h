                                              ...
                                              const unsigned int component) const
{
  ...
  double weight_ym = (p[1]-Y_VALUES[iy-1])/(Y_VALUES[iy]-Y_VALUES[iy-1]);
  double weight_yp = 1 - weight_ym;
  double weight_zm = (p[2]-Z_VALUES[iz-1])/(Z_VALUES[iz]-Z_VALUES[iz-1]);
  double weight_zp = 1 - weight_zm;

  result =   weight_ym * weight_zm * VELOCITY_VALUES[(iy-1)*N_POINTS_Y + (iz-1)][component]
           + weight_ym * weight_zp * VELOCITY_VALUES[(iy-1)*N_POINTS_Y + (iz)][component]
           + weight_yp * weight_zm * VELOCITY_VALUES[(iy)*N_POINTS_Y + (iz-1)][component]
           + weight_yp * weight_zp * VELOCITY_VALUES[(iy)*N_POINTS_Y + (iz)][component];
...
public:
  PostProcessorTurbulentChannel(PostProcessorDataTurbulentChannel<dim> const & pp_data_turb_channel)
    ...
    inflow_data(pp_data_turb_channel.inflow_data),
