...
double const HEIGTH_CHANNEL = 2.0*H;
...
double const HEIGTH_BFS_STEP = H;
double const HEIGTH_BFS_INFLOW = HEIGTH_CHANNEL;
...
std::string OUTPUT_FOLDER_VTU = OUTPUT_FOLDER + "vtu_div_conti_all/";
...
void initialize_y_and_z_values()
{
  ...
  for(unsigned int i=0; i<N_POINTS_Y; ++i)
    Y_VALUES[i] = double(i)/double(N_POINTS_Y-1)*HEIGTH_CHANNEL;
...
void InputParametersNavierStokes<dim>::set_input_parameters(unsigned int const domain_id)
{
  ...
  if(domain_id == 1)
  {
    ...
    output_solver_info_every_timesteps = 100; //1e3; //1e4;
...
{
  double result = 0.0;
  double const y = (p[1] - HEIGTH_CHANNEL/2.0)/(HEIGTH_CHANNEL/2.0);
      ...
      Triangulation<dim>::cell_iterator> >                 &periodic_faces)
{
  ...
  else if(dim==3)
  {
    ...
    GridGenerator::subdivided_hyper_rectangle(tria_1,
                                              ...
                                              Point<dim>(0.0,HEIGTH_BFS_INFLOW,WIDTH_BFS/2.0));
    ...
    GridGenerator::subdivided_hyper_rectangle(tria_2,
                                              ...
                                              Point<dim>(LENGTH_BFS_DOWN,HEIGTH_BFS_INFLOW,WIDTH_BFS/2.0));

    // downstream part of BFS (lower = step)
    GridGenerator::subdivided_hyper_rectangle(tria_3,
                                              std::vector<unsigned int>({10,1,1}),
                                              Point<dim>(0.0,0.0,-WIDTH_BFS/2.0),
                                              Point<dim>(LENGTH_BFS_DOWN,-HEIGTH_BFS_STEP,WIDTH_BFS/2.0));
...
{
public:
  PostProcessorBFS(PostProcessorDataBFS<dim> const & pp_data_bfs)
    :
    PostProcessor<dim,fe_degree_u,fe_degree_p, Number>(pp_data_bfs.pp_data),
    write_final_output(true),
    turb_ch_data(pp_data_bfs.turb_ch_data)
  {
...
construct_postprocessor(InputParametersNavierStokes<dim> const &param)
{
  ...
  PostProcessorDataBFS<dim> pp_data_turb_ch;
  ...
  pp.reset(new PostProcessorBFS<dim,FE_DEGREE_VELOCITY,FE_DEGREE_PRESSURE,Number>(pp_data_turb_ch));
