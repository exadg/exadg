...
template<>
double
vectorize_or_not<double, double>(double val)
{
  return val;
}
...
void
zero_out(Number &, const std::vector<bool> &)
{
  ;
}
...
template<int dim, typename Number>
void
zero_out(VectorizedArray<Number> & qp_enrichment, const std::vector<bool> & enriched_components)
{
...
template<int dim, typename Number>
void zero_out(Tensor<1, dim, VectorizedArray<Number>> & qp_grad_enrichment,
...
bool
convergence_check(VectorizedArray<Number> & inc,
                  VectorizedArray<Number> & fn,
                  const int                 count,
                  const std::vector<bool> & enriched_components)
{
  bool test = false;
...
template<typename Number>
bool
convergence_check(Number & inc, Number & fn, const int count, const std::vector<bool> &)
{
  bool test = false;
...
void
SpaldingsLawEvaluation<dim, Number, V>::reinit_zero(const unsigned int n_q_points)
{
...
template<int dim, typename Number, typename V>
void
SpaldingsLawEvaluation<dim, Number, V>::reinit(
  const AlignedVector<V> &                 qp_wdist,
  ...
  const std::vector<bool> &                enriched_components)
{
  ...
  for(unsigned int q = 0; q < n_q_points; ++q)
  {
    qp_enrichment[q] = EnrichmentShapeDer(qp_wdist[q],
                                          qp_tauw[q],
                                          qp_gradwdist[q],
                                          qp_gradtauw[q],
                                          qp_grad_enrichment[q],
                                          enriched_components);
...
template<int dim, typename Number, typename V>
void
SpaldingsLawEvaluation<dim, Number, V>::reinit(const AlignedVector<V> & qp_wdist,
                                               const AlignedVector<V> & qp_tauw,
                                               const unsigned int       n_q_points)
{
  qp_enrichment.resize(n_q_points);
  ...
  {
    const V utau =
      std::sqrt(qp_tauw[q] * internalSpalding::vectorize_or_not<Number, V>((Number)1.0) / density);
...
template<int dim, typename Number, typename V>
V
SpaldingsLawEvaluation<dim, Number, V>::EnrichmentShapeDer(
  const V &                 wdist,
  const V &                 tauw,
  ...
  const std::vector<bool> & enriched_components)
{
  ...
  const V utau =
    std::sqrt(tauw * internalSpalding::vectorize_or_not<Number, V>((Number)1.0) / density);
  const V fac =
    internalSpalding::vectorize_or_not<Number, V>((Number)0.5) / std::sqrt(density * tauw);
  const V wdistfac = wdist * fac;

  for(unsigned int sdm = 0; sdm < dim; ++sdm)
    gradtrans[sdm] = (utau * gradwdist[sdm] + wdistfac * gradtauw[sdm]) *
                     internalSpalding::vectorize_or_not<Number, V>((Number)1.0 / viscosity);

  // get enrichment function and scalar derivatives
  V psigp = SpaldingsLaw(wdist, utau, enriched_components) *
            internalSpalding::vectorize_or_not<Number, V>((Number)1.0);
  V derpsigpsc =
    DerSpaldingsLaw(psigp) * internalSpalding::vectorize_or_not<Number, V>((Number)1.0);
...
template<int dim, typename Number, typename V>
void
SpaldingsLawEvaluation<dim, Number, V>::initial_value(VectorizedArray<Number> &       psi,
...
template<int dim, typename Number, typename V>
void
SpaldingsLawEvaluation<dim, Number, V>::initial_value(Number &       psi,
                                                      const Number & yplus,
...
template<int dim, typename Number, typename V>
V
SpaldingsLawEvaluation<dim, Number, V>::SpaldingsLaw(const V                   dist,
                                                     const V                   utau,
                                                     const std::vector<bool> & enriched_components)
{
  ...
  // multiplications
  const V yplus =
    dist * utau * internalSpalding::vectorize_or_not<Number, V>((Number)1.0 / viscosity);
  ...
  while(not converged)
  {
    ...
    fn        = -yplus + psi * internalSpalding::vectorize_or_not<Number, V>(km1) +
         internalSpalding::vectorize_or_not<Number, V>(expmkmb) *
           (exppsi - internalSpalding::vectorize_or_not<Number, V>((Number)1.0) - psi -
            psiquad * num2m1 - psiquad * psi * num6m1 -
    ...
    converged =
      not internalSpalding::convergence_check<Number>(inc, fn, count, enriched_components);
    count++;
  }
...
template<int dim, typename Number, typename V>
V
SpaldingsLawEvaluation<dim, Number, V>::DerSpaldingsLaw(const V psi)
{
  // derivative with respect to y+!
  // spaldings law according to paper (derivative)
  return internalSpalding::vectorize_or_not<Number, V>((Number)1.0) /
         (internalSpalding::vectorize_or_not<Number, V>(km1) +
          internalSpalding::vectorize_or_not<Number, V>(expmkmb) *
            (std::exp(psi) - internalSpalding::vectorize_or_not<Number, V>((Number)1.0) - psi -
             psi * psi * num2m1 - psi * psi * psi * num6m1));
...
template<>
void
SpaldingsLawEvaluation<3, double, VectorizedArray<double>>::reinit(
  const AlignedVector<VectorizedArray<double>> &               enrichment_in,
...
template<>
void
SpaldingsLawEvaluation<2, double, VectorizedArray<double>>::reinit(
  const AlignedVector<VectorizedArray<double>> &               enrichment_in,
...
template VectorizedArray<double>
internalSpalding::vectorize_or_not<double, VectorizedArray<double>>(double val);
template double
internalSpalding::vectorize_or_not<double, double>(double val);
template VectorizedArray<float>
internalSpalding::vectorize_or_not<float, VectorizedArray<float>>(float val);
