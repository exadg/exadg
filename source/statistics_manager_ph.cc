...
#  ifdef DEBUG_Y
                std::cout
                  << "something went wrong when sorting y in vector y_loc. The algorythm sees that index idx = "
                  << idx
                  << " is appropriate according to its distance to y, but this index has already been used !!! -> the code sort this value in next higher position of y_loc (idx = idx+1) "
                  << std::endl;
#  endif
...
//      geom_appr_error = std::max_element(diff.);
    ...
    for(unsigned int i = 0; i < n_points_y_glob; ++i)
    {
      AssertThrow(
        ...
        ExcMessage(
          "The geometry approximation is too bad for the statistics_manager_PH to work. Use minimum l3_p2 for a GRID_STRETCH_FAC up to 2.5 (using the double amount of cells in x-direction). "
      ...
      //            ExcMessage("WARNING (If you think its OK comment this assert out!): The geometry
      //            approximation is too bad for the statistics_manager_PH to work. Use minimum
      //            l3_p2 or l2_p4."
      //                       "std::abs(y_vec_glob[i_x].at(i) - tmp.at(i)) = " +
      //                       patch::to_string(std::abs(y_vec_glob[i_x].at(i) - tmp.at(i))) + " and
  ...
  if(Utilities::MPI::this_mpi_process(communicator) == 0)
  {
    ...
    for(unsigned int idx = 0; idx < x_glob.size(); idx++)
    {
      double tau_w = -viscosity * dudy_top_glob.at(idx) /
                     (double)numchsamp; // minus is set because on the top of the domain, the domain
