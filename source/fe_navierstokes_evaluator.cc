...
public:
  ...
  virtual void
  do_advect(const parallel::distributed::Vector<double> & src,
            parallel::distributed::Vector<double> &       dst) const = 0;

  virtual void
  do_divergence(const parallel::distributed::Vector<double> & src,
                parallel::distributed::Vector<double> &       dst) const = 0;

  virtual void
  do_curl(const parallel::distributed::Vector<double> & src,
          parallel::distributed::Vector<double> &       dst) const = 0;
};
...
public:
  ...
  {
    matrix_free.cell_loop(&FENavierStokesEvaluatorImpl::local_advect, this, dst, src);
  }
  ...
  do_divergence(const parallel::distributed::Vector<double> & src,
                parallel::distributed::Vector<double> &       dst) const
  {
    matrix_free.cell_loop(&FENavierStokesEvaluatorImpl::local_divergence, this, dst, src);
  }
  ...
  do_curl(const parallel::distributed::Vector<double> & src,
          parallel::distributed::Vector<double> &       dst) const
  {
    matrix_free.cell_loop(&FENavierStokesEvaluatorImpl::local_vorticity, this, dst, src);
  }
...
private:
  ...
  void
  local_advect(const MatrixFree<dim> &,
               parallel::distributed::Vector<double> &       dst,
               const parallel::distributed::Vector<double> & src,
               const std::pair<unsigned int, unsigned int> & cell_range) const
  {
    const VectorizedArray<double> inv_time_step =
      make_vectorized_array(1. / fluid_algorithm.get_time_step());
    ...
    const Tensor<1, dim> constant_body_force =
      body_force_is_constant ? fluid_algorithm.get_body_force(Point<dim>()) : Tensor<1, dim>();
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      for(unsigned int q = 0; q < phi_u.n_q_points; ++q)
      {
        ...
        Tensor<1, dim, VectorizedArray<double>> mom_val = grad * vel + 0.5 * div * vel;
  ...
  void
  local_divergence(const MatrixFree<dim> &,
                   parallel::distributed::Vector<double> &       dst,
                   const parallel::distributed::Vector<double> & src,
                   const std::pair<unsigned int, unsigned int> & cell_range) const
  {
  ...
  void
  local_vorticity(const MatrixFree<dim> &,
                  parallel::distributed::Vector<double> &       dst,
                  const parallel::distributed::Vector<double> & src,
                  const std::pair<unsigned int, unsigned int> & cell_range) const
  {
...
template<int dim>
FENavierStokesEvaluator<dim>::FENavierStokesEvaluator(
  const MatrixFree<dim> &                       matrix_free,
  const parallel::distributed::Vector<double> & pressure,
  const parallel::distributed::Vector<double> & last_pressure_update,
  const FluidBaseAlgorithm<dim> &               fluid_algorithm)
...
template<int dim>
void
FENavierStokesEvaluator<dim>::advection_integrals(const parallel::distributed::Vector<double> & src,
                                                  parallel::distributed::Vector<double> & dst) const
{
...
template<int dim>
void
FENavierStokesEvaluator<dim>::divergence_integrals(
  const parallel::distributed::Vector<double> & src,
  parallel::distributed::Vector<double> &       dst) const
{
...
template<int dim>
void
FENavierStokesEvaluator<dim>::curl_integrals(const parallel::distributed::Vector<double> & src,
                                             parallel::distributed::Vector<double> & dst) const
{
