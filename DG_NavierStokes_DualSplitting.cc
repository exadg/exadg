...
namespace dealii
{
  ...
  void prolongation_vmult(const bool transpose_and_add,
                          const TrilinosWrappers::SparseMatrix &mat,
                          const parallel::distributed::Vector<Number> &src,
                          parallel::distributed::Vector<Number> &dst)
  {
    parallel::distributed::Vector<double> src_copy, dst_copy;
    src_copy = src;
    dst_copy = dst;
    if (transpose_and_add)
      mat.Tvmult_add(dst_copy, src_copy);
    else
      mat.vmult(dst_copy, src_copy);
    dst = dst_copy;
  }
    ...
    const VECTOR      &src) const
  {
    ...
    prolongation_vmult(false, *prolongation_matrices[to_level-1], src, dst);
  }
    ...
    const VECTOR &src) const
  {
    ...
    prolongation_vmult(true, *prolongation_matrices[from_level-1], src, dst);
  }
...
namespace DG_NavierStokes
{
  ...
  const double VISCOSITY = 0.1; // Taylor vortex: 0.01; vortex problem (Hesthaven): 0.025; Poisseuille 0.005; Kovasznay 0.025; Stokes 1.0
  const double MAX_VELOCITY = 3.5; // Taylor vortex: 1; vortex problem (Hesthaven): 1.5; Poisseuille 1.0; Kovasznay 4.0
...
//    result = -pi*cost*sin2x*pow(siny,2.);
  ...
  template<int dim, int fe_degree, int fe_degree_p, typename value_type>
  class MGCoarsePressure : public MGCoarseGridBase<parallel::distributed::Vector<value_type> >
  {
  public:
    MGCoarsePressure() {}
    ...
    {
      ReductionControl solver_control (1e3, 1e-15, 1e-4);
      SolverCG<parallel::distributed::Vector<value_type> > solver_coarse (solver_control);
  ...
  template<int dim, int fe_degree, int fe_degree_p, typename value_type>
  class MGCoarseViscous : public MGCoarseGridBase<parallel::distributed::Vector<value_type> >
  {
  public:
    MGCoarseViscous() {}
    ...
    virtual void operator() (const unsigned int   level,
                             ...
                             const parallel::distributed::Vector<value_type> &src) const
    {
      ReductionControl solver_control (1e3, 1e-16, 1e-4);
      SolverCG<parallel::distributed::Vector<value_type> > solver_coarse (solver_control);
  ...
  private:
    ...
    MGTransferPrebuilt<parallel::distributed::Vector<value_type> > mg_transfer_pressure;

    typedef PreconditionChebyshev<NavierStokesPressureMatrix<dim,fe_degree, fe_degree_p>,
            parallel::distributed::Vector<value_type> > SMOOTHER_PRESSURE;
    ...
    MGSmootherPrecondition<NavierStokesPressureMatrix<dim,fe_degree,fe_degree_p>,
                           SMOOTHER_PRESSURE, parallel::distributed::Vector<value_type> > mg_smoother_pressure;
    MGCoarsePressure<dim,fe_degree,fe_degree_p,value_type> mg_coarse_pressure;

    MGLevelObject<NavierStokesViscousMatrix<dim,fe_degree,fe_degree_p> > mg_matrices_viscous;
    MGTransferPrebuilt<parallel::distributed::Vector<value_type> > mg_transfer_viscous;

    typedef PreconditionChebyshev<NavierStokesViscousMatrix<dim,fe_degree, fe_degree_p>,
            parallel::distributed::Vector<value_type> > SMOOTHER_VISCOUS;
    ...
    MGSmootherPrecondition<NavierStokesViscousMatrix<dim,fe_degree,fe_degree_p>,
                           SMOOTHER_VISCOUS, parallel::distributed::Vector<value_type> > mg_smoother_viscous;
    MGCoarseViscous<dim,fe_degree,fe_degree_p,value_type> mg_coarse_viscous;
    ...
    dof_handler_wall_distance(dof_handler.get_tria())
  {
    ...
    for (unsigned int level=mg_matrices_pressure.min_level(); level<=mg_matrices_pressure.max_level(); ++level)
      {
        ...
        dof_handler_vec.push_back(&dof_handler_wall_distance);
    ...
    data.back().initialize_dof_vector(wall_distance, 2);
          ...
          for (unsigned int v=0; v<GeometryInfo<dim>::vertices_per_cell; ++v)
            {
              ...
              const double distance_value = std::sqrt(p[1] * p[1]);
              wall_distance(element_dof_indices[v]) = distance_value;
            }
        }
    //wall_distance.print(std::cout);
    Vector<value_type> local_distance_values(fe_wall_distance.dofs_per_cell);
    for (typename DoFHandler<dim>::active_cell_iterator cell=dof_handler_wall_distance.begin_active();
         cell != dof_handler_wall_distance.end(); ++cell)
      if (cell->is_locally_owned())
        {
  ...
  struct NavierStokesPressureMatrix : public Subscriptor
  {
    ...
    void Tvmult (parallel::distributed::Vector<value_type> &dst,
                 const parallel::distributed::Vector<value_type> &src) const
    {
      dst = 0;
      vmult_add(dst,src);
    ...
    void Tvmult_add (parallel::distributed::Vector<value_type> &dst,
                     const parallel::distributed::Vector<value_type> &src) const
    {
      vmult_add(dst,src);
    }

    void vmult_add (parallel::distributed::Vector<value_type> &dst,
                    const parallel::distributed::Vector<value_type> &src) const
    {
      if (pure_dirichlet_bc)
        {
          parallel::distributed::Vector<value_type> temp1(src);
...
//    data_out.write_vtk(output);
    ...
    ReductionControl solver_control (1e3, 1.e-15, 1e-6);
...
//    solution = solution_n[dim];
    ...
    mg::Matrix<parallel::distributed::Vector<value_type> > mgmatrix_pressure(mg_matrices_pressure);
    Multigrid<parallel::distributed::Vector<value_type> > mg_pressure(data.back().get_dof_handler(1),
        mgmatrix_pressure,
        mg_coarse_pressure,
        mg_transfer_pressure,
        mg_smoother_pressure,
        mg_smoother_pressure);
    PreconditionMG<dim, parallel::distributed::Vector<value_type>, MGTransferPrebuilt<parallel::distributed::Vector<value_type> > >
...
//  if (false)
//  {
//    parallel::distributed::Vector<value_type> check1(mg_matrices_pressure[mg_matrices_pressure.max_level()].m()),
...
//  }
    ...
    // set maximum number of iterations, tolerance
    ReductionControl solver_control_velocity (1e3, 1.e-15, 1e-8);
    SolverCG<parallel::distributed::Vector<value_type> > solver_velocity (solver_control_velocity);
...
//    solution = solution_n[d];
        ...
        // PCG-Solver with GMG + Chebyshev smoother as a preconditioner
//    mg::Matrix<parallel::distributed::Vector<value_type> > mgmatrix_viscous(mg_matrices_viscous);
//    Multigrid<parallel::distributed::Vector<value_type> > mg_viscous(data.back().get_dof_handler(0),
//                               mgmatrix_viscous,
//                               mg_coarse_viscous,
//                               mg_transfer_viscous,
//                               mg_smoother_viscous,
        ...
        for (unsigned int j=0; j<pressure.dofs_per_cell; ++j)
          {
            for (unsigned int i=0; i<pressure.dofs_per_cell; ++i)
              pressure.begin_dof_values()[i] = make_vectorized_array<value_type>(0.);
            pressure.begin_dof_values()[j] = make_vectorized_array<value_type>(1.);
        ...
        for (unsigned int j=0; j<velocity.dofs_per_cell; ++j)
          {
            for (unsigned int i=0; i<velocity.dofs_per_cell; ++i)
              velocity.begin_dof_values()[i] = make_vectorized_array<value_type>(0.);
            velocity.begin_dof_values()[j] = make_vectorized_array<value_type>(1.);
...
//        data_out.add_data_vector (dof_handler,vorticity[d], "vorticity_" + Utilities::int_to_string(d+1));
  ...
  void NavierStokesProblem<dim>::calculate_time_step()
  {
    ...
    double a, max_cell_a = std::numeric_limits<double>::min();
    for (; cell!=endc; ++cell)
      {
