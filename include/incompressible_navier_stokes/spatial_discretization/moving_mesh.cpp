...
MovingMesh<dim, Number>::MovingMesh(InputParameters const & param_in,
...
template<int dim, typename Number>
  ...
  MovingMesh<dim, Number>::get_analytical_grid_velocity(double const evaluation_time)
  {
    ...
    VectorTools::interpolate(*mapping_init,//coordinates on which analytical_solution_grid_velocity() relies
                             dof_handler_u_grid,
  ...
  void MovingMesh<dim, Number>::interpolate_mg(MappingTypeIn& mapping_in)
  {
  ...
  template<int dim, typename Number>
  void
  MovingMesh<dim, Number>::compute_grid_velocity(std::vector<Number> time_integrator_constants, double time_step_size)
  {
    push_back(d_grid);
    fill_d_grid();
