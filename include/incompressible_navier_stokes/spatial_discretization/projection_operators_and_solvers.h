/*
 * projection_operators_and_solvers.h
 *
...
{
/*
 * Base class for different projection operators. This class is derived from the BaseOperator where
 * we just add the time step size as element variable along with the respective set() and
 * get()-functions.
 */
template<int dim>
...
{
public:
  ProjectionOperatorBase() : time_step_size(1.0)
  {
  }
...
public:
  ...
  ProjectionOperatorDivergencePenaltyIterative(
    DivergencePenaltyOperator<dim,
                              fe_degree,
                              fe_degree_p,
                              fe_degree_xwall,
                              xwall_quad_rule,
                              value_type> const & div_div_penalty)
    : fe_eval(1,
  ...
  void
  precondition(VectorizedArray<value_type> * dst, const VectorizedArray<value_type> * src) const
  {
...
private:
  ...
  DivergencePenaltyOperator<dim,
                            ...
                            fe_degree_xwall,
                            xwall_quad_rule,
                            value_type> const * divergence_penalty_operator;
};
...
class ElementwiseInverseMassMatrixPreconditioner
  : public InternalSolvers::PreconditionerBase<value_type>
{
public:
  ElementwiseInverseMassMatrixPreconditioner(Operator & operator_in) : op(operator_in)
  {
  }
...
public:
  ...
  ProjectionOperatorDivergencePenaltyDirect(
    DivergencePenaltyOperator<dim,
                              fe_degree,
                              fe_degree_p,
                              fe_degree_xwall,
...
public:
  ...
  ProjectionOperatorDivergenceAndContinuityPenalty(
    MassMatrixOperator<dim, fe_degree, fe_degree_xwall, xwall_quad_rule, value_type> const &
    ...
    DivergencePenaltyOperator<dim,
                              ...
                              fe_degree_xwall,
                              xwall_quad_rule,
                              value_type> const & divergence_penalty_operator_in,
    ContinuityPenaltyOperator<dim,
                              fe_degree,
                              fe_degree_p,
                              fe_degree_xwall,
                              xwall_quad_rule,
                              value_type> const & continuity_penalty_operator_in)
        ...
        parallel::distributed::Vector<value_type> const & src) const
  {
    ...
    divergence_penalty_operator->apply(dst, src);
  ...
  update_inverse_block_diagonal() const
  {
    if(block_jacobi_matrices_have_been_initialized == false)
    {
      ...
      unsigned int dofs_per_cell =
        this->mass_matrix_operator->get_data().get_shape_info().dofs_per_component_on_cell * dim;

      matrices.resize(this->mass_matrix_operator->get_data().n_macro_cells() *
...
private:
  ...
  calculate_block_jacobi_matrices() const
  {
    ...
    for(typename std::vector<LAPACKFullMatrix<value_type>>::iterator it = matrices.begin();
        it != matrices.end();
        ++it)
    {
  ...
  ContinuityPenaltyOperator<dim,
                            ...
                            fe_degree_xwall,
                            xwall_quad_rule,
                            value_type> const * continuity_penalty_operator;
              ...
              const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      const VectorizedArray<value_type> tau =
        projection_operator->get_time_step_size() *
...
{
  ProjectionSolverData() : max_iter(1e4), solver_tolerance_abs(1.e-12), solver_tolerance_rel(1.e-6)
  {
  }
...
public:
  ...
  IterativeProjectionSolverDivergencePenalty(PROJ_OPERATOR &            projection_operator_in,
    ...
    : projection_operator(projection_operator_in), solver_data(solver_data_in)
  {
  }
              ...
              const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    InternalSolvers::SolverCG<VectorizedArray<value_type>> cg_solver(
      total_dofs_per_cell,
    ...
    // TODO
    //    InternalSolvers::PreconditionerIdentity<VectorizedArray<value_type> >
    //    preconditioner(total_dofs_per_cell);

    ElementwiseInverseMassMatrixPreconditioner<VectorizedArray<value_type>, PROJ_OPERATOR>
      preconditioner(projection_operator);
...
public:
  ...
  calculate_array_penalty_parameter_div(parallel::distributed::Vector<value_type> const & velocity)
  {
    ...
    for(unsigned int cell = 0; cell < data.n_macro_cells() + data.n_macro_ghost_cells(); ++cell)
    {
      ...
      {
        array_penalty_parameter_div[cell] =
    ...
    for(unsigned int cell = 0; cell < data.n_macro_cells() + data.n_macro_ghost_cells(); ++cell)
    {
      ...
      else if(operator_data.type_penalty_parameter == TypePenaltyParameter::ViscousTerm)
      {
        array_penalty_parameter_conti[cell] = operator_data.penalty_parameter * tau_viscous;
              ...
              TypePenaltyParameter::ViscousAndConvectiveTerms)
      {
        array_penalty_parameter_conti[cell] =
          operator_data.penalty_parameter * (tau_convective + tau_viscous);
      }
    }
  }
  ...
  AlignedVector<VectorizedArray<value_type>> const &
  get_array_penalty_parameter_conti() const
  {
    return array_penalty_parameter_conti;
  }

  AlignedVector<VectorizedArray<value_type>> const &
  get_array_penalty_parameter_div() const
  {
...
private:
            ...
            const std::pair<unsigned int, unsigned int> &     cell_range) const
  {
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      VectorizedArray<value_type> tau =
        this->get_array_penalty_parameter_div()[cell] * this->get_time_step_size();
            ...
            const std::pair<unsigned int, unsigned int> &     face_range) const
  {
    ...
    for(unsigned int face = face_range.first; face < face_range.second; face++)
    {
      ...
      VectorizedArray<value_type> tau =
        0.5 *
        (fe_eval.read_cell_data(this->get_array_penalty_parameter_conti()) +
