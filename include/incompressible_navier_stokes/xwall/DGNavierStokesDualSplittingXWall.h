...
#ifndef INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGNAVIERSTOKESDUALSPLITTINGXWALL_H_
#define INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGNAVIERSTOKESDUALSPLITTINGXWALL_H_

#include <deal.II/base/utilities.h>

#include "../../incompressible_navier_stokes/spatial_discretization/dg_navier_stokes_dual_splitting.h"
#include "../../incompressible_navier_stokes/xwall/InverseMassMatrixXWall.h"

template<int dim>
class Enrichment;

template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
class DGNavierStokesDualSplittingXWall : public DGNavierStokesDualSplitting<dim,
                                                                            fe_degree,
                                                                            fe_degree_p,
                                                                            fe_degree_xwall,
                                                                            xwall_quad_rule>
...
public:
  ...
  enum class DofHandlerSelector
  {
    ...
    n_variants =
      static_cast<typename std::underlying_type<DofHandlerSelector>::type>(wdist_tauw) + 1
  };
  ...
  {
    this->fe_u.reset(
      new FESystem<dim>(FE_DGQArbitraryNodes<dim>(QGaussLobatto<1>(fe_degree + 1)),
                        dim,
                        FE_DGQArbitraryNodes<dim>(QGaussLobatto<1>(fe_degree_xwall + 1)),
                        dim));
  }
  ...
  get_dof_handler_wdist() const
  {
    return dof_handler_wdist;
  }

  FEParameters<dim> const &
  get_fe_parameters_n() const
  {
    return fe_param_n;
  }
...
private:
  ...
  void
  local_rhs_normalization_boundary_face(
...
private:
  ...
  std::shared_ptr<
    InverseMassMatrixXWallOperator<dim, fe_degree, fe_degree_xwall, xwall_quad_rule, value_type>>
...
DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  setup(const std::vector<GridTools::PeriodicFacePair<typename Triangulation<dim>::cell_iterator>>
        ...
        std::shared_ptr<FieldFunctionsNavierStokes<dim>>     field_functions)
{
  DGNavierStokesBase<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::setup(
    periodic_face_pairs,
  ...
  this->inverse_mass_matrix_operator.reset(new InverseMassMatrixXWallOperator<dim,
                                                                              fe_degree,
                                                                              fe_degree_xwall,
                                                                              xwall_quad_rule,
                                                                              value_type>());
  inverse_mass_matrix_operator_xwall = std::dynamic_pointer_cast<
    InverseMassMatrixXWallOperator<dim, fe_degree, fe_degree_xwall, xwall_quad_rule, value_type>>(
    this->inverse_mass_matrix_operator);
  inverse_mass_matrix_operator_xwall->initialize(
    this->data,
    this->fe_param,
  ...
  data_reinit(typename MatrixFree<dim, value_type>::AdditionalData & additional_data)
{
  ...
  dof_handler_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::wdist_tauw)] = &dof_handler_wdist;
  ...
  constraint_matrix_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::wdist_tauw)] = &constraint_periodic;
  ...
  init_wdist()
{
  ...
  AssertThrow(dof_handler_wdist.n_dofs() < (types::global_dof_index(1ull) << 53),
              ExcMessage("Sizes larger than 2^53 currently not supported"));
#endif

  IndexSet locally_relevant_set;
  DoFTools::extract_locally_relevant_dofs(this->dof_handler_wdist, locally_relevant_set);
  ...
  // directions
  {
    ...
    FEFaceValues<dim> fe_face_values(this->fe_wdist, face_quadrature, update_normal_vectors);
    std::vector<types::global_dof_index> dof_indices(this->fe_wdist.dofs_per_face);
    int                                  found = 0;
    for(typename DoFHandler<dim>::active_cell_iterator cell =
          this->dof_handler_wdist.begin_active();
        cell != this->dof_handler_wdist.end();
        ++cell)
      if(cell->is_locally_owned())
        for(unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
          if(cell->at_boundary(f) &&
             wall_boundaries.find(cell->face(f)->boundary_id()) != wall_boundaries.end())
          {
            found = 1;
            ...
            for(unsigned int i = 0; i < dof_indices.size(); ++i)
            {
              ...
              if(constraint_periodic.is_constrained(dof_indices[i]))
                aux_vectors[dim + 1](dof_indices[i]) =
                  (*constraint_periodic.get_constraint_entries(dof_indices[i]))[0].first;
              else
  ...
  while(aux_vectors[dim].linfty_norm() == unreached)
  {
    ...
    AssertThrow(this->fe_wdist.dofs_per_cell == GeometryInfo<dim>::vertices_per_cell,
                ExcNotImplemented());
              ...
              if(distances_step(dof_indices[w]) < unreached)
              {
                ...
                */
                   ...
                   aux_vectors[dim](dof_indices[v]))
                {
                  ...
                  aux_vectors[dim + 1](dof_indices[v]) = aux_vectors[dim + 1](dof_indices[w]);
                  for(unsigned int d = 0; d < dim; ++d)
                    aux_vectors[d](dof_indices[v]) += aux_vectors[d](dof_indices[w]);
  ...
  IndexSet accessed_indices(aux_vectors[dim + 1].size());
  {
    ...
    my_indices.erase(std::unique(my_indices.begin(), my_indices.end()), my_indices.end());
    accessed_indices.add_indices(my_indices.begin(), my_indices.end());
  }
  ...
  rhs_for_inverse_mass_matrix(parallel::distributed::Vector<value_type> & velocity)
{
  this->data.cell_loop(
    &DGNavierStokesDualSplittingXWall<dim,
                                      ...
                                      fe_degree_p,
                                      fe_degree_xwall,
                                      xwall_quad_rule>::local_rhs_for_inverse_mass_matrix,
    this,
    velocity,
    velocity);
}
...
DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  local_rhs_for_inverse_mass_matrix(const MatrixFree<dim, value_type> &               data,
                                    ...
                                    const std::pair<unsigned int, unsigned int> &     cell_range)
{
  FEEval_Velocity_Velocity_linear fe_eval_velocity(
    data,
    this->fe_param,
    static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
      DofHandlerSelector::velocity));
...
DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  xwall_projection(parallel::distributed::Vector<value_type> & velocity)
{
  this->data.cell_loop(&DGNavierStokesDualSplittingXWall<dim,
                                                         fe_degree,
                                                         fe_degree_p,
                                                         fe_degree_xwall,
                                                         xwall_quad_rule>::local_project_xwall,
                       this,
                       velocity,
                       velocity);
...
DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  local_project_xwall(const MatrixFree<dim, value_type> &               data,
                      ...
                      const std::pair<unsigned int, unsigned int> &     cell_range)
{
  FEEval_Velocity_Velocity_linear fe_eval_velocity_n(
    data,
    &this->fe_param_n,
    static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
      DofHandlerSelector::velocity));
  // FEEvaluationXWall<dim,fe_degree,fe_degree_xwall,xwall_quad_rule,1,value_type> fe_eval_xwall_n
  // (data,xwallstatevec[0],*xwall.ReturnTauWN(),0,3);
  FEEval_Velocity_Velocity_linear fe_eval_velocity(
    data,
    &this->fe_param,
    static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
  ...
  calculate_wall_shear_stress(const parallel::distributed::Vector<value_type> & src)
{
  ...
  this->data.loop(&DGNavierStokesDualSplittingXWall<dim,
                                                    fe_degree,
                                                    fe_degree_p,
                                                    fe_degree_xwall,
                                                    xwall_quad_rule>::local_rhs_dummy,
                  &DGNavierStokesDualSplittingXWall<dim,
                                                    fe_degree,
                                                    fe_degree_p,
                                                    fe_degree_xwall,
                                                    xwall_quad_rule>::local_rhs_dummy_face,
                  &DGNavierStokesDualSplittingXWall<dim,
                                                    fe_degree,
                                                    fe_degree_p,
                                                    fe_degree_xwall,
                                                    xwall_quad_rule>::local_rhs_wss_boundary_face,
                  this,
                  force,
                  src);
  ...
  this->data.loop(
    ...
    &DGNavierStokesDualSplittingXWall<dim,
                                      ...
                                      fe_degree_p,
                                      fe_degree_xwall,
                                      xwall_quad_rule>::local_rhs_normalization_boundary_face,
    this,
    normalization,
    src);
  ...
  for(unsigned int i = 0; i < tauw.local_size(); ++i)
    tauw.local_element(i) =
      (1. - this->param.dtauw) * tauw_n.local_element(i) +
      this->param.dtauw * tauw_boundary.local_element(vector_to_tauw_boundary[i]);
}
  ...
  precompute_spaldings_law()
{
  ...
  this->data.cell_loop(
    &DGNavierStokesDualSplittingXWall<dim,
                                      ...
                                      fe_degree_p,
                                      fe_degree_xwall,
                                      xwall_quad_rule>::local_precompute_spaldings_law,
    this,
    dummy,
    dummy);
}
...
DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  local_precompute_spaldings_law(const MatrixFree<dim, value_type> & data,
                                 ...
                                 const std::pair<unsigned int, unsigned int> & cell_range)
{
  FEEvaluationWrapper<dim, fe_degree, fe_degree_xwall, xwall_quad_rule, dim, value_type, true>
                                                             fe_eval(data,
            &this->fe_param,
            static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
              DofHandlerSelector::velocity));
                              ...
                              const std::pair<unsigned int, unsigned int> &     face_range) const
{
  ...
  if(this->fe_param.max_wdist_xwall > EPSILON)
  {
    ...
    FEFaceEvaluation<dim, 1, xwall_quad_rule, 1, value_type> fe_eval_tauw(data, true, 2, 3);
    ...
    for(unsigned int face = face_range.first; face < face_range.second; face++)
    {
      if(data.get_boundary_indicator(face) == 0) // Infow and wall boundaries
      {
        ...
        AssertThrow(fe_eval_velocity_face.n_q_points == fe_eval_tauw.n_q_points,
        ...
        for(unsigned int q = 0; q < fe_eval_velocity_face.n_q_points; ++q)
        {
          Tensor<1, dim, VectorizedArray<value_type>> average_gradient =
            fe_eval_velocity_face.get_normal_gradient(q);
  ...
  else
  {
    FEFaceEval_Velocity_Velocity_linear fe_eval_velocity_face(data, &this->fe_param, true, 0);
    // these faces should always be enriched, therefore quadrature rule enriched (3)
    FEFaceEvaluation<dim, 1, fe_degree + (fe_degree + 2) / 2, 1, value_type> fe_eval_tauw(data,
                                                                                          true,
                                                                                          2,
                                                                                          2);
      ...
      periodic_face_pairs)
{
  ...
  DoFTools::extract_locally_relevant_dofs(this->dof_handler_wdist, xwall_relevant_set);
  ...
  for(unsigned int i = 0; i < periodic_face_pairs.size(); ++i)
  {
    ...
    pair.cell[0] =
      typename DoFHandler<dim>::cell_iterator(&periodic_face_pairs[i].cell[0]->get_triangulation(),
                                              periodic_face_pairs[i].cell[0]->level(),
                                              periodic_face_pairs[i].cell[0]->index(),
                                              &this->dof_handler_wdist);
    pair.cell[1] =
      typename DoFHandler<dim>::cell_iterator(&periodic_face_pairs[i].cell[1]->get_triangulation(),
  ...
  DoFTools::make_periodicity_constraints<DoFHandler<dim>>(periodic_face_pairs_dh,
                                                          constraint_periodic);
  DoFTools::make_hanging_node_constraints(this->dof_handler_wdist, constraint_periodic);
  ...
  setup_helmholtz_preconditioner(HelmholtzOperatorData<dim> &)
{
  ...
  {
    this->helmholtz_preconditioner.reset(
      new InverseMassMatrixPreconditionerPtr<dim, fe_degree, value_type>(
        this->inverse_mass_matrix_operator));
  }
  ...
  else if(this->param.preconditioner_viscous == PreconditionerViscous::GeometricMultigrid)
  {
    AssertThrow(false,
                ExcMessage(
                  "GeometricMultigrid is currently not supported as Helmholtz preconditioner"));
  }

  // some further safety checks
  AssertThrow(this->param.solver_viscous == SolverViscous::GMRES, ExcMessage("only gmres allowed"));
  AssertThrow(this->param.IP_formulation_viscous == InteriorPenaltyFormulation::NIPG,
              ExcMessage("need non-symmetric formulation of viscous part for stability"));
}
  ...
  setup_projection_solver()
{
  ...
  projection_operator_data.solve_stokes_equations =
  ...
  if(this->param.projection_type == ProjectionType::NoPenalty)
  {
    AssertThrow(
      false,
      ExcMessage(
        "This is not implemented, use the local penalty-projection method with zero penalty parameter"));
  }
          ...
          this->param.solver_projection == SolverProjection::LU)
  {
    ...
    this->projection_operator = new ProjectionOperatorBase<dim,
                                                           fe_degree,
                                                           fe_degree_p,
                                                           fe_degree_xwall,
                                                           xwall_quad_rule,
    ...
    this->projection_solver.reset(
      new DirectProjectionSolverDivergencePenalty<dim,
                                                  fe_degree,
                                                  fe_degree_p,
                                                  fe_degree_xwall,
                                                  xwall_quad_rule,
                                                  value_type>(this->projection_operator));
  }
  else if(this->param.projection_type == ProjectionType::DivergencePenalty &&
          this->param.solver_projection == SolverProjection::PCG)
  {
    ...
    this->projection_operator = new ProjectionOperatorDivergencePenaltyXWall<dim,
                                                                             fe_degree,
                                                                             fe_degree_p,
                                                                             fe_degree_xwall,
                                                                             xwall_quad_rule,
    ...
    this->projection_solver.reset(
      new IterativeProjectionSolverDivergencePenaltyXWall<dim,
                                                          fe_degree,
                                                          fe_degree_p,
                                                          fe_degree_xwall,
                                                          xwall_quad_rule,
...
#endif /* INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGNAVIERSTOKESDUALSPLITTINGXWALL_H_ */
