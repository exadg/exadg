...
#ifndef INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGSPALARTALLMARASMODEL_H_
#define INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGSPALARTALLMARASMODEL_H_

#include "../../incompressible_navier_stokes/infrastructure/fe_parameters.h"
#include "../../incompressible_navier_stokes/xwall/DGNavierStokesDualSplittingXWall.h"
...
public:
  ...
  // situation is equal to pressure: do not want to have enrichment but need same quadrature rule as
  // with enrichment
  ...
  typedef FEFaceEvaluationWrapperPressure<dim,
                                          ...
                                          value_type,
                                          is_xwall>
    FEFaceEval_Vt_Velocity_nonlinear;
...
private:
  ...
  // compute face
  void
  local_evaluate_spalart_allmaras_face(
  ...
  void
  local_evaluate_spalart_allmaras_boundary_face(
    const MatrixFree<dim, value_type> &               data,
    parallel::distributed::Vector<value_type> &       dst,
    const parallel::distributed::Vector<value_type> & src,
    const std::pair<unsigned int, unsigned int> &     cell_range) const;
...
template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
void
DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::setup(
...
template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
void
DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
  compute_array_penalty_parameter(MatrixFree<dim, value_type> & data, const Mapping<dim> & mapping)
{
  ...
  compute_element_length(MatrixFree<dim, value_type> & data, const Mapping<dim> & mapping)
{
  ...
  for(unsigned int i = 0; i < data.n_macro_cells() + data.n_macro_ghost_cells(); ++i)
    for(unsigned int v = 0; v < data.n_components_filled(i); ++v)
    {
      ...
      for(unsigned int q = 0; q < quadrature.size(); ++q)
        volume += fe_values.JxW(q);
      element_length[i][v] = std::pow(volume, 1. / (double)dim) /
                             (double)(fe_degree + 1); // actually, this makes only sense in 3D
    }
}

template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
void
DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::evaluate(
  ...
  parallel::distributed::Vector<value_type> &       dst)
{
  ...
  data.loop(&DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
              local_evaluate_spalart_allmaras,
            &DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
              local_evaluate_spalart_allmaras_face,
            &DGSpalartAllmarasModel<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
              local_evaluate_spalart_allmaras_boundary_face,
                                  ...
                                  const std::pair<unsigned int, unsigned int> & cell_range) const
{
  ...
  for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
  {
    ...
    {
      Tensor<1, dim, VectorizedArray<value_type>> submit_gradient =
      ...
      // viscous flux
      submit_gradient += (viscosity + std::max(vt, make_vectorized_array<value_type>(0.))) / cb3 *
                         fe_eval_vt.get_gradient(q);

      // source terms
      {
        VectorizedArray<value_type> submit_value = VectorizedArray<value_type>();

        VectorizedArray<value_type> vt =
          std::max(fe_eval_vt.get_value(q), make_vectorized_array(0.));
        ...
        //          if(Sbar[v] < -0.7 *S[v])
        //            Stilde[v] = S[v] * (1. + (0.7 * 0.7 * S[v] + 0.9 * Sbar[v])/((0.9-2.*0.7)*
        //            S[v]-Sbar[v]));
        VectorizedArray<value_type> r = vt / (Stilde * kappa * kappa * wdist[q] * wdist[q]);
        VectorizedArray<value_type> g = r + cw2 * (r * r * r * r * r * r - r);
        VectorizedArray<value_type> fw =
          g * std::pow(((1. + cw3 * cw3 * cw3 * cw3 * cw3 * cw3) /
                        (g * g * g * g * g * g + cw3 * cw3 * cw3 * cw3 * cw3 * cw3)),
                       1. / 6.);

        VectorizedArray<value_type> ft2 =
          VectorizedArray<value_type>(); // ct3 * std::exp(-ct4 * chi * chi);

        //        Tensor<1,dim,VectorizedArray<value_type> > submit_gradient_add = vt * grad_vt
        //        *cb2/cb3;
        ...
        submit_value +=
          -(cb1 / (kappa * kappa) * ft2 - cw1 * fw) * (vt / wdist[q]) * (vt / wdist[q]);
    ...
    const std::pair<unsigned int, unsigned int> &     face_range) const
{
  FEFaceEval_Velocity_Velocity_nonlinear fe_eval_velocity(data,
                                                          ...
                                                          true,
                                                          dof_index_velocity);
  FEFaceEval_Velocity_Velocity_nonlinear fe_eval_velocity_neighbor(data,
                                                                   &fe_param,
                                                                   false,
                                                                   dof_index_velocity);
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    for(unsigned int q = 0; q < fe_eval_velocity.n_q_points; ++q)
    {
      ...
      // convective flux
      {
        ...
        const VectorizedArray<value_type> average_normal_flux =
          (vtM * uM_n + vtP * uP_n) * make_vectorized_array<value_type>(0.5);
      ...
      // viscous flux
      {
        VectorizedArray<value_type> facM =
          (viscosity + std::max(vtM, make_vectorized_array<value_type>(0.))) / cb3;
        VectorizedArray<value_type> facP =
          (viscosity + std::max(vtP, make_vectorized_array<value_type>(0.))) / cb3;
        ...
        average_gradient =
          average_gradient - jump_value * sigmaF * 2. * fac_harmonic_weight_penalty;
      ...
      //      submit_value += -(std::max(vtM,make_vectorized_array<value_type>(0.)) *
      //      fe_eval_vt.get_gradient(q)
      //                      + std::max(vtP,make_vectorized_array<value_type>(0.)) *
      //                      fe_eval_vt_neighbor.get_gradient(q))* 0.5 * normal *cb2/cb3;
      // submit values
      fe_eval_vt.submit_value(submit_value, q);
    ...
    const std::pair<unsigned int, unsigned int> &     face_range) const
{
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    VectorizedArray<value_type> sigmaF =
      fe_eval_vt.read_cell_data(array_penalty_parameter) * get_penalty_factor();
    ...
    if(dirichlet_boundary.find(data.get_boundary_indicator(face)) != dirichlet_boundary.end())
    {
      for(unsigned int q = 0; q < fe_eval_velocity.n_q_points; ++q)
      {
        ...
        Tensor<1, dim, VectorizedArray<value_type>> g_n =
          Tensor<1, dim, VectorizedArray<value_type>>(); // walls only
        Tensor<1, dim, VectorizedArray<value_type>> uP =
          -uM + make_vectorized_array<value_type>(2.0) * g_n;
        Tensor<1, dim, VectorizedArray<value_type>> normal = fe_eval_vt.get_normal_vector(q);

        const VectorizedArray<value_type> g_vt_n(
          make_vectorized_array<value_type>(0.)); // only walls considered
        const VectorizedArray<value_type> vtM = fe_eval_vt.get_value(q);
        const VectorizedArray<value_type> vtP =
          -vtM + make_vectorized_array<value_type>(2.0) * g_vt_n;
        const VectorizedArray<value_type> jump_value = vtM - vtP;
        ...
        // convective flux
        {
          ...
          const VectorizedArray<value_type> average_normal_flux =
            (vtM * uM_n + vtP * uP_n) * make_vectorized_array<value_type>(0.5);
          const VectorizedArray<value_type> lf_flux =
        ...
        // viscous flux
        {
          ...
          // harmonic weighting of variable viscosity, see Schott and Rasthofer et al. (2015)
          // this term is difficult to specify including all the measures for robustness and
          // fulfilling the boundary conditions so we apply a minor simplification here and just
          // state the following
          VectorizedArray<value_type> average_gradient = fe_eval_vt.get_gradient(q) * facM * normal;
    ...
    else if(neumann_boundary.find(data.get_boundary_indicator(face)) != neumann_boundary.end())
    {
      for(unsigned int q = 0; q < fe_eval_velocity.n_q_points; ++q)
      {
        ...
        {
          // harmonic weighting of variable viscosity, see Schott and Rasthofer et al. (2015)
          VectorizedArray<value_type> fac_harmonic_weight_penalty =
            make_vectorized_array<value_type>(0.);
          VectorizedArray<value_type> average_gradient =
...
template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
class DGNavierStokesDualSplittingXWallSpalartAllmaras
  : public DGNavierStokesDualSplittingXWall<dim,
                                            fe_degree,
                                            fe_degree_p,
                                            fe_degree_xwall,
                                            xwall_quad_rule>
...
public:
  typedef
    typename DGNavierStokesBase<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
      value_type value_type;
  typedef
    typename DGNavierStokesBase<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
      FEFaceEval_Velocity_Velocity_linear FEFaceEval_Velocity_Velocity_linear;
  typedef
    typename DGNavierStokesBase<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::
      FEEval_Velocity_Velocity_linear FEEval_Velocity_Velocity_linear;
  ...
  enum class DofHandlerSelector
  {
    velocity = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       ...
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::DofHandlerSelector::velocity),
    pressure = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::DofHandlerSelector::pressure),
    wdist_tauw = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::DofHandlerSelector::wdist_tauw),
    vt = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::DofHandlerSelector::n_variants),
    n_variants = static_cast<typename std::underlying_type<DofHandlerSelector>::type>(vt) + 1
  };

  // same quadrature rules as in DGNavierStokesDualSplittingXWall<dim, fe_degree, fe_degree_p,
  // fe_degree_xwall, xwall_quad_rule>
  enum class QuadratureSelector
  {
    velocity = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       ...
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::QuadratureSelector::velocity),
    pressure = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::QuadratureSelector::pressure),
    velocity_nonlinear = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::QuadratureSelector::velocity_nonlinear),
    enriched = static_cast<int>(
      DGNavierStokesDualSplittingXWall<dim,
                                       fe_degree,
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>::QuadratureSelector::enriched),
    n_variants = static_cast<typename std::underlying_type<QuadratureSelector>::type>(enriched) + 1
  };

  DGNavierStokesDualSplittingXWallSpalartAllmaras(
    ...
    : DGNavierStokesDualSplittingXWall<dim,
                                       ...
                                       fe_degree_p,
                                       fe_degree_xwall,
                                       xwall_quad_rule>(triangulation, parameter),
      fe_vt(QGaussLobatto<1>(fe_degree + 1)),
      dof_handler_vt(triangulation),
      spalart_allmaras(this->fe_param,
                       parameter.xwall_turb == XWallTurbulenceApproach::ClassicalDESSpalartAllmaras)
  {
  }
  ...
  unsigned int
  get_dof_index_vt() const
  {
    return static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
      DofHandlerSelector::vt);
  }

  FE_DGQArbitraryNodes<dim> const &
  get_fe_vt() const
  {
    return *fe_vt;
  }
  ...
  void
  prescribe_initial_condition_vt(parallel::distributed::Vector<value_type> & vt) const
  {
    vt = vt_initial_value;
  };
  ...
  void
  initialize_vector_vt(parallel::distributed::Vector<value_type> & src) const
  {
  ...
  void
  evaluate_spalart_allmaras(parallel::distributed::Vector<value_type> *       src_vel,
...
template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
void
DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                fe_degree,
                                                fe_degree_p,
                                                fe_degree_xwall,
                                                xwall_quad_rule>::
        ...
        std::shared_ptr<FieldFunctionsNavierStokes<dim>>     field_functions)
{
  ...
  spalart_allmaras.setup(this->data,
                         this->mapping,
                         this->dirichlet_boundary,
                         this->neumann_boundary);
}
                                                ...
                                                xwall_quad_rule>::create_dofs()
{
  ...
  unsigned int ndofs_per_cell_velocity =
    Utilities::fixed_int_power<fe_degree + 1, dim>::value * dim;
  unsigned int ndofs_per_cell_xwall =
    Utilities::fixed_int_power<fe_degree_xwall + 1, dim>::value * dim;
  ...
  pcout << std::endl
        << "Discontinuous finite element discretization:" << std::endl
        << std::endl
        << "Velocity:" << std::endl
        << "  degree of 1D polynomials:\t" << std::fixed << std::setw(10) << std::right << fe_degree
        << " (polynomial) and " << std::setw(10) << std::right << fe_degree_xwall
        << " (enrichment) " << std::endl
        << "  number of dofs per cell:\t" << std::fixed << std::setw(10) << std::right
        << ndofs_per_cell_velocity << " (polynomial) and " << std::setw(10) << std::right
        << ndofs_per_cell_xwall << " (enrichment) " << std::endl
        << "  number of dofs (velocity):\t" << std::fixed << std::setw(10) << std::right
        << this->dof_handler_u.n_dofs() << std::endl
        << "Pressure:" << std::endl
        << "  degree of 1D polynomials:\t" << std::fixed << std::setw(10) << std::right
        << fe_degree_p << std::endl
        << "  number of dofs per cell:\t" << std::fixed << std::setw(10) << std::right
        << ndofs_per_cell_pressure << std::endl
        << "  number of dofs (pressure):\t" << std::fixed << std::setw(10) << std::right
        << this->dof_handler_p.n_dofs() << std::endl
        << "Eddy viscosity:" << std::endl
        << "  degree of 1D polynomials:\t" << std::fixed << std::setw(10) << std::right << fe_degree
        << std::endl
        << "  number of dofs per cell:\t" << std::fixed << std::setw(10) << std::right
        << ndofs_per_cell_vt << std::endl
        << "  number of dofs (eddy visc):\t" << std::fixed << std::setw(10) << std::right
        << this->dof_handler_vt.n_dofs() << std::endl;
}
  ...
  data_reinit(typename MatrixFree<dim, value_type>::AdditionalData & additional_data)
{
  ...
  dof_handler_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::velocity)]   = &this->dof_handler_u;
  dof_handler_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::pressure)]   = &this->dof_handler_p;
  dof_handler_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::wdist_tauw)] = &this->dof_handler_wdist;
  dof_handler_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::vt)]         = &dof_handler_vt;
  ...
  constraint_matrix_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::velocity)]   = &constraint_u;
  constraint_matrix_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::pressure)]   = &constraint_p;
  constraint_matrix_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::wdist_tauw)] = &this->constraint_periodic;
  constraint_matrix_vec[static_cast<typename std::underlying_type<DofHandlerSelector>::type>(
    DofHandlerSelector::vt)]         = &constraint_p;
  ...
  quadratures[static_cast<typename std::underlying_type<QuadratureSelector>::type>(
    QuadratureSelector::velocity)] = QGauss<1>(fe_degree + 1);
  // pressure
  quadratures[static_cast<typename std::underlying_type<QuadratureSelector>::type>(
    QuadratureSelector::pressure)] = QGauss<1>(fe_degree_p + 1);
  // exact integration of nonlinear convective term
  quadratures[static_cast<typename std::underlying_type<QuadratureSelector>::type>(
    QuadratureSelector::velocity_nonlinear)] = QGauss<1>(fe_degree + (fe_degree + 2) / 2);
  // high-order integration of enrichment
  quadratures[static_cast<typename std::underlying_type<QuadratureSelector>::type>(
    QuadratureSelector::enriched)] = QGauss<1>(xwall_quad_rule);

  this->data.reinit(
    this->mapping, dof_handler_vec, constraint_matrix_vec, quadratures, additional_data);
}
...
DGNavierStokesDualSplittingXWallSpalartAllmaras<
  ...
  fe_degree_p,
  fe_degree_xwall,
  xwall_quad_rule>::set_eddy_viscosity(const parallel::distributed::Vector<value_type> & src)
{
  ...
  this->data.loop(
    &DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                     ...
                                                     fe_degree_p,
                                                     fe_degree_xwall,
                                                     xwall_quad_rule>::local_set_eddyviscosity,
    &DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                     fe_degree,
                                                     fe_degree_p,
                                                     fe_degree_xwall,
                                                     xwall_quad_rule>::local_set_eddyviscosity_face,
    &DGNavierStokesDualSplittingXWallSpalartAllmaras<
      dim,
      fe_degree,
      fe_degree_p,
      fe_degree_xwall,
      xwall_quad_rule>::local_set_eddyviscosity_boundary_face,
    this,
    dummy,
    src);
}
...
DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
  ...
  local_set_eddyviscosity(const MatrixFree<dim, value_type> & data,
                          ...
                          const std::pair<unsigned int, unsigned int> &     cell_range)
{
  FEEvaluationWrapperPressure<dim, fe_degree, fe_degree_xwall, xwall_quad_rule, 1, value_type, true>
    fe_eval_vt(data, &this->fe_param, 3);
  ...
  for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
  {
    ...
    {
      VectorizedArray<value_type> vt =
        std::max(fe_eval_vt.get_value(q), make_vectorized_array<value_type>(0.));
      ...
      for(unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements; v++)
        if(not enriched[v])
          fv1[v] = 0.;
      this->viscous_operator.set_viscous_coefficient_cell(cell,
                                                          q,
                                                          vt * fv1 + this->fe_param.viscosity);
    }
  }
}
...
DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
  ...
  local_set_eddyviscosity_face(const MatrixFree<dim, value_type> & data,
                               ...
                               const std::pair<unsigned int, unsigned int> &     face_range)
{
  FEFaceEvaluationWrapperPressure<dim,
                                  fe_degree,
                                  fe_degree_xwall,
                                  xwall_quad_rule,
                                  1,
                                  value_type,
                                  true>
    fe_eval_vt(data, &this->fe_param, true, 3);
  FEFaceEvaluationWrapperPressure<dim,
                                  fe_degree,
                                  fe_degree_xwall,
                                  xwall_quad_rule,
                                  ...
                                  value_type,
                                  true>
    fe_eval_vt_neighbor(data, &this->fe_param, false, 3);
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    for(unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements &&
                            data.faces.at(face).left_cell[v] != numbers::invalid_unsigned_int;
        ++v)
    {
    ...
    for(unsigned int q = 0; q < fe_eval_vt.n_q_points; q++)
    {
      ...
      for(unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements; v++)
        if(not left_enriched[v])
          fv1[v] = 0.;
      this->viscous_operator.set_viscous_coefficient_face(face,
                                                          q,
                                                          vt * fv1 + this->fe_param.viscosity);
    }
    ...
    {
      VectorizedArray<value_type> vt =
        std::max(fe_eval_vt_neighbor.get_value(q), make_vectorized_array<value_type>(0.));
      VectorizedArray<value_type> chi = vt / this->fe_param.viscosity;
      VectorizedArray<value_type> fv1 = chi * chi * chi / (chi * chi * chi + 7.1 * 7.1 * 7.1);
      for(unsigned int v = 0; v < VectorizedArray<value_type>::n_array_elements; v++)
        if(not right_enriched[v])
          fv1[v] = 0.;
      this->viscous_operator.set_viscous_coefficient_face_neighbor(
        face, q, vt * fv1 + this->fe_param.viscosity);
    }
  }
}
...
DGNavierStokesDualSplittingXWallSpalartAllmaras<
  ...
  fe_degree_p,
  fe_degree_xwall,
  xwall_quad_rule>::rhs_viscous(parallel::distributed::Vector<value_type> &       dst,
                                const parallel::distributed::Vector<value_type> & src,
                                const parallel::distributed::Vector<value_type> & vt)
{
...
#endif /* INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_DGSPALARTALLMARASMODEL_H_ */
