...
template<int dim,
         ...
         int xwall_quad_rule,
         typename value_type>
class TimeIntBDFDualSplittingXWallSpalartAllmaras
  : virtual public TimeIntBDFDualSplittingXWall<dim,
                                                fe_degree,
                                                fe_degree_p,
                                                fe_degree_xwall,
                                                xwall_quad_rule,
  ...
  TimeIntBDFDualSplittingXWallSpalartAllmaras(
    ...
    : TimeIntBDFDualSplitting<dim,
      ...
      ns_operation_xwall_sa(
        std::dynamic_pointer_cast<DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                                                  fe_degree,
                                                                                  fe_degree_p,
                                                                                  fe_degree_xwall,
                                                                                  xwall_quad_rule>>(
...
protected:
  void
  read_restart_data_sa(boost::archive::binary_iarchive & ia)
  {
    Vector<double> tmp;
    for(unsigned int i = 0; i < this->vt.size(); i++)
    {
      ia >> tmp;
      std::copy(tmp.begin(), tmp.end(), this->vt[i].begin());
    }

    for(unsigned int i = 0; i < this->vt_rhs.size(); i++)
    {
      ia >> tmp;
      std::copy(tmp.begin(), tmp.end(), this->vt_rhs[i].begin());
    }
  ...
  void
  write_restart_data_sa(boost::archive::binary_oarchive & oa) const
  {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    VectorView<double> tmp(this->vt[0].local_size(), this->vt[0].begin());
#pragma GCC diagnostic pop
    oa << tmp;
    for(unsigned int i = 1; i < this->vt.size(); i++)
    {
      tmp.reinit(this->vt[i].local_size(), this->vt[i].begin());
      oa << tmp;
    }

    for(unsigned int i = 0; i < this->vt_rhs.size(); i++)
    {
      tmp.reinit(this->vt_rhs[i].local_size(), this->vt_rhs[i].begin());
      oa << tmp;
    }
...
private:
  ...
  std::shared_ptr<DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                                  fe_degree,
                                                                  fe_degree_p,
                                                                  fe_degree_xwall,
                                                                  xwall_quad_rule>>
...
template<int dim,
         ...
         typename value_type>
void
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            fe_degree,
                                            fe_degree_p,
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::initialize_vectors()
{
  TimeIntBDFDualSplittingXWall<dim,
                               fe_degree,
                               fe_degree_p,
                                            ...
                                            value_type>::solve_timestep()
{
  for(unsigned int n = 0; n < num_sa_substeps; n++)
  {
    ...
    if(this->param.calculation_of_time_step_size == TimeStepCalculation::AdaptiveTimeStepCFL)
    {
      ...
      {
        this->set_adaptive_time_integrator_constants(
          this->time_step_number, time_steps_sa, alpha_sa, beta_sa, gamma0_sa);
      }
      else // otherwise, adjust time integrator constants since this is adaptive time stepping
      {
        this->set_adaptive_time_integrator_constants(
          this->order, time_steps_sa, alpha_sa, beta_sa, gamma0_sa);
      }
    }
  }
...
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            ...
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::prepare_vectors_for_next_timestep()
{
  TimeIntBDFDualSplittingXWall<dim,
                               fe_degree,
                               fe_degree_p,
...
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            ...
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::postprocessing() const
{
  this->postprocessor->do_postprocessing(this->velocity[0],
                                         this->intermediate_velocity,
                                         this->pressure[0],
                                         this->vorticity[0],
                                         vt[0],
                                         this->time,
                                            ...
                                            value_type>::viscous_step()
{
  ...
  unsigned int iterations_viscous =
    ns_operation_xwall_sa->solve_viscous(this->velocity_np, this->rhs_vec_viscous);
     ...
     this->time_step_number % this->param.output_solver_info_every_timesteps == 0)
  {
    std::cout << std::endl
              ...
              << "  PCG iterations:    " << std::setw(4) << std::right << iterations_viscous
...
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            ...
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::calculate_time_step()
{
  // there is no enrichment in the first time step
  TimeIntBDFNavierStokes<
    dim,
    fe_degree,
    value_type,
    DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                    ...
                                                    fe_degree_p,
                                                    fe_degree_xwall,
                                                    xwall_quad_rule>>::calculate_time_step();
  ...
  if(this->param.calculation_of_time_step_size == TimeStepCalculation::AdaptiveTimeStepCFL)
  {
    ...
    pcout << "Calculation of time step size according to adaptive Diffusion number condition:"
          << std::endl
          << std::endl;
...
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            ...
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::recalculate_adaptive_time_step()
{
  /*
  ...
  this->time_steps[0] =
    calculate_adaptive_time_step_cfl_xwall<dim, fe_degree, fe_degree_xwall, value_type>(
      ns_operation_xwall_sa->get_data(),
...
TimeIntBDFDualSplittingXWallSpalartAllmaras<dim,
                                            ...
                                            fe_degree_xwall,
                                            xwall_quad_rule,
                                            value_type>::update_time_integrator_constants()
{
  TimeIntBDFNavierStokes<dim,
                         fe_degree,
                         value_type,
                         DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                                         fe_degree,
