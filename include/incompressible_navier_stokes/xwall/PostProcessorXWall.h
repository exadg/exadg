...
#ifndef INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_POSTPROCESSORXWALL_H_
#define INCLUDE_INCOMPRESSIBLE_NAVIER_STOKES_XWALL_POSTPROCESSORXWALL_H_
...
public:
  Postprocessor(const unsigned int partition, const double viscosity)
    : partition(partition), viscosity(viscosity)
  {
  }
  ...
  get_data_component_interpretation() const
  {
    std::vector<DataComponentInterpretation::DataComponentInterpretation> interpretation(
      2 * dim + number_vorticity_components + 3,
    ...
    // vorticity
    if(dim == 2)
      interpretation[2 * dim + number_vorticity_components + 1] =
                                    ...
                                    std::vector<Vector<double>> & computed_quantities) const
  {
    ...
    for(unsigned int q = 0; q < n_quadrature_points; ++q)
    {
      ...
      for(unsigned int d = 0; d < dim; ++d)
        computed_quantities[q](d) = (uh[q](d) + uh[q](dim + d) * enrichment_func);
      ...
      // vorticity
      for(unsigned int d = 0; d < number_vorticity_components; ++d)
        computed_quantities[q](2 * dim + 2 + d) =
          uh[q](2 * dim + 2 + d) + uh[q](2 * dim + 2 + d + dim) * enrichment_func;
...
public:
   ...
   */
  virtual void
  do_postprocessing(parallel::distributed::Vector<double> const & velocity,
                    ...
                    int const                                          = -1)
  {
    ...
    if(time > (this->pp_data.output_data.output_start_time +
               this->output_counter * this->pp_data.output_data.output_interval_time - EPSILON))
    {
...
protected:
  std::shared_ptr<const DGNavierStokesDualSplittingXWall<dim,
                                                         fe_degree,
                                                         fe_degree_p,
                                                         fe_degree_xwall,
                                                         xwall_quad_rule>>
...
template<int dim, int fe_degree, int fe_degree_p, int fe_degree_xwall, int xwall_quad_rule>
void
PostProcessorXWall<dim, fe_degree, fe_degree_p, fe_degree_xwall, xwall_quad_rule>::write_output(
...
{
  // velocity + xwall dofs
  const FESystem<dim> joint_fe(this->dof_handler_velocity->get_fe(),
                               1, // velocity
                               ns_operation_xw_->get_fe_wdist(),
                               1, // wdist
                               ns_operation_xw_->get_fe_wdist(),
                               1, // tauw
                               this->dof_handler_velocity->get_fe(),
                               1 // vorticity
  );
  ...
  parallel::distributed::Vector<double> joint_solution(joint_dof_handler.locally_owned_dofs(),
                                                       joint_relevant_set,
                                                       MPI_COMM_WORLD);
  ...
  typename DoFHandler<dim>::active_cell_iterator
    ...
    joint_endc = joint_dof_handler.end(), vel_cell = this->dof_handler_velocity->begin_active(),
    wdist_cell = ns_operation_xw_->get_dof_handler_wdist().begin_active();

  for(; joint_cell != joint_endc; ++joint_cell, ++vel_cell, ++wdist_cell)
  ...
  Postprocessor<dim> postprocessor(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD),
                                   pp_data.turb_stat_data.viscosity);
  ...
  {
    std::shared_ptr<const DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                                          fe_degree,
                                                                          fe_degree_p,
                                                                          fe_degree_xwall,
                                                                          xwall_quad_rule>>
      nsa;
    nsa = std::dynamic_pointer_cast<
      const DGNavierStokesDualSplittingXWallSpalartAllmaras<dim,
                                                            fe_degree,
                                                            fe_degree_p,
                                                            fe_degree_xwall,
                                                            xwall_quad_rule>>(ns_operation_xw_);
    if(nsa != nullptr)
    {
  ...
  if(this->pp_data.output_data.compute_divergence == true)
    AssertThrow(false,
                ExcMessage("Computing the divergence for xwall elements currently not supported"));

  std::ostringstream filename;
  filename << "output/" << this->pp_data.output_data.output_prefix << "_Proc"
           << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) << "_" << this->output_counter
           << ".vtu";

  data_out.build_patches(*(this->mapping),
                         this->pp_data.output_data.number_of_patches,
  ...
  if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
  {
    ...
    {
      std::ostringstream filename;
      filename << this->pp_data.output_data.output_prefix << "_Proc" << i << "_"
               << this->output_counter << ".vtu";

      filenames.push_back(filename.str().c_str());
    }
    std::string master_name = "output/" + this->pp_data.output_data.output_prefix + "_" +
                              Utilities::int_to_string(this->output_counter) + ".pvtu";
