...
public:
  ...
  void
  calculate_array_penalty_parameter(parallel::distributed::Vector<value_type> const & velocity_n,
                                    double const                                      cfl,
                                    double const                                      time_step)
  {
    ...
    for(unsigned int cell = 0; cell < data.n_macro_cells() + data.n_macro_ghost_cells(); ++cell)
    {
      ...
      {
        //        array_penalty_parameter_divergence[cell] =
        //        projection_operator_data.penalty_parameter_divergence * norm_U_mean *
        //        std::exp(std::log(volume)/(double)dim) * time_step;
        //        array_penalty_parameter_continuity[cell] =
        //        projection_operator_data.penalty_parameter_continuity * norm_U_mean * time_step;

        array_penalty_parameter_divergence[cell] =
          projection_operator_data.penalty_parameter_divergence * norm_U_mean *
          std::exp(std::log(volume) / (double)dim);
        array_penalty_parameter_continuity[cell] =
          projection_operator_data.penalty_parameter_continuity * norm_U_mean;
      }
      else
      {
        array_penalty_parameter_divergence[cell] =
          projection_operator_data.penalty_parameter_divergence / cfl * norm_U_mean *
          std::exp(std::log(volume) / (double)dim) * time_step;
        array_penalty_parameter_continuity[cell] =
          projection_operator_data.penalty_parameter_continuity / cfl * norm_U_mean * time_step;
      }
    }
  }
  ...
  get_array_penalty_parameter_divergence() const
  {
    return array_penalty_parameter_divergence;
  }
  AlignedVector<VectorizedArray<value_type>> const &
  get_array_penalty_parameter_continuity() const
  {
...
template<int dim,
         ...
         int xwall_quad_rule,
         typename value_type>
class ProjectionOperatorDivergencePenaltyXWall : public ProjectionOperatorBaseOld<dim,
                                                                                  fe_degree,
                                                                                  fe_degree_p,
                                                                                  fe_degree_xwall,
                                                                                  xwall_quad_rule,
...
public:
  ...
  ProjectionOperatorDivergencePenaltyXWall(
    ...
    : ProjectionOperatorBaseOld<dim,
                                ...
                                value_type>(data_in,
                                            dof_index_in,
                                            quad_index_in,
                                            projection_operator_data_in),
      fe_eval(1,
              FEEval_Velocity_Velocity_linear(data_in, fe_param_in, dof_index_in, quad_index_in)),
  ...
  void
  precondition(VectorizedArray<double> * dst, const VectorizedArray<double> * src) const
  {
    inverse_mass_matrix_operator_xwall->local_apply_inverse_mass_matrix(curr_cell, dst, src);
  }

  void
  vmult(VectorizedArray<double> * dst, VectorizedArray<double> * src) const
  {
...
template<int dim,
         ...
         int xwall_quad_rule,
         typename value_type>
class IterativeProjectionSolverDivergencePenaltyXWall
  : public IterativeProjectionSolverDivergencePenalty<dim,
                                                      fe_degree,
                                                      fe_degree_p,
                                                      fe_degree_xwall,
                                                      xwall_quad_rule,
...
public:
  ...
  typedef FEEvaluationWrapper<dim,
                              ...
                              is_xwall>
    FEEval_Velocity_Velocity_linear;
  IterativeProjectionSolverDivergencePenaltyXWall(
    ProjectionOperatorBaseOld<dim,
                              fe_degree,
                              fe_degree_p,
                              fe_degree_xwall,
                              xwall_quad_rule,
                              value_type> * projection_operator_in,
    ProjectionSolverData const              solver_data_in)
    : IterativeProjectionSolverDivergencePenalty<dim,
                                                 fe_degree,
                                                 fe_degree_p,
                                                 fe_degree_xwall,
                                                 xwall_quad_rule,
                                                 value_type>(projection_operator_in, solver_data_in)
  {
  }
  ...
  {
    AssertThrow(
      this->solver_data.preconditioner_projection == PreconditionerProjection::InverseMassMatrix,
      ExcMessage(
        "Specified preconditioner is not available for projection_type = ProjectionType::DivergencePenalty and solver_projection = SolverProjection::PCG"));

    FEEval_Velocity_Velocity_linear fe_eval(data,
                                            this->projection_operator->get_fe_param(),
                                            this->projection_operator->get_dof_index(),
                                            this->projection_operator->get_quad_index());

    ProjectionOperatorDivergencePenaltyXWall<dim,
                                             fe_degree,
                                             fe_degree_p,
                                             fe_degree_xwall,
                                             xwall_quad_rule,
                                             value_type> * projection_operator_div =
      static_cast<ProjectionOperatorDivergencePenaltyXWall<dim,
                                                           fe_degree,
                                                           fe_degree_p,
                                                           fe_degree_xwall,
                                                           xwall_quad_rule,
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
    {
      ...
      InternalSolvers::SolverCG<VectorizedArray<double>> cg_solver(
        total_dofs_per_cell,
