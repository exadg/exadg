...
public:
          ...
          for(unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements; ++v)
          {
            if(enriched_components[v])
              AssertThrow(fe_eval_tauw[quad_type]->get_value(q)[v] > 1.0e-9,
                          ExcMessage("Wall shear stress has to be above zero for Spalding's law"));
          }
  ...
  integrate(const bool integrate_val, const bool integrate_grad)
  {
    if(enriched)
    {
      ...
      // this function is quite nasty because deal.ii doesn't seem to be made for enrichments
      // the scalar product of the second part of the gradient is computed directly and added to the
      // value
      if(integrate_grad)
      {
...
private:
  ...
  void add_array_component_to_value(Tensor<1, n_components_, VectorizedArray<Number>> &       val,
                                    const Tensor<1, n_components_, VectorizedArray<Number>> & toadd,
                                    unsigned int                                              v)
  {
...
public:
  ...
  reinit(const unsigned int f)
  {
    ...
    if(is_left_face)
    {
      ...
      // there may be interface elements that need a high-order quadrature despite that they are not
...
public:
  ...
  reinit(const unsigned int f)
  {
    ...
    if(this->is_left_face)
    {
      // decide if we have an enriched element via the y component of the cell center
      for(unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements &&
                              this->data.faces.at(f).left_cell[v] != numbers::invalid_unsigned_int;
          ++v)
      {
      ...
      // enriched in that case, the other element is enriched
      for(unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements &&
                              this->data.faces.at(f).right_cell[v] != numbers::invalid_unsigned_int;
          ++v)
      {
        typename DoFHandler<dim>::cell_iterator dcell = this->data.get_cell_iterator(
