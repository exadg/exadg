...
//
// template<int dim>
//// void StatisticsManagerPH<dim>::setup(const std::function<Point<dim>(const Point<dim> &)>
//// &grid_transform)
// void
// StatisticsManagerPH<dim>::setup(const Function<dim> & push_forward_function,
...
//
//  // note: this code only works with discretizations that approximate the geometry adequate. That
//  // means you need a refinement level of minimum  3 and fe_degree of minimum  2 to ensure the
//  // statistics manager to work robust
//
//
//  // ---------------------------------------------------
...
//          y_h_glob[i].push_back(
//            (yupper - ylower + y_h_glob[i].back()) *
//            0.5); // take the average between the current and the previous element, results in
//                  // better consistent behavior at element interfaces
//        else
...
//      y_h_glob[i_x][j] = std::pow(y_h_glob[i_x][j] * (4.5 * 0.028 / (double)n_cells_y_dir) *
//                                    (0.5 * 9. * 0.028 / (double)n_cells_y_dir),
//                                  1. / 3.) /
//                         (double)(fe_degree + 1);
//  }
//
...
//        std::vector<double>::iterator min_x =
//          std::min_element(vertex_x_pos.begin(), vertex_x_pos.end());
//        std::vector<double>::iterator max_x =
//          std::max_element(vertex_x_pos.begin(), vertex_x_pos.end());
//        std::vector<double>::iterator min_z =
//          std::min_element(vertex_z_pos.begin(), vertex_z_pos.end());
//        std::vector<double>::iterator max_z =
//          std::max_element(vertex_z_pos.begin(), vertex_z_pos.end());
//
...
//              points[j][0] = xi1_pos;
//              points[j][1] =
//                (double)i / (n_points_y - 1); // this are the "real" y-values in parameter space
//              if(dim == 3)
//              {
...
//
//            const double y =
//              fe_values[i]->quadrature_point(0)[1]; // this are the "real" y-values in real space
//#ifdef DEBUG_Y
...
//            unsigned int idx =
//              std::distance(y_vec_glob[i_x].begin(),
//                            std::lower_bound(y_vec_glob[i_x].begin(), y_vec_glob[i_x].end(), y));
//
//            // ..., then, check whether the point before was closer (but off
//            // by 1e-13 or less)
//            if(idx > 0 &&
//               std::abs(y_vec_glob[i_x][idx - 1] - y) < std::abs(y_vec_glob[i_x][idx] - y))
//              idx--;
//
...
//#  ifdef DEBUG_Y
//                std::cout
//                  << "something went wrong when sorting y in vector y_loc. The algorythm sees that
//                  index idx = "
//                  << idx
//                  << " is appropriate according to its distance to y, but this index has already
//                  been used !!! -> the code sort this value in next higher position of y_loc (idx
//                  = idx+1) "
//                  << std::endl;
//#  endif
...
//      y_loc,
//      communicator,
//      y_vec_glob[i_x]); // after this step (*), all entries in y_vec_glob have been changed
//
...
//    {
//      AssertThrow(
//        y_vec_glob[i_x].at(i) > -0.9,
//        ExcMessage(
//          "The geometry approximation is too bad for the statistics_manager_PH to work. Use
//          minimum l3_p2 for a GRID_STRETCH_FAC up to 2.5 (using the double amount of cells in
//          x-direction). " "If GRID_STRETCH_FAC is higher a more accurate spatial discretization is
//          necessary to make the statistics manager work")); // after step (*) ... . Thus, a "-1"
//          idicates that something wents wrong
//      //        AssertThrow(std::abs(y_vec_glob[i_x].at(i) - tmp.at(i)) <= delta_y.at(i_x),
//      //            ExcMessage("WARNING (If you think its OK comment this assert out!): The
//      geometry
//      //            approximation is too bad for the statistics_manager_PH to work. Use minimum
//      //            l3_p2 or l2_p4."
//      //                       "std::abs(y_vec_glob[i_x].at(i) - tmp.at(i)) = " +
//      //                       patch::to_string(std::abs(y_vec_glob[i_x].at(i) - tmp.at(i))) + "
//      and
//      //                       delta_y.at(i) = " + patch::to_string(delta_y.at(i))));
//    }
//
...
//  unsigned int n_cells_x_dir =
//    2 * std::pow(2, dof_handler.get_triangulation().n_global_levels() - 1);
//#else
...
//  {
//    double x = x_max / ((double)n_points_x_glob - 1.0) *
//               (double)i_x; // linear distributed before AND after transform with push forward
//                            // (opposed to y-values in setup code before)
//    x_glob.push_back(x);
//
...
//              ExcMessage("x_glob.size() = " + patch::to_string(x_glob.size()) +
//                         " and n_points_x_glob = " + patch::to_string(n_points_x_glob)));
//#ifdef DEBUG_TAU_W
//  AssertThrow(y_glob_debug.size() == n_points_x_glob,
//              ExcMessage("y_glob_debug.size() = " + patch::to_string(y_glob_debug.size()) +
//                         " and n_points_x_glob = " + patch::to_string(n_points_x_glob)));
//#endif
//
//
...
//          unsigned int idx =
//            std::distance(x_glob.begin(), std::lower_bound(x_glob.begin(), x_glob.end(), x));
//          // ..., then, check whether the point before was closer (but off
//          // by 1e-13 or less)
//          if(idx > 0 && std::abs(x_glob[idx - 1] - x) < std::abs(x_glob[idx] - x))
//            idx--;
//          AssertThrow(std::abs(x_glob[idx] - x) < 1e-13,
//                      ExcMessage("Could not locate " + patch::to_string(x) +
//                                 " among "
...
//    y_loc,
//    communicator,
//    y_hill_contour_actual); // after this step (*), all entries in y_vec_glob have been changed
//
//  for(unsigned int i = 0; i < n_points_y_glob; ++i)
//  {
...
//    f << "refinement level:   " << dof_handler.get_triangulation().n_global_levels() - 1
//      << std::endl;
//    f << "fe degree:   " << fe_degree << std::endl;
//    //    f<<"friction Reynolds number:   " <<
//    //    sqrt(viscosity*(velx_glob.at(1)/numchsamp/(x_glob.at(1)+1.)))/viscosity << std::endl;
//
...
//      f << std::scientific << std::setprecision(7) << std::setw(15)
//        << y_hill_contour_actual.at(idx);
//      f << std::scientific << std::setprecision(7) << std::setw(15)
//        << y_hill_contour_nominal.at(idx);
//      f << std::scientific << std::setprecision(7) << std::setw(15)
//        << std::abs(y_hill_contour_actual.at(idx) - y_hill_contour_nominal.at(idx));
//      f << std::endl;
//    }
//    f.close();
//  }
...
//    filename.str("");
//    filename.clear();
//    filename << output_prefix << "_slice" << statistics_number;
...
//    {
//      std::ofstream f;
//      f.open((new_output_prefix + "_" + patch::to_string(i_x) + ".flow_statistics").c_str(),
//             std::ios::trunc);
//      f << "statistics of periodic hill flow for x/h = " << x_over_h[i_x] << std::endl;
//      f << "number of samples:   " << numchsamp << std::endl;
//      f << "viscosity:           " << std::scientific << std::setprecision(9) << viscosity
//        << std::endl;
//
//      f << "       y/H       |      u/U_b     |      v/U_b     |      w/U_b     |   u'u'/U_b^2   |   v'v'/U_b^2   |   w'w'/U_b^2   |   u'v'/U_b^2   |   TKE/U_b^2    |      eps       |     eta/H      |     h/eta      |       h      "
//        << std::endl;
...
//
//        double veluu = velsq_glob[0][i_x].at(idx) / ((double)(numchsamp)) - velx * velx;
//        double velvv = velsq_glob[1][i_x].at(idx) / ((double)(numchsamp)) - vely * vely;
//        double velww = velsq_glob[2][i_x].at(idx) / ((double)(numchsamp)) - velz * velz;
//        double veluv = veluv_glob[i_x].at(idx) / ((double)(numchsamp)) - velx * vely;
...
//        std::cout << "velxssq using abs: "
//                  << abs(velsq_glob[0][i_x].at(idx) / ((double)(numchsamp)) -
//                         vel_glob[0][i_x].at(idx) * vel_glob[0][i_x].at(idx) /
//                           (((double)numchsamp) * ((double)numchsamp)))
//                  << std::endl;
//        std::cout << "sqrt(velyssq) = " << sqrt(velyssq) << std::endl;
//        std::cout << "std::sqrt(velyssq) = " << std::sqrt(velyssq) << std::endl;
//#endif
//
//        f << std::scientific << std::setprecision(9) << std::setw(17)
//          << y_vec_glob[i_x].at(idx) / H;
...
//      f << std::scientific << std::setprecision(9) << std::setw(17)
//        << std::sqrt(std::abs(tau_w)) * y1_bottom_glob.at(idx) / viscosity;
...
//    for(unsigned int idx = 0; idx < x_glob.size(); idx++)
//    {
//      double tau_w = -viscosity * dudy_top_glob.at(idx) /
//                     (double)numchsamp; // minus is set because on the top of the domain, the
//                     domain
//                                        // the gradient is negative for unseparated u-profiles
...
//      f << std::scientific << std::setprecision(9) << std::setw(17) << x_glob.at(idx) / H;
//      f << std::scientific << std::setprecision(9) << std::setw(17)
//        << std::sqrt(std::abs(tau_w)) * y1_top_glob.at(idx) / viscosity;
//      f << std::scientific << std::setprecision(9) << std::setw(17) << cf;
//      f << std::scientific << std::setprecision(9) << std::setw(17) << cp;
//      f << std::endl;
//    }
...
//            {
//              // Assert
//              const Point<dim> point_assert =
//                fe_values[i]->quadrature_point(q); // all quadrature points have the same x-value
//              (void)point_assert;
//              Assert(std::abs(point_assert[0] - x_pos) < 1e-13,
//                     ExcMessage("Check calculation of xi1-value for quadrature. "
//                                "x_pos = " +
//                                patch::to_string(x_pos) +
//                                " and point_assert[0] = " + patch::to_string(point_assert[0])));
//
...
//                    gradu[idim] += fe_values[i]->shape_grad(j, q)[idim] *
//                    velocity_vector[j][idim];
...
//              length += length_ele; // length is the length of one element. length_ele is one
//                                    // summand of the quadrature
//              for(unsigned int j = 0; j < dim; j++)
...
//
//            // check quadrature
//            double n_cells_z_dir =
//              (n_points_y_glob - 1) / (n_points_y - 1); // ASSUMED: n_cells_y_dir = n_cells_z_dir
//            (void)n_cells_z_dir;
//            if(dim == 3)
//              Assert(std::abs(length - 4.5 * h / n_cells_z_dir) < 1e-13,
//                     ExcMessage("check quadrature code. length = " + patch::to_string(length) +
//                                " lz_ele = " + patch::to_string(4.5 * h / n_cells_z_dir) +
...
//                   ExcMessage("idx is out of range. The current x/h-position is " +
//                              patch::to_string(x_over_h[i_x]) + ". idx = " + patch::to_string(idx)
//                              + " n_points_y_glob = " + patch::to_string(n_points_y_glob)));
//
//            Assert(
//              std::abs(y_vec_glob[i_x][idx] - y) < 1e-13, // 1e-13,
//              ExcMessage(
//                "Could not locate " + patch::to_string(y) +
//                " among "
//                "pre-evaluated points. Closest point is " +
...
//                " Check your discretization. It should have a refinement level of minimum 3 and a
//                fe-degree of minimum 2."));
//
//
//            for(unsigned int j = 0; j < dim; j++)
//              vel_loc[j][idx] += vel[j];
...
//    for(unsigned int idx = 0; idx < y_vec_glob[i_x].size(); idx++)
//      if(dim == 3)
//        Assert(std::abs(length_loc.at(idx) - 4.5 * h) < 1e-13 ||
//                 std::abs(length_loc.at(idx) - 2 * 4.5 * h) < 1e-13,
//               ExcMessage("check quadrature code. length_loc.at(" + patch::to_string(idx) +
//                          ") = " + patch::to_string(length_loc.at(idx)) +
//                          " 4.5*h = " + patch::to_string(4.5 * h)));
//
//
//
//    for(unsigned int idx = 0; idx < y_vec_glob[i_x].size(); idx++)
...
//                                         update_values | update_jacobians |
//                                           update_quadrature_points | update_gradients));
//    fe_face_values[i].reset(
//      new FEFaceValues<dim>(mapping_,
//                            dof_handler.get_fe().base_element(0),
//                            Quadrature<dim - 1>(points_face, weights),
//                            update_jacobians | update_quadrature_points | update_normal_vectors));
//  }
//
//  const unsigned int  scalar_dofs_per_cell = dof_handler.get_fe().base_element(0).dofs_per_cell;
//  std::vector<double> vel_values(fe_values[0]->n_quadrature_points);
...
//  for(unsigned int idx = 0; idx < x_glob.size(); idx++)
//    if(dim == 3)
//      Assert(std::abs(length_top_loc.at(idx) - 4.5 * h) < 1e-13 ||
//               std::abs(length_top_loc.at(idx) - 2 * 4.5 * h) < 1e-13,
//             ExcMessage("check quadrature code. length_loc.at(" + patch::to_string(idx) +
//                        ") = " + patch::to_string(length_top_loc.at(idx)) +
//                        " 4.5*h = " + patch::to_string(4.5 * h)));
//
//  for(unsigned int idx = 0; idx < x_glob.size(); idx++)
//  {
...
//                          Quadrature<dim>(points, weights),
//                          update_values | update_jacobians | update_quadrature_points));
//      fe_values_xwall[i].reset(
//        new FEValues<dim>(dof_handler.get_fe().base_element(1),
//                          Quadrature<dim>(points, weights),
//                          update_values | update_jacobians | update_quadrature_points));
//      fe_values_tauw[i].reset(
//        new FEValues<dim>(dof_handler_wdist.get_fe().base_element(0),
//                          Quadrature<dim>(points, weights),
//                          update_values | update_jacobians | update_quadrature_points));
//    }
//
//    const unsigned int scalar_dofs_per_cell = dof_handler.get_fe().base_element(0).dofs_per_cell;
//    const unsigned int scalar_dofs_per_cell_xwall =
//      dof_handler.get_fe().base_element(1).dofs_per_cell;
//    const unsigned int scalar_dofs_per_cell_tauw =
//      dof_handler_wdist.get_fe().base_element(0).dofs_per_cell;
//    //    std::vector<double> vel_values(fe_values[0]->n_quadrature_points);
//    std::vector<Tensor<1, dim>> velocity_vector(scalar_dofs_per_cell); // \vec{u}^e_h
...
//    dof_indices_tauw(dof_handler_wdist.get_fe().dofs_per_cell); typename
//    DoFHandler<dim>::active_cell_iterator cell_tauw = dof_handler_wdist.begin_active();
//    for(typename DoFHandler<dim>::active_cell_iterator cell = dof_handler.begin_active();
//        cell != dof_handler.end();
//        ++cell, ++cell_tauw)
//      if(cell->is_locally_owned())
...
//                velocity_vector[comp.second][comp.first] = (*velocity[0])(dof_indices[j]);
//              else
//                velocity_vector_xwall[comp.second][comp.first - dim] =
...
//                          ExcMessage("Check calculation of xi1-value for quadrature. "
//                                     "x_pos = " +
//                                     patch::to_string(x_pos) + " and point_assert[0] = " +
//                                     patch::to_string(point_assert[0])));
//
//
//              // interpolate velocity to the quadrature point
//              Tensor<1, dim> velocity;
...
//                velocity += fe_values_xwall[i]->shape_value(j, q) * velocity_vector_xwall[j] *
//                            spalding.enrichment(q);
//
//              double reduced_jacobian;
//              if(dim == 3)
//                reduced_jacobian = fe_values[i]->jacobian(q)[2][2];
...
//                        ExcMessage("idx is out of range. The current x/h-position is " +
//                                   patch::to_string(x_over_h[i_x]) +
//                                   ". idx = " + patch::to_string(idx) +
//                                   " n_points_y_glob = " + patch::to_string(n_points_y_glob)));
//
//            AssertThrow(
//              std::abs(y_vec_glob[i_x][idx] - y) < 1e-13, // 1e-13,
...
//    for(unsigned int idx = 0; idx < y_vec_glob[i_x].size(); idx++)
//      if(dim == 3)
//        AssertThrow(std::abs(length_loc.at(idx) - 4.5 * h) < 1e-13 ||
//                      std::abs(length_loc.at(idx) - 2 * 4.5 * h) < 1e-13,
//                    ExcMessage("check quadrature code. length_loc.at(" + patch::to_string(idx) +
//                               ") = " + patch::to_string(length_loc.at(idx)) +
//                               " 4.5*h = " + patch::to_string(4.5 * h)));
...
//          enrcomp[0] = true;
//          spalding.reinit(
//            wdist, tauw, gradwdist, gradtauw, fe_values[i]->n_quadrature_points, enrcomp);
//
//          for(unsigned int q = 0; q < fe_values[i]->n_quadrature_points; ++q)
//          {
//            // calculate dudy at the quadrature point
...
//
//            for(unsigned int j = 0; j < velocity_vector_xwall.size(); ++j)
//              du += fe_values_xwall[i]->shape_grad(j, q) * velocity_vector_xwall[j][0] *
//                      spalding.enrichment(q) +
//                    fe_values_xwall[i]->shape_value(j, q) * velocity_vector_xwall[j][0] *
//                      spalding.enrichment_gradient(q);
//            for(unsigned int j = 0; j < velocity_vector_xwall.size(); ++j)
//              dv += fe_values_xwall[i]->shape_grad(j, q) * velocity_vector_xwall[j][1] *
//                      spalding.enrichment(q) +
//                    fe_values_xwall[i]->shape_value(j, q) * velocity_vector_xwall[j][1] *
//                      spalding.enrichment_gradient(q);
//
//            Tensor<1, dim, double> n =
//              gradwdist[q] /
//              gradwdist[q]
//                .norm(); // normalized normal vector in r-s coordinates (element cordinates)
...
//          enrcomp[0] = true;
//          spalding.reinit(
//            wdist, tauw, gradwdist, gradtauw, fe_values_top[i]->n_quadrature_points, enrcomp);
//
//          for(unsigned int q = 0; q < fe_values_top[i]->n_quadrature_points; ++q)
//          {
//            // calculate dudy at the quadrature point
...
//            for(unsigned int j = 0; j < velocity_vector_xwall.size(); ++j)
//              dudy += fe_values_xwall_top[i]->shape_grad(j, q)[1] * velocity_vector_xwall[j][0] *
//                        spalding.enrichment(q) +
//                      fe_values_xwall_top[i]->shape_value(j, q) * velocity_vector_xwall[j][0] *
//                        spalding.enrichment_gradient(q)[1];
//
//            for(unsigned int j = 0; j < velocity_vector.size(); ++j)
...
//  for(unsigned int idx = 0; idx < x_glob.size(); idx++)
//    if(dim == 3)
//      AssertThrow(std::abs(length_top_loc.at(idx) - 4.5 * h) < 1e-13 ||
//                    std::abs(length_top_loc.at(idx) - 2 * 4.5 * h) < 1e-13,
//                  ExcMessage("check quadrature code. length_loc.at(" + patch::to_string(idx) +
//                             ") = " + patch::to_string(length_top_loc.at(idx)) +
//                             " 4.5*h = " + patch::to_string(4.5 * h)));
