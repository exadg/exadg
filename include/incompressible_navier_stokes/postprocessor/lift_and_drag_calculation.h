...
void calculate_lift_and_drag_force(MatrixFree<dim,Number> const                &matrix_free_data,
                                   ...
                                   Tensor<1,dim,Number>                        &Force)
{
  ...
  FEFaceEvaluation<dim,fe_degree_p,fe_degree_u+1,1,Number> fe_eval_pressure
      ...
      face++)
  {
    ...
    if (it != boundary_IDs.end())
    {
      ...
      {
        VectorizedArray<Number> pressure = fe_eval_pressure.get_value(q);
        Tensor<1,dim,VectorizedArray<Number> > normal = fe_eval_velocity.get_normal_vector(q);
        Tensor<2,dim,VectorizedArray<Number> > velocity_gradient = fe_eval_velocity.get_gradient(q);
        fe_eval_velocity.submit_value(pressure*normal -  make_vectorized_array<Number>(viscosity)*
                                        (velocity_gradient+transpose(velocity_gradient))*normal,q);
      }
      Tensor<1,dim,VectorizedArray<Number> > Force_local = fe_eval_velocity.integrate_value();
  ...
  void evaluate(parallel::distributed::Vector<Number> const &velocity,
                ...
                Number const                                &time) const
   {
     if(lift_and_drag_data.calculate_lift_and_drag == true)
     {
       ...
       calculate_lift_and_drag_force<dim,fe_degree_u,fe_degree_p,Number>(
           *matrix_free_data,
