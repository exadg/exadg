  ...
  const unsigned int coarse_level)
{
  switch(mg_data.coarse_solver)
  {
    case MultigridCoarseGridSolver::Chebyshev:
    {
      ...
      mg_coarse.reset(
        new MGCoarseInverseOperator<parallel::distributed::Vector<value_type_operator>, SMOOTHER>(
          mg_smoother[0]));
      break;
    }
    case MultigridCoarseGridSolver::ChebyshevNonsymmetricOperator:
    ...
    case MultigridCoarseGridSolver::AMG_ML:
    {
      ...
      mg_coarse.reset(new MGCoarseML<Operator>(matrix, matrix_q, true, coarse_level));
      return;
    }
    default:
    {
