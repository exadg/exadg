...
void
MGTransferMF_MGLevelObject<VectorType>::reinit(
  ...
  MGLevelObject<std::shared_ptr<MGConstrainedDoFs>> &     mg_constrained_dofs)
{
  ...
  for(unsigned int i = 1; i < global_levels.size(); i++)
  {
    ...
    std::shared_ptr<MGTransferBase<VectorType>> temp;
...
template<typename VectorType>
void
MGTransferMF_MGLevelObject<VectorType>::prolongate(const unsigned int level,
                                                   VectorType &       dst,
                                                   const VectorType & src) const
{
