...
namespace InternalSolvers
{
  ...
  template<typename Number>
  bool all_true(const VectorizedArray<Number> a)
  {
    for (unsigned int i=0; i<VectorizedArray<Number>::n_array_elements; ++i)
      if (a[i] < 0)
        return false;
  ...
  template <typename Number, typename Number2>
  bool converged(Number &is_converged,
                 Number norm_r_abs,
                 Number2 ABS_TOL,
                 Number norm_r_rel,
                 Number2 REL_TOL,
  ...
  template <typename Number, typename Number2>
  bool converged(VectorizedArray<Number> &is_converged,
  ...
  template<typename value_type>
  value_type l2_norm(value_type const *vector, unsigned int const size)
  {
    return std::sqrt(inner_product(vector, vector,size));
  }

  template<typename value_type>
  void vector_init(value_type *vector, unsigned int const size)
  {
    for(unsigned int i=0;i<size;++i)
      vector[i] = 0.0;
  }
...
//    std::cout<<"Number of iterations = "<< n_iter << std::endl;
  ...
  public:
    ...
    virtual void vmult(value_type *dst, value_type const *src) const = 0;
  ...
  public:
    ...
    virtual void vmult(value_type *dst, value_type const *src) const
    {
      value_type one;
      one = 1.0;
      equ(dst,one,src,M);
  ...
  public:
    ...
    template<typename Matrix, typename Preconditioner>
    void solve(Matrix const         *A,
               value_type           *x,
               value_type const     *b,
               Preconditioner const *P=nullptr);
  ...
  private:
    ...
    template<typename Matrix, typename Preconditioner>
    void do_solve(Matrix const         *A,
                  value_type           *x,
                  value_type const     *b,
                  Preconditioner const *P);

    void modified_gram_schmidt(AlignedVector<value_type>                       &w,
                               AlignedVector<AlignedVector<value_type> >       &H,
    ...
    template<typename Number>
    void print(VectorizedArray<Number> y, std::string name);
  };
  ...
  template<typename value_type>
  void SolverGMRES<value_type>::clear()
  {
    // clear all data
    V.clear();
    H.clear();
  ...
  template<typename value_type>
  template<typename Number>
  void SolverGMRES<value_type>::print(VectorizedArray<Number> y, std::string name)
  {
   ...
   */
  template<typename value_type>
  void SolverGMRES<value_type>::modified_gram_schmidt(AlignedVector<value_type>                       &w,
                                                      AlignedVector<AlignedVector<value_type> >       &H,
                                                      AlignedVector<AlignedVector<value_type> > const &V,
                                                      unsigned int const                              dim)
  {
  ...
  template<typename value_type>
  template<typename Number>
  void SolverGMRES<value_type>::perform_givens_rotation_and_calculate_residual(VectorizedArray<Number>)
  {
                                      ...
                                      Preconditioner const *P)
  {
    ...
    }while(!converged(convergence_status,
                      norm_r_abs,ABS_TOL,
                      norm_r_rel,REL_TOL,
                      iterations,MAX_ITER));
...
//    print(l2_norm(temp.begin()),"l2-norm of residual");
  ...
  template<typename value_type>
  template<typename Matrix, typename Preconditioner>
  void SolverGMRES<value_type>::do_solve(Matrix const         *A,
                                         value_type           *x,
                                         value_type const     *b,
                                         Preconditioner const *P)
  {
