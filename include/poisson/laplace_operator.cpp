...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::clear()
{
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::reinit(const MatrixFree<dim, Number> &  mf_data,
                                             ...
                                             const unsigned int        level)
{
  ...
  if(is_feq && level == numbers::invalid_unsigned_int)
  {
    ...
    for(typename std::map<types::boundary_id, std::shared_ptr<Function<dim>>>::const_iterator it =
        ...
        it != operator_data.bc->dirichlet.end();
        ++it)
    ...
    for(typename std::vector<
          GridTools::PeriodicFacePair<typename Triangulation<dim>::cell_iterator>>::const_iterator it =
          operator_data.periodic_face_pairs_level0.begin();
        it != operator_data.periodic_face_pairs_level0.end();
        ++it)
    {
      GridTools::PeriodicFacePair<typename DoFHandler<dim>::cell_iterator> periodic;
      for(unsigned int i = 0; i < 2; ++i)
      {
        periodic.cell[i]     = typename DoFHandler<dim>::cell_iterator(&dof_handler.get_triangulation(),
                                                                   it->cell[i]->level(),
                                                                   it->cell[i]->index(),
                                                                   &dof_handler);
  ...
  else if(is_feq)
  {
        ...
        ++it)
    {
      ...
      add_periodicity_constraints<dim>(
        level, level, cell1->face(it->face_idx[1]), cell0->face(it->face_idx[0]), constraints);
    }
  ...
  if(verify_boundary_conditions(dof_handler, operator_data) && is_feq &&
     Utilities::MPI::sum(edge_constrained_indices.size(), MPI_COMM_WORLD) == 0 &&
...
template<int dim, int degree, typename Number>
bool
LaplaceOperator<dim, degree, Number>::verify_boundary_conditions(
  ...
  for(typename Triangulation<dim>::cell_iterator cell = tria.begin(); cell != tria.end(); ++cell)
    for(unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
      if(cell->at_boundary(f))
      {
        ...
        if(operator_data.bc->dirichlet.find(bid) != operator_data.bc->dirichlet.end())
        {
          AssertThrow(operator_data.bc->neumann.find(bid) == operator_data.bc->neumann.end(),
        ...
        if(operator_data.bc->neumann.find(bid) != operator_data.bc->neumann.end())
        {
          AssertThrow(periodic_boundary_ids.find(bid) == periodic_boundary_ids.end(),
                      ExcMessage("Boundary id " + Utilities::to_string((int)bid) +
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::disable_mean_value_constraint()
{
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::Tvmult(parallel::distributed::Vector<Number> &       dst,
                                             parallel::distributed::Vector<Number> const & src) const
{
  dst = 0;
  vmult_add(dst, src);
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::Tvmult_add(parallel::distributed::Vector<Number> &       dst,
                                                 parallel::distributed::Vector<Number> const & src) const
{
  vmult_add(dst, src);
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::vmult_add(parallel::distributed::Vector<Number> &       dst,
                                                parallel::distributed::Vector<Number> const & src) const
{
  ...
  for(unsigned int i = 0; i < edge_constrained_indices.size(); ++i)
  {
    ...
    edge_constrained_values[i] = std::pair<Number, Number>(src.local_element(edge_constrained_indices[i]),
  ...
  for(unsigned int i = 0; i < edge_constrained_indices.size(); ++i)
  {
    const_cast<parallel::distributed::Vector<Number> &>(src).local_element(edge_constrained_indices[i]) =
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::run_vmult_loop(parallel::distributed::Vector<Number> &       dst,
                                                     parallel::distributed::Vector<Number> const & src) const
{
  Assert(src.partitioners_are_globally_compatible(
           *data->get_dof_info(operator_data.laplace_dof_index).vector_partitioner),
         ExcInternalError());
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::vmult_interface_down(
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::vmult_add_interface_up(
  parallel::distributed::Vector<Number> &       dst,
  parallel::distributed::Vector<Number> const & src) const
{
  for(unsigned int i = 0; i < edge_constrained_indices.size(); ++i)
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::rhs(parallel::distributed::Vector<Number> & dst) const
{
  dst = 0;
  rhs_add(dst);
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::rhs_add(parallel::distributed::Vector<Number> & dst) const
{
  run_rhs_loop(dst);
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::run_rhs_loop(parallel::distributed::Vector<Number> & dst) const
{
  Assert(dst.partitioners_are_globally_compatible(
           *data->get_dof_info(operator_data.laplace_dof_index).vector_partitioner),
         ExcInternalError());
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::apply_nullspace_projection(
...
LaplaceOperator<dim, degree, Number>::m() const
{
...
template<int dim, int degree, typename Number>
types::global_dof_index
LaplaceOperator<dim, degree, Number>::n() const
{
  return data->get_vector_partitioner(operator_data.laplace_dof_index)->size();
}
...
template<int dim, int degree, typename Number>
Number
LaplaceOperator<dim, degree, Number>::el(const unsigned int, const unsigned int) const
{
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::initialize_dof_vector(
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::calculate_inverse_diagonal(
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::apply_block_jacobi(
  parallel::distributed::Vector<Number> &       dst,
  parallel::distributed::Vector<Number> const & src) const
{
  // apply_inverse_matrices
  data->cell_loop(&LaplaceOperator<dim, degree, Number>::cell_loop_apply_inverse_block_jacobi_matrices,
                  this,
                  dst,
                  src);
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::cell_loop_apply_inverse_block_jacobi_matrices(
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::update_block_jacobi() const
{
  if(block_jacobi_matrices_have_been_initialized == false)
  {
    matrices.resize(data->n_macro_cells() * VectorizedArray<Number>::n_array_elements,
                    LAPACKFullMatrix<Number>(data->get_shape_info().dofs_per_component_on_cell,
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::local_apply(
  const MatrixFree<dim, Number> &               data,
  parallel::distributed::Vector<Number> &       dst,
  const parallel::distributed::Vector<Number> & src,
  const std::pair<unsigned int, unsigned int> & cell_range) const
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  ...
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> fe_eval(data,
                                                               true,
                                                               operator_data.laplace_dof_index,
                                                               operator_data.laplace_quad_index);
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> fe_eval_neighbor(data,
                                                                        false,
                                                                        operator_data.laplace_dof_index,
                                                                        operator_data.laplace_quad_index);
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    VectorizedArray<Number> sigmaF = std::max(fe_eval.read_cell_data(array_penalty_parameter),
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    fe_eval.read_dof_values(src);
    fe_eval.evaluate(true, true);
    VectorizedArray<Number> sigmaF = fe_eval.read_cell_data(array_penalty_parameter) *
                                     IP::get_penalty_factor<Number>(degree, operator_data.penalty_factor);
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::local_rhs(const MatrixFree<dim, Number> &,
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::local_rhs_face(const MatrixFree<dim, Number> &,
                                                     parallel::distributed::Vector<Number> &,
                                                     const parallel::distributed::Vector<Number> &,
                                                     const std::pair<unsigned int, unsigned int> &) const
{
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    for(unsigned int q = 0; q < fe_eval.n_q_points; ++q)
    {
      ...
      if(it != operator_data.bc->dirichlet.end())
      {
        ...
        fe_eval.submit_normal_gradient(-0.5 * (-jump_value), q); // (-jump_value) since this term is
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::local_diagonal_cell(
  const MatrixFree<dim, Number> &         data,
  parallel::distributed::Vector<Number> & dst,
  const unsigned int &,
  const std::pair<unsigned int, unsigned int> & cell_range) const
{
  ...
  for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
  {
    ...
    for(unsigned int i = 0; i < fe_eval.dofs_per_cell; ++i)
    {
      ...
      fe_eval.begin_dof_values()[i] = 1.;

      do_cell_integral(fe_eval);

      local_diagonal_vector[i] = fe_eval.begin_dof_values()[i];
    }
    for(unsigned int i = 0; i < fe_eval.tensor_dofs_per_cell; ++i)
      fe_eval.begin_dof_values()[i] = local_diagonal_vector[i];
    fe_eval.distribute_local_to_global(dst);
  }
}
...
//      FullMatrix<Number>(fe_eval.dofs_per_cell, fe_eval.dofs_per_cell));
//  VectorizedArray<Number> local_diagonal_vector[fe_eval.tensor_dofs_per_cell];
//  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
//    {
//      fe_eval.reinit (cell);
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  ...
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> fe_eval(data,
                                                           ...
                                                           operator_data.laplace_dof_index,
                                                           operator_data.laplace_quad_index);
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> fe_eval_outer(data,
                                                                 false,
                                                                 operator_data.laplace_dof_index,
                                                                 operator_data.laplace_quad_index);
  ...
  {
    fe_eval.reinit(face);
    fe_eval_outer.reinit(face);

    VectorizedArray<Number> sigmaF = std::max(fe_eval.read_cell_data(array_penalty_parameter),
                                              fe_eval_outer.read_cell_data(array_penalty_parameter)) *
                                     IP::get_penalty_factor<Number>(degree, operator_data.penalty_factor);

    // element-
    for(unsigned int j = 0; j < fe_eval.dofs_per_cell; ++j)
      fe_eval_outer.begin_dof_values()[j] = VectorizedArray<Number>();
    fe_eval_outer.evaluate(true, true);
    for(unsigned int i = 0; i < fe_eval.dofs_per_cell; ++i)
    {
      for(unsigned int j = 0; j < fe_eval.dofs_per_cell; ++j)
        fe_eval.begin_dof_values()[j] = VectorizedArray<Number>();
      ...
      {
        VectorizedArray<Number> valueM = fe_eval.get_value(q);
        VectorizedArray<Number> valueP = fe_eval_outer.get_value(q);

        VectorizedArray<Number> jump_value = valueM - valueP;
        VectorizedArray<Number> average_gradient =
          (fe_eval.get_normal_gradient(q) + fe_eval_outer.get_normal_gradient(q)) * 0.5;
        average_gradient = average_gradient - jump_value * sigmaF;
    ...
    for(unsigned int i = 0; i < fe_eval.dofs_per_cell; ++i)
    {
      for(unsigned int j = 0; j < fe_eval.dofs_per_cell; ++j)
        fe_eval_outer.begin_dof_values()[j] = VectorizedArray<Number>();
      fe_eval_outer.begin_dof_values()[i] = 1.;
      fe_eval_outer.evaluate(true, true);
      ...
      for(unsigned int q = 0; q < fe_eval.n_q_points; ++q)
      {
        ...
        fe_eval_outer.submit_normal_gradient(-0.5 * jump_value, q);
        fe_eval_outer.submit_value(average_gradient, q);
      }
      fe_eval_outer.integrate(true, true);
      local_diagonal_vector[i] = fe_eval_outer.begin_dof_values()[i];
    }
    for(unsigned int i = 0; i < fe_eval.dofs_per_cell; ++i)
      fe_eval_outer.begin_dof_values()[i] = local_diagonal_vector[i];
    fe_eval_outer.distribute_local_to_global(dst);
  }
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::add_block_jacobi_matrices(
  std::vector<LAPACKFullMatrix<Number>> & matrices) const
{
  ...
  data->loop(&LaplaceOperator<dim, degree, Number>::cell_loop_calculate_block_jacobi_matrices,
             ...
             &LaplaceOperator<dim, degree, Number>::boundary_face_loop_calculate_block_jacobi_matrices,
             this,
             matrices,
             src);
}
...
template<int dim, int degree, typename Number>
void
LaplaceOperator<dim, degree, Number>::cell_loop_calculate_block_jacobi_matrices(
  const MatrixFree<dim, Number> &         data,
  ...
  const std::pair<unsigned int, unsigned int> & face_range) const
{
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    for(unsigned int j = 0; j < fe_eval.dofs_per_cell; ++j)
    {
      ...
      for(unsigned int q = 0; q < fe_eval.n_q_points; ++q)
      {
        ...
        VectorizedArray<Number> average_gradient = 0.5 * fe_eval.get_normal_gradient(q);
        average_gradient                         = average_gradient - jump_value * sigmaF;
  ...
  for(unsigned int face = face_range.first; face < face_range.second; face++)
  {
    ...
    for(unsigned int j = 0; j < fe_eval_outer.dofs_per_cell; ++j)
    {
      // set dof value j of element+ to 1 and all other dof values of element+
      // to zero
      for(unsigned int i = 0; i < fe_eval_outer.dofs_per_cell; ++i)
        fe_eval_outer.begin_dof_values()[i] = make_vectorized_array<Number>(0.);
      fe_eval_outer.begin_dof_values()[j] = make_vectorized_array<Number>(1.);

      fe_eval_outer.evaluate(true, true);

      for(unsigned int q = 0; q < fe_eval.n_q_points; ++q)
      {
        VectorizedArray<Number> jump_value = fe_eval_outer.get_value(q);
        // minus sign to get the correct normal vector n⁺ = -n⁻
        VectorizedArray<Number> average_gradient = -0.5 * fe_eval_outer.get_normal_gradient(q);
        average_gradient                         = average_gradient - jump_value * sigmaF;
      ...
      for(unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements; ++v)
      {
        ...
        if(cell_number != numbers::invalid_unsigned_int)
          for(unsigned int i = 0; i < fe_eval_outer.dofs_per_cell; ++i)
            matrices[cell_number](i, j) += fe_eval_outer.begin_dof_values()[i][v];
      }
    }
  }
}
...
template<int dim, int degree, typename Number>
double
LaplaceOperator<dim, degree, Number>::get_penalty_factor() const
{
...
template<int dim, int degree, typename Number>
bool
LaplaceOperator<dim, degree, Number>::is_empty_locally() const
{
