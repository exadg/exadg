...
LaplaceOperator<dim, degree, Number>::LaplaceOperator()
    : data(0), needs_mean_value_constraint(false),
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::reinit(
    const MatrixFree<dim, Number> &mf_data, const Mapping<dim> &mapping,
    const LaplaceOperatorData<dim> &operator_data) {
  this->data = &mf_data;
  this->operator_data = operator_data;
  //  this->fe_degree =
  //  mf_data.get_dof_handler(operator_data.laplace_dof_index).get_fe().degree;
  ...
  //    linfty_norm / linfty_norm_compare <
  //    std::pow(std::numeric_limits<Number>::epsilon(), 2./3.);

  // TODO
  // alternative approach: specify whether the system of equations is singular
  // by using an input parameter
...
void LaplaceOperator<dim, degree, Number>::reinit(
  ...
  if (is_feq && level == numbers::invalid_unsigned_int) {
    ...
    std::vector<
        ...
        periodic_faces;
    for (typename std::vector<GridTools::PeriodicFacePair<
             typename Triangulation<dim>::cell_iterator>>::const_iterator it =
             operator_data.periodic_face_pairs_level0.begin();
         it != operator_data.periodic_face_pairs_level0.end(); ++it) {
      GridTools::PeriodicFacePair<typename DoFHandler<dim>::cell_iterator>
          periodic;
      for (unsigned int i = 0; i < 2; ++i) {
        periodic.cell[i] = typename DoFHandler<dim>::cell_iterator(
            &dof_handler.get_triangulation(), it->cell[i]->level(),
            it->cell[i]->index(), &dof_handler);
    ...
    for (typename std::vector<GridTools::PeriodicFacePair<
             ...
             operator_data.periodic_face_pairs_level0.begin();
         it != operator_data.periodic_face_pairs_level0.end(); ++it) {
      typename DoFHandler<dim>::cell_iterator cell1(
          &dof_handler.get_triangulation(), 0, it->cell[1]->index(),
          &dof_handler),
          cell0(&dof_handler.get_triangulation(), 0, it->cell[0]->index(),
                &dof_handler);
      add_periodicity_constraints<dim>(
          level, level, cell1->face(it->face_idx[1]),
          cell0->face(it->face_idx[0]), constraints);
    }
  ...
  // is required.
  if (verify_boundary_conditions(dof_handler, operator_data) && is_feq &&
      Utilities::MPI::sum(edge_constrained_indices.size(), MPI_COMM_WORLD) ==
          0 &&
...
template <int dim, int degree, typename Number>
bool LaplaceOperator<dim, degree, Number>::verify_boundary_conditions(
  ...
  MPI_Comm mpi_communicator =
      dynamic_cast<const parallel::Triangulation<dim> *>(&tria)
          ? (dynamic_cast<const parallel::Triangulation<dim> *>(&tria))
                ->get_communicator()
          : MPI_COMM_SELF;
  const int max_pure_neumann =
      Utilities::MPI::max(my_neumann, mpi_communicator);
  int min_pure_neumann = Utilities::MPI::min(my_neumann, mpi_communicator);
  AssertThrow(
      max_pure_neumann == min_pure_neumann,
      ExcMessage(
          "Neumann/Dirichlet assignment over processors does not match."));
...
// template <int dim, int degree, typename Number>
// void
// LaplaceOperator<dim,degree,Number>::compute_array_penalty_parameter(const
// Mapping<dim> &mapping)
//{
//  std::set<types::boundary_id> periodic_boundary_ids;
//  for (unsigned int i=0; i<operator_data.periodic_face_pairs_level0.size();
...
//
//  for (unsigned int i=0; i<data->n_macro_cells()+data->n_macro_ghost_cells();
//  ++i)
//  {
//    for (unsigned int v=0; v<data->n_components_filled(i); ++v)
//    {
//      typename DoFHandler<dim>::cell_iterator cell =
//      data->get_cell_iterator(i,v,operator_data.laplace_dof_index);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::vmult(
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::Tvmult(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  dst = 0;
  vmult_add(dst, src);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::Tvmult_add(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  vmult_add(dst, src);
}
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::vmult_add(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  ...
  for (unsigned int i = 0; i < edge_constrained_indices.size(); ++i) {
    ...
    const_cast<parallel::distributed::Vector<Number> &>(src).local_element(
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::run_vmult_loop(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  Assert(src.partitioners_are_globally_compatible(
             *data->get_dof_info(operator_data.laplace_dof_index)
                  .vector_partitioner),
         ExcInternalError());
  ...
  data->loop(&LaplaceOperator<dim, degree, Number>::local_apply,
             ...
             &LaplaceOperator<dim, degree, Number>::local_apply_boundary, this,
             dst, src);
  ...
  const std::vector<unsigned int> &constrained_dofs =
      data->get_constrained_dofs(operator_data.laplace_dof_index);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::vmult_interface_down(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  dst = 0;
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::vmult_add_interface_up(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  for (unsigned int i = 0; i < edge_constrained_indices.size(); ++i) {
    edge_constrained_values[i] = std::pair<Number, Number>(
  ...
  for (unsigned int i = 0; i < edge_constrained_indices.size(); ++i) {
    dst.local_element(edge_constrained_indices[i]) =
        edge_constrained_values[i].first + edge_constrained_values[i].second;
  }
}
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::rhs(
    parallel::distributed::Vector<Number> &dst) const {
  dst = 0;
  rhs_add(dst);
}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::rhs_add(
    parallel::distributed::Vector<Number> &dst) const {
  run_rhs_loop(dst);
}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::run_rhs_loop(
    parallel::distributed::Vector<Number> &dst) const {
  Assert(dst.partitioners_are_globally_compatible(
             *data->get_dof_info(operator_data.laplace_dof_index)
                  .vector_partitioner),
  ...
  data->loop(&LaplaceOperator<dim, degree, Number>::local_rhs,
             ...
             &LaplaceOperator<dim, degree, Number>::local_rhs_boundary, this,
             dst, src);
}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::apply_nullspace_projection(
...
template <int dim, int degree, typename Number>
Number LaplaceOperator<dim, degree, Number>::el(const unsigned int,
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::initialize_dof_vector(
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::calculate_inverse_diagonal(
  ...
  if (apply_mean_value_constraint_in_matvec) {
    ...
    inverse_diagonal_entries.add(-2. / length, d, factor / pow(length, 2.),
                                 vec1);
  }
  ...
  //  pcout << "L2 norm diagonal = "<< std::scientific << std::setprecision(15)
  //  << l2_norm << std::endl;
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::apply_block_jacobi(
    parallel::distributed::Vector<Number> &dst,
    parallel::distributed::Vector<Number> const &src) const {
  // apply_inverse_matrices
  data->cell_loop(
      &LaplaceOperator<dim, degree,
                       Number>::cell_loop_apply_inverse_block_jacobi_matrices,
      this, dst, src);
}
...
void LaplaceOperator<dim, degree, Number>::
  ...
  FEEvaluation<dim, degree, degree + 1, 1, Number> fe_eval(
      data, operator_data.laplace_dof_index, operator_data.laplace_quad_index);
  ...
  for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
    ...
    for (unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements;
      ...
      matrices[cell * VectorizedArray<Number>::n_array_elements + v].solve(
          src_vector, false);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::update_block_jacobi() const {
  if (block_jacobi_matrices_have_been_initialized == false) {
    matrices.resize(data->n_macro_cells() *
                        VectorizedArray<Number>::n_array_elements,
                    LAPACKFullMatrix<Number>(
                        data->get_shape_info().dofs_per_component_on_cell,
                        data->get_shape_info().dofs_per_component_on_cell));
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_apply(
    const MatrixFree<dim, Number> &data,
    parallel::distributed::Vector<Number> &dst,
    const parallel::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &cell_range) const {
  FEEvaluation<dim, degree, degree + 1, 1, Number> phi(
      data, operator_data.laplace_dof_index, operator_data.laplace_quad_index);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_apply_boundary(
    const MatrixFree<dim, Number> &data,
    parallel::distributed::Vector<Number> &dst,
    const parallel::distributed::Vector<Number> &src,
    const std::pair<unsigned int, unsigned int> &face_range) const {
  ...
  for (unsigned int face = face_range.first; face < face_range.second; face++) {
    ...
    for (unsigned int q = 0; q < fe_eval.n_q_points; ++q) {
      ...
      if (it != operator_data.bc->neumann.end()) {
        // set gradient in normal direction to zero, i.e. u+ = u-, grad+ =
        // -grad-
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_rhs(
    const MatrixFree<dim, Number> &, parallel::distributed::Vector<Number> &,
    const parallel::distributed::Vector<Number> &,
    const std::pair<unsigned int, unsigned int> &) const {}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_rhs_face(
    const MatrixFree<dim, Number> &, parallel::distributed::Vector<Number> &,
    const parallel::distributed::Vector<Number> &,
    const std::pair<unsigned int, unsigned int> &) const {}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_rhs_boundary(
    const MatrixFree<dim, Number> &data,
    parallel::distributed::Vector<Number> &dst,
    const parallel::distributed::Vector<Number> &,
    const std::pair<unsigned int, unsigned int> &face_range) const {
  ...
  for (unsigned int face = face_range.first; face < face_range.second; face++) {
    ...
    for (unsigned int q = 0; q < fe_eval.n_q_points; ++q) {
      ...
      if (it != operator_data.bc->dirichlet.end()) {
        ...
        Point<dim, VectorizedArray<Number>> q_points =
            fe_eval.quadrature_point(q);
        Number array[VectorizedArray<Number>::n_array_elements];
        for (unsigned int n = 0; n < VectorizedArray<Number>::n_array_elements;
             ++n) {
          Point<dim> q_point;
        ...
        fe_eval.submit_normal_gradient(
            -0.5 * (-jump_value), q); // (-jump_value) since this term is
                                      // shifted to the rhs of the equations
        fe_eval.submit_value(average_gradient, q); // (+average_gradient) since
                                                   // this term is shifted to
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_diagonal_cell(
    const MatrixFree<dim, Number> &data,
...
//  matrices(data.n_macro_cells()*VectorizedArray<Number>::n_array_elements,
...
//              matrices[cell*VectorizedArray<Number>::n_array_elements+v](j,i)
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_diagonal_face(
    const MatrixFree<dim, Number> &data,
    parallel::distributed::Vector<Number> &dst, const unsigned int &,
    const std::pair<unsigned int, unsigned int> &face_range) const {
  // Nothing to do for continuous elements
  ...
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> phi(
      data, true, operator_data.laplace_dof_index,
      operator_data.laplace_quad_index);
  FEFaceEvaluation<dim, degree, degree + 1, 1, Number> phi_outer(
      data, false, operator_data.laplace_dof_index,
      operator_data.laplace_quad_index);
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::local_diagonal_boundary(
    const MatrixFree<dim, Number> &data,
    parallel::distributed::Vector<Number> &dst, const unsigned int &,
    const std::pair<unsigned int, unsigned int> &face_range) const {
  // Nothing to do for continuous elements
  ...
  for (unsigned int face = face_range.first; face < face_range.second; face++) {
    ...
    for (unsigned int i = 0; i < phi.dofs_per_cell; ++i) {
      ...
      for (unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        it = operator_data.bc->neumann.find(boundary_id);
        if (it != operator_data.bc->neumann.end()) {
          // set solution gradient in normal direction to zero, i.e. u+ = u-,
          // grad+ = -grad-
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::add_block_jacobi_matrices(
    std::vector<LAPACKFullMatrix<Number>> &matrices) const {
  parallel::distributed::Vector<Number> src;

  data->loop(
      &LaplaceOperator<dim, degree,
                       Number>::cell_loop_calculate_block_jacobi_matrices,
      &LaplaceOperator<dim, degree,
                       Number>::face_loop_calculate_block_jacobi_matrices,
      &LaplaceOperator<dim, degree, Number>::
  ...
  for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
    ...
    for (unsigned int j = 0; j < phi.dofs_per_cell; ++j) {
      ...
      for (unsigned int i = 0; i < phi.dofs_per_cell; ++i)
        for (unsigned int v = 0; v < VectorizedArray<Number>::n_array_elements;
             ++v)
          matrices[cell * VectorizedArray<Number>::n_array_elements + v](
              i, j) += phi.begin_dof_values()[i][v];
    }
  }
}
...
void LaplaceOperator<dim, degree, Number>::
  ...
  for (unsigned int face = face_range.first; face < face_range.second; face++) {
    ...
    for (unsigned int j = 0; j < phi.dofs_per_cell; ++j) {
      // set dof value j of element- to 1 and all other dof values of element-
      // to zero
      for (unsigned int i = 0; i < phi.dofs_per_cell; ++i)
        phi.begin_dof_values()[i] = make_vectorized_array<Number>(0.);
      phi.begin_dof_values()[j] = make_vectorized_array<Number>(1.);
  ...
  for (unsigned int face = face_range.first; face < face_range.second; face++) {
    ...
    for (unsigned int j = 0; j < phi_outer.dofs_per_cell; ++j) {
      ...
      for (unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        VectorizedArray<Number> average_gradient =
            -0.5 * phi_outer.get_normal_gradient(q);
        average_gradient = average_gradient - jump_value * sigmaF;
...
// TODO: This function has to be removed as soon as the new infrastructure is
// used that
// allows to perform face integrals over all faces of the current element.
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::
...
LaplaceOperator<dim, degree, Number>::get_array_penalty_parameter() const {
  return array_penalty_parameter;
}

template <int dim, int degree, typename Number>
double LaplaceOperator<dim, degree, Number>::get_penalty_factor() const {
  return IP::get_penalty_factor<Number>(degree, operator_data.penalty_factor);
}

template <int dim, int degree, typename Number>
const LaplaceOperatorData<dim> &
LaplaceOperator<dim, degree, Number>::get_operator_data() const {
  return operator_data;
}

template <int dim, int degree, typename Number>
bool LaplaceOperator<dim, degree, Number>::is_empty_locally() const {
...
template <int dim, int degree, typename Number>
unsigned int LaplaceOperator<dim, degree, Number>::get_dof_index() const {
  return operator_data.laplace_dof_index;
}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::cell(
    MeshWorker::DoFInfo<dim, dim> &dinfo,
  ...
  for (unsigned int point = 0; point < fe_values.n_quadrature_points; ++point)
    for (unsigned int i = 0; i < fe_values.dofs_per_cell; ++i)
      for (unsigned int j = 0; j < fe_values.dofs_per_cell; ++j)
        local_matrix(i, j) += fe_values.shape_grad(i, point) *
                              fe_values.shape_grad(j, point) * JxW[point];
}

template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::boundary(
    MeshWorker::DoFInfo<dim, dim> &dinfo,
    typename MeshWorker::IntegrationInfo<dim> &info) const {
  ...
  if (it != operator_data.bc->dirichlet.end()) {
    ...
    double tau =
        array_penalty_parameter_nv[dinfo.cell->index()] *
        IP::get_penalty_factor<Number>(degree, operator_data.penalty_factor);
    ...
    for (unsigned int point = 0; point < fe_face_values.n_quadrature_points;
      ...
      for (unsigned int i = 0; i < fe_face_values.dofs_per_cell; ++i)
        for (unsigned int j = 0; j < fe_face_values.dofs_per_cell; ++j)
          local_matrix(i, j) +=
              +2.0 * fe_face_values.shape_value(i, point) *
                  fe_face_values.shape_value(j, point) * tau * JxW[point] -
              1.0 * fe_face_values.shape_grad(i, point) *
                  (fe_face_values.shape_value(j, point) * normals[point]) *
                  JxW[point] -
              1.0 * fe_face_values.shape_value(i, point) *
                  (fe_face_values.shape_grad(j, point) * normals[point]) *
                  JxW[point];
    }
  }
}
...
template <int dim, int degree, typename Number>
void LaplaceOperator<dim, degree, Number>::face(
    MeshWorker::DoFInfo<dim, dim> &dinfo1,
  ...
  const std::vector<Tensor<1, dim>> &normals =
      fe_face_values_1.get_normal_vectors();

  double tau =
      std::max(array_penalty_parameter_nv[dinfo1.cell->index()],
               array_penalty_parameter_nv[dinfo2.cell->index()]) *
      IP::get_penalty_factor<Number>(degree, operator_data.penalty_factor);

  for (unsigned int point = 0; point < fe_face_values_1.n_quadrature_points;
       ++point) {
    for (unsigned int i = 0; i < fe_face_values_1.dofs_per_cell; ++i)
      for (unsigned int j = 0; j < fe_face_values_1.dofs_per_cell; ++j)
        q1_p1_matrix(i, j) +=
            +fe_face_values_1.shape_value(i, point) *
                fe_face_values_1.shape_value(j, point) * tau * JxW[point] +
            0.5 * fe_face_values_1.shape_grad(i, point) *
                (fe_face_values_1.shape_value(j, point) * normals[point]) *
                JxW[point] -
            0.5 * fe_face_values_1.shape_value(i, point) *
                (fe_face_values_1.shape_grad(j, point) * normals[point]) *
                JxW[point];
    ...
    for (unsigned int i = 0; i < fe_face_values_1.dofs_per_cell; ++i)
      for (unsigned int j = 0; j < fe_face_values_2.dofs_per_cell; ++j)
        q1_p2_matrix(i, j) +=
            -fe_face_values_1.shape_value(i, point) *
                fe_face_values_2.shape_value(j, point) * tau * JxW[point] -
            0.5 * fe_face_values_1.shape_grad(i, point) *
                (fe_face_values_2.shape_value(j, point) * normals[point]) *
                JxW[point] -
            0.5 * fe_face_values_1.shape_value(i, point) *
                (fe_face_values_2.shape_grad(j, point) * normals[point]) *
                JxW[point];

    for (unsigned int i = 0; i < fe_face_values_2.dofs_per_cell; ++i)
      for (unsigned int j = 0; j < fe_face_values_1.dofs_per_cell; ++j)
        q2_p1_matrix(i, j) +=
            -fe_face_values_2.shape_value(i, point) *
                fe_face_values_1.shape_value(j, point) * tau * JxW[point] +
            0.5 * fe_face_values_2.shape_grad(i, point) *
                (fe_face_values_1.shape_value(j, point) * normals[point]) *
                JxW[point] +
            0.5 * fe_face_values_2.shape_value(i, point) *
                (fe_face_values_1.shape_grad(j, point) * normals[point]) *
                JxW[point];
