...
LaplaceOperator<dim, fe_degree, Number>::calculate_exterior_value(
    VectorizedArray<Number> const &value_m, unsigned int const q,
...
LaplaceOperator<dim, fe_degree, Number>::
    calculate_interior_normal_gradient(
        unsigned int const q, FEEvalFace const &fe_eval,
        OperatorType const &operator_type) const {
  VectorizedArray<Number> normal_gradient_m =
...
LaplaceOperator<dim, fe_degree, Number>::
    calculate_exterior_normal_gradient(
        ...
        types::boundary_id const boundary_id) const {
  VectorizedArray<Number> normal_gradient_p =
      make_vectorized_array<Number>(0.0);
...
void LaplaceOperator<dim, fe_degree, value_type>::do_face_integral(
...
template <int dim, int fe_degree, typename value_type>
void LaplaceOperator<dim, fe_degree, value_type>::do_face_int_integral(
    FEEvalFace &fe_eval, FEEvalFace &fe_eval_neighbor) const {
  VectorizedArray<value_type> tau_IP =
      std::max(fe_eval.read_cell_data(array_penalty_parameter),
               fe_eval_neighbor.read_cell_data(array_penalty_parameter)) *
...
template <int dim, int fe_degree, typename value_type>
void LaplaceOperator<dim, fe_degree, value_type>::do_face_ext_integral(
    FEEvalFace &fe_eval, FEEvalFace &fe_eval_neighbor) const {
  VectorizedArray<value_type> tau_IP =
      std::max(fe_eval.read_cell_data(array_penalty_parameter),
               fe_eval_neighbor.read_cell_data(array_penalty_parameter)) *
...
template <int dim, int fe_degree, typename value_type>
void LaplaceOperator<dim, fe_degree, value_type>::do_boundary_integral(
