...
public:
  ...
  virtual void solve_timestep(Vector       &dst,
                              Vector       &src,
                              double const time,
...
public:
                      ...
                      double const time_step)
  {
    ...
    vec_tmp1.add(a32*time_step,vec_tmp2 /* F_2 */,(a31-b1)*time_step,vec_np /* F_1 */); /* = u_3 */
    ...
    vec_np.add(a43*time_step,vec_n /* F_3 */,(a42-b2)*time_step,vec_tmp2 /* F_2 */); /* = u_4 */
    vec_tmp2.sadd((b2-a42)*time_step,1,vec_np /* u_4 */);
    vec_tmp2.add((b3-a43)*time_step,vec_n /* F_3 */); /* u_p */

    // stage 4
    this->underlying_operator->evaluate(vec_tmp1 /* F_4 */,vec_np /* u_4 */,time + c4*time_step);
    vec_tmp2.add(a54*time_step,vec_tmp1 /* F_4 */,(a53-b3)*time_step,vec_n /* F_3 */); /* = u_5 */
    vec_n.sadd((b3-a53)*time_step,1,vec_tmp2 /* u_5 */);
    vec_n.add((b4-a54)*time_step,vec_tmp1 /* F_4 */); /* = u_p */

    // stage 5
    this->underlying_operator->evaluate(vec_tmp1 /* F_5 */,vec_tmp2 /* u_5 */,time + c5*time_step);
    vec_np = vec_n;
...
public:
                      ...
                      double const time_step)
  {
    ...
    std::vector<double> A; A.resize(stages);
    std::vector<double> B; B.resize(stages);
    std::vector<double> c; c.resize(stages);
