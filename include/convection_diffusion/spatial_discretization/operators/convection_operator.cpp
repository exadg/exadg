...
void ConvectiveOperator<dim, fe_degree, value_type>::initialize(
...
ConvectiveOperator<dim, fe_degree, value_type>::calculate_central_flux(
    VectorizedArray<value_type> &value_m, VectorizedArray<value_type> &value_p,
...
template <int dim, int fe_degree, typename value_type>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<value_type>
ConvectiveOperator<dim, fe_degree, value_type>::calculate_flux(
    unsigned int const q, FEEvalFace &fe_eval,
  ...
  Point<dim, VectorizedArray<value_type>> q_points =
      fe_eval.quadrature_point(q);
  Tensor<1, dim, VectorizedArray<value_type>> velocity;

  evaluate_vectorial_function(velocity, this->ad.velocity, q_points,
                              this->eval_time);
...
template <int dim, int fe_degree, typename value_type>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<value_type>
ConvectiveOperator<dim, fe_degree, value_type>::calculate_interior_value(
...
ConvectiveOperator<dim, fe_degree, value_type>::calculate_exterior_value(
  ...
  if (boundary_type == BoundaryType::dirichlet) {
    if (operator_type == OperatorType::full ||
      ...
      AssertThrow(false,
                  ExcMessage("Specified OperatorType is not implemented!"));
    }
...
template <int dim, int fe_degree, typename value_type>
void ConvectiveOperator<dim, fe_degree, value_type>::do_cell_integral(
...
template <int dim, int fe_degree, typename value_type>
void ConvectiveOperator<dim, fe_degree, value_type>::do_face_integral(
  ...
  for (unsigned int q = 0; q < fe_eval.n_q_points; ++q) {
    ...
    VectorizedArray<value_type> numerical_flux =
        calculate_flux(q, fe_eval, value_m, value_p);
...
template <int dim, int fe_degree, typename value_type>
void ConvectiveOperator<dim, fe_degree, value_type>::do_face_int_integral(
    FEEvalFace &fe_eval, FEEvalFace & /*fe_eval_neighbor*/) const {
  for (unsigned int q = 0; q < fe_eval.n_q_points; ++q) {
    VectorizedArray<value_type> value_m = fe_eval.get_value(q);
    // set value_p to zero
...
template <int dim, int fe_degree, typename value_type>
void ConvectiveOperator<dim, fe_degree, value_type>::do_face_ext_integral(
    FEEvalFace & /*fe_eval*/, FEEvalFace &fe_eval_neighbor) const {
  for (unsigned int q = 0; q < fe_eval_neighbor.n_q_points; ++q) {
    ...
    VectorizedArray<value_type> value_p = fe_eval_neighbor.get_value(q);
    VectorizedArray<value_type> numerical_flux =
        calculate_flux(q, fe_eval_neighbor, value_m, value_p);
...
template <int dim, int fe_degree, typename value_type>
void ConvectiveOperator<dim, fe_degree, value_type>::do_boundary_integral(
