...
void DiffusiveOperator<dim, fe_degree, value_type>::initialize(
    Mapping<dim> const &mapping, MatrixFree<dim, value_type> const &mf_data,
    DiffusiveOperatorData<dim> const &operator_data_in) {
  ConstraintMatrix cm;
  Parent::reinit(mf_data, cm, operator_data_in);

  IP::calculate_penalty_parameter<dim, fe_degree, value_type>(
...
template <int dim, int fe_degree, typename value_type>
void DiffusiveOperator<dim, fe_degree, value_type>::apply_add(
...
template <int dim, int fe_degree, typename value_type>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<value_type>
DiffusiveOperator<dim, fe_degree, value_type>::
