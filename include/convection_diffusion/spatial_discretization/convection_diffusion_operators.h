...
{
  ConvectiveOperatorDataDiscontinuousVelocity() : dof_index(0), dof_index_velocity(0), quad_index(0)
  {
  }
...
public:
  ...
  ConvectiveOperatorDiscontinuousVelocity() : data(nullptr), velocity(nullptr)
  {
  }
...
private:
            ...
            std::pair<unsigned int, unsigned int> const & face_range) const
  {
    ...
    FEFaceEvaluation<dim, fe_degree_velocity, fe_degree + 1, dim, value_type>
      fe_eval_velocity_neighbor(data,
                                false,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; face++)
    {
      ...
      for(unsigned int q = 0; q < fe_eval.n_q_points; ++q)
      {
        ...
        Tensor<1, dim, VectorizedArray<value_type>> velocity_p =
        ...
        VectorizedArray<value_type> average_value =
          0.5 * (velocity_m_normal * value_m + velocity_p_normal * value_p);
        VectorizedArray<value_type> jump_value = value_m - value_p;
        VectorizedArray<value_type> lambda =
          std::max(std::abs(velocity_m_normal), std::abs(velocity_p_normal));
