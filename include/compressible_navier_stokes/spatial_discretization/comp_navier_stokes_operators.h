...
template<int dim, typename value_type>
inline Tensor<1,dim,VectorizedArray<value_type> >
...
template<int dim, typename value_type>
inline Tensor<2,dim,VectorizedArray<value_type> >
...
{
  Tensor<1,dim,VectorizedArray<value_type> > u = rho_inverse * rho_u;

  return rho_inverse * (grad_rho_u - outer_product(u,grad_rho));
}
...
{
  SymmetricTensor<2,dim,VectorizedArray<value_type> > I =
      unit_symmetric_tensor<dim,VectorizedArray<value_type> >();

  return viscosity * (grad_u + transpose(grad_u) - 2.0/3.0 * trace(grad_u) * I);
}
...
template<int dim, typename value_type>
inline VectorizedArray<value_type>
calc_exterior_value(VectorizedArray<value_type> const                       &value_m,
...
template<int dim, typename value_type>
inline Tensor<1,dim,VectorizedArray<value_type> >
calc_exterior_value(Tensor<1,dim,VectorizedArray<value_type> > const        &value_m,
                    BoundaryType const                                      boundary_type,
                    std::shared_ptr<CompNS::BoundaryDescriptor<dim> > const &boundary_descriptor,
                    types::boundary_id const                                &boundary_id,
                    Point<dim,VectorizedArray<value_type> > const           &q_point,
...
template<int dim, typename value_type>
inline Tensor<1,dim,VectorizedArray<value_type> >
calc_exterior_normal_grad(Tensor<1,dim,VectorizedArray<value_type> > const        &tau_M_normal,
                          BoundaryType const                                      &boundary_type,
                          std::shared_ptr<CompNS::BoundaryDescriptor<dim> > const &boundary_descriptor,
                          types::boundary_id const                                &boundary_id,
                          Point<dim,VectorizedArray<value_type> > const           &q_point,
...
template<int dim, typename value_type>
inline VectorizedArray<value_type>
calc_exterior_normal_grad(VectorizedArray<value_type> const                       &grad_T_M_normal,
                          BoundaryType const                                      &boundary_type,
                          std::shared_ptr<CompNS::BoundaryDescriptor<dim> > const &boundary_descriptor,
                          types::boundary_id const                                &boundary_id,
                          Point<dim,VectorizedArray<value_type> > const           &q_point,
...
{
  Tensor<2,dim,VectorizedArray<value_type> > average_flux =
    make_vectorized_array<value_type>(0.5) * (momentum_flux_M + momentum_flux_P);

  return average_flux * normal + 0.5 * lambda * (rho_u_M - rho_u_P);
}
...
private:
  void cell_loop (const MatrixFree<dim,value_type>                &data,
                  ...
                  const std::pair<unsigned int,unsigned int>      &cell_range) const
  {
    FEEval_scalar fe_eval_density(data, mass_matrix_operator_data.dof_index, mass_matrix_operator_data.quad_index, 0);
    FEEval_velocity fe_eval_momentum(data, mass_matrix_operator_data.dof_index, mass_matrix_operator_data.quad_index, 1);
    FEEval_scalar fe_eval_energy(data, mass_matrix_operator_data.dof_index, mass_matrix_operator_data.quad_index, 1+dim);
...
private:
  ...
  {
    FEFaceEval_scalar fe_eval_density(data,true, operator_data.dof_index, operator_data.quad_index,0);
    FEFaceEval_scalar fe_eval_density_neighbor(data,false, operator_data.dof_index, operator_data.quad_index,0);
    FEFaceEval_vectorial fe_eval_momentum(data,true, operator_data.dof_index, operator_data.quad_index,1);
    FEFaceEval_vectorial fe_eval_momentum_neighbor(data,false, operator_data.dof_index, operator_data.quad_index,1);
    FEFaceEval_scalar fe_eval_energy(data,true, operator_data.dof_index, operator_data.quad_index,1+dim);
    FEFaceEval_scalar fe_eval_energy_neighbor(data,false, operator_data.dof_index, operator_data.quad_index,1+dim);
    ...
    for(unsigned int face=face_range.first; face<face_range.second; face++)
    {
      ...
      for(unsigned int q=0;q<fe_eval_momentum.n_q_points;++q)
      {
        ...
        scalar flux_energy = calculate_flux(energy_flux_M,energy_flux_P,rho_E_M,rho_E_P,lambda,normal);
                           ...
                           const std::pair<unsigned int,unsigned int>      &face_range) const
  {
    ...
    for(unsigned int face=face_range.first; face<face_range.second; face++)
    {
      ...
      if(operator_data.bc_rho->dirichlet_bc.find(boundary_id) != operator_data.bc_rho->dirichlet_bc.end())
        boundary_type_density = BoundaryType::dirichlet;
      else if(operator_data.bc_rho->neumann_bc.find(boundary_id) != operator_data.bc_rho->neumann_bc.end())
      ...
      if(operator_data.bc_u->dirichlet_bc.find(boundary_id) != operator_data.bc_u->dirichlet_bc.end())
        boundary_type_velocity = BoundaryType::dirichlet;
      else if(operator_data.bc_u->neumann_bc.find(boundary_id) != operator_data.bc_u->neumann_bc.end())
      ...
      if(operator_data.bc_p->dirichlet_bc.find(boundary_id) != operator_data.bc_p->dirichlet_bc.end())
        boundary_type_pressure = BoundaryType::dirichlet;
      else if(operator_data.bc_p->neumann_bc.find(boundary_id) != operator_data.bc_p->neumann_bc.end())
      ...
      if(operator_data.bc_E->dirichlet_bc.find(boundary_id) != operator_data.bc_E->dirichlet_bc.end())
        boundary_type_energy = BoundaryType::dirichlet;
      else if(operator_data.bc_E->neumann_bc.find(boundary_id) != operator_data.bc_E->neumann_bc.end())
        boundary_type_energy = BoundaryType::neumann;
      AssertThrow(boundary_type_energy != BoundaryType::undefined,
          ExcMessage("Boundary type of face is invalid or not implemented."));

      EnergyBoundaryVariable boundary_variable = operator_data.bc_E->boundary_variable.find(boundary_id)->second;
      AssertThrow(boundary_variable != EnergyBoundaryVariable::Undefined, ExcMessage("Energy boundary variable is undefined!"));
      ...
      for(unsigned int q=0;q<fe_eval_density.n_q_points;++q)
      {
        ...
        // calculate rho_P
        scalar rho_P = calc_exterior_value<dim,value_type>(rho_M, boundary_type_density, operator_data.bc_rho, boundary_id,
                                                           fe_eval_density.quadrature_point(q), eval_time);

        // calculate u_P
        vector u_P = calc_exterior_value<dim,value_type>(u_M, boundary_type_velocity, operator_data.bc_u, boundary_id,
                                                         fe_eval_momentum.quadrature_point(q), eval_time);

        vector rho_u_P = rho_P * u_P;

        // calculate p_P
        scalar p_P = calc_exterior_value<dim,value_type>(p_M, boundary_type_pressure, operator_data.bc_p, boundary_id,
                                                         fe_eval_density.quadrature_point(q), eval_time);
        ...
        {
          E_P = calc_exterior_value<dim,value_type>(E_M, boundary_type_energy, operator_data.bc_E, boundary_id,
                                                    fe_eval_energy.quadrature_point(q), eval_time);

        }
        else if(boundary_variable == EnergyBoundaryVariable::Temperature)
        {
          scalar T_M = calculate_temperature(p_M, rho_M, R);
          scalar T_P = calc_exterior_value<dim,value_type>(T_M, boundary_type_energy, operator_data.bc_E, boundary_id,
                                                           fe_eval_energy.quadrature_point(q), eval_time);
...
private:
                  ...
                  const std::pair<unsigned int,unsigned int>      &face_range) const
  {
    ...
    for(unsigned int face=face_range.first; face<face_range.second; face++)
    {
      ...
      VectorizedArray<value_type> tau_IP = std::max(fe_eval_momentum.read_cell_data(array_penalty_parameter),
      ...
      for(unsigned int q=0; q<fe_eval_momentum.n_q_points; ++q)
      {
        ...
        tensor grad_u_using_jumps_M = calculate_grad_u(rho_inv_M, rho_u_M, jump_rho /*instead of grad_rho*/, jump_rho_u /*instead of grad_rho_u*/);
        ...
        tensor grad_u_using_jumps_P = calculate_grad_u(rho_inv_P, rho_u_P, jump_rho /*instead of grad_rho*/, jump_rho_u /*instead of grad_rho_u*/);
        ...
        // value flux energy
        vector grad_E_using_jumps_M = calculate_grad_E(rho_inv_M, rho_E_M, jump_rho /*instead of grad_rho*/, jump_rho_E /*instead of grad_rho_E*/);
        vector grad_T_using_jumps_M = calculate_grad_T(grad_E_using_jumps_M, u_M, grad_u_using_jumps_M, gamma, R);
        vector value_flux_energy_M = -0.5 * (tau_using_jumps_M * u_M + lambda * grad_T_using_jumps_M);

        vector grad_E_using_jumps_P = calculate_grad_E(rho_inv_P, rho_E_P, jump_rho /*instead of grad_rho*/, jump_rho_E /*instead of grad_rho_E*/);
        vector grad_T_using_jumps_P = calculate_grad_T(grad_E_using_jumps_P, u_P, grad_u_using_jumps_P, gamma, R);
        ...
        fe_eval_momentum_neighbor.submit_gradient(value_flux_momentum_P,q); // note that value_flux_momentum is not conservative
        ...
        fe_eval_energy_neighbor.submit_gradient(value_flux_energy_P,q); // note that value_flux_energy is not conservative
                           ...
                           const std::pair<unsigned int,unsigned int>      &face_range) const
  {
    ...
    for(unsigned int face=face_range.first; face<face_range.second; face++)
    {
      ...
      VectorizedArray<value_type> tau_IP = fe_eval_momentum.read_cell_data(array_penalty_parameter)
                                              * IP::get_penalty_factor<value_type>(fe_degree, operator_data.IP_factor) * nu;
      ...
      for(unsigned int q=0; q<fe_eval_momentum.n_q_points; ++q)
      {
        ...
        vector tau_P_normal = calc_exterior_normal_grad(tau_M*normal, boundary_type_velocity, operator_data.bc_u, boundary_id,
                                                        fe_eval_momentum.quadrature_point(q), eval_time);
        ...
        scalar grad_T_P_normal = calc_exterior_normal_grad<dim,value_type>(grad_T_M_normal, boundary_type_energy, operator_data.bc_E, boundary_id,
                                                                           fe_eval_energy.quadrature_point(q), eval_time);

        scalar jump_energy = rho_E_M - rho_E_P;
        scalar gradient_flux_energy = 0.5 * (u_M * tau_M * normal + u_P * tau_P_normal + lambda * (grad_T_M*normal + grad_T_P_normal)) - tau_IP * jump_energy;
                  ...
                  const std::pair<unsigned int,unsigned int>       &cell_range) const
  {
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      for (unsigned int q=0; q<fe_eval_momentum.n_q_points; ++q)
      {
        ...
        fe_eval_momentum.submit_gradient (-(outer_product(rho_u,u) /*CONV*/ + p*I) + tau /*VIS*/, q);
                  ...
                  const std::pair<unsigned int,unsigned int>      &face_range) const
  {
    ...
    for(unsigned int face=face_range.first; face<face_range.second; face++)
    {
      ...
      for(unsigned int q=0; q<fe_eval_momentum.n_q_points; ++q)
      {
        ...
        fe_eval_density_neighbor.submit_value(-flux_density /*CONV*/ + gradient_flux_density /*VIS*/,q);
        ...
        fe_eval_momentum_neighbor.submit_value(-flux_momentum /*CONV*/ + gradient_flux_momentum /*VIS*/,q);
        ...
        fe_eval_energy_neighbor.submit_value(-flux_energy /*CONV*/ + gradient_flux_energy /*VIS*/,q);
      }
