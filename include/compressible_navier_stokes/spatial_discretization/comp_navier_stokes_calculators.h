...
private:
  void local_compute_vorticity(MatrixFree<dim,value_type> const                &data,
                               ...
                               std::pair<unsigned int,unsigned int> const      &cell_range) const
  {
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      {
        Tensor<1,number_vorticity_components,VectorizedArray<value_type> > omega = fe_eval.get_curl(q);
...
private:
  void local_compute_divergence(MatrixFree<dim,value_type> const                 &data,
                                parallel::distributed::Vector<value_type>        &dst,
                                parallel::distributed::Vector<value_type>  const &src,
                                std::pair<unsigned int,unsigned int> const       &cell_range) const
  {
...
private:
  void local_apply_pressure(MatrixFree<dim,value_type> const                &data,
                            parallel::distributed::Vector<value_type>       &dst,
                            parallel::distributed::Vector<value_type> const &src,
                            std::pair<unsigned int,unsigned int> const      &cell_range) const
  {
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      for (unsigned int q=0; q<fe_eval_energy.n_q_points; ++q)
      {
        ...
        VectorizedArray<value_type> p = make_vectorized_array<value_type>(heat_capacity_ratio - 1.0) * (rho_E - 0.5 * rho * scalar_product(u,u));
  ...
  void local_apply_velocity(MatrixFree<dim,value_type> const                &data,
                            parallel::distributed::Vector<value_type>       &dst,
                            parallel::distributed::Vector<value_type> const &src,
                            std::pair<unsigned int,unsigned int> const      &cell_range) const
  {
  ...
  void local_apply_temperature(MatrixFree<dim,value_type> const                &data,
                               parallel::distributed::Vector<value_type>       &dst,
                               parallel::distributed::Vector<value_type> const &src,
                               std::pair<unsigned int,unsigned int> const      &cell_range) const
  {
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      for (unsigned int q=0; q<fe_eval_energy.n_q_points; ++q)
      {
        ...
        VectorizedArray<value_type> T = make_vectorized_array<value_type>((heat_capacity_ratio - 1.0)/specific_gas_constant) * (E - 0.5 * scalar_product(u,u));
