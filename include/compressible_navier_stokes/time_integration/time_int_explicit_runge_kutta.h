...
{
public:
  TimeIntExplRKCompNavierStokes(std::shared_ptr<NavierStokesOperation>                  comp_navier_stokes_operation_in,
...
private:
  ...
  std::shared_ptr<ExplicitTimeIntegrator<NavierStokesOperation,
...
setup()
{
  ...
  {
    rk_time_integrator.reset(new ExplicitRungeKuttaTimeIntegrator<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(param.order_time_integrator,comp_navier_stokes_operation));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK3Stage4Reg2C)
  {
    rk_time_integrator.reset(new LowStorageRK3Stage4Reg2C<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK4Stage5Reg2C)
  {
    rk_time_integrator.reset(new LowStorageRK4Stage5Reg2C<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK4Stage5Reg3C)
  {
    rk_time_integrator.reset(new LowStorageRK4Stage5Reg3C<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK5Stage9Reg2S)
  {
    rk_time_integrator.reset(new LowStorageRK5Stage9Reg2S<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK3Stage7Reg2)
  {
    rk_time_integrator.reset(new LowStorageRKTD<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation,3,7));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::ExplRK4Stage8Reg2)
  {
    rk_time_integrator.reset(new LowStorageRKTD<NavierStokesOperation,
        parallel::distributed::Vector<value_type> >(comp_navier_stokes_operation,4,8));
  }
  else if(this->param.temporal_discretization == CompNS::TemporalDiscretization::SSPRK)
  {
    rk_time_integrator.reset(new SSPRK<NavierStokesOperation,parallel::distributed::Vector<value_type> >
...
calculate_timestep()
{
  ...
  else if(param.calculation_of_time_step_size == CompNS::TimeStepCalculation::ConstTimeStepCFL)
  {
    ...
    const double h_min =
        calculate_minimum_vertex_distance(comp_navier_stokes_operation->get_data().get_dof_handler().get_triangulation());
    ...
    double const speed_of_sound = sqrt(param.heat_capacity_ratio*param.specific_gas_constant*param.max_temperature);
    double const acoustic_wave_speed = param.max_velocity + speed_of_sound;

    time_step = calculate_const_time_step_cfl(cfl_number,
                                              acoustic_wave_speed,
                                              h_min,
                                              fe_degree,
  ...
  else if(param.calculation_of_time_step_size == CompNS::TimeStepCalculation::ConstTimeStepDiffusion)
  {
    // calculate minimum vertex distance
    const double h_min =
        calculate_minimum_vertex_distance(comp_navier_stokes_operation->get_data().get_dof_handler().get_triangulation());
  ...
  else if(param.calculation_of_time_step_size == CompNS::TimeStepCalculation::ConstTimeStepCFLAndDiffusion)
  {
    // calculate minimum vertex distance
    const double h_min =
        calculate_minimum_vertex_distance(comp_navier_stokes_operation->get_data().get_dof_handler().get_triangulation());
  ...
  {
    AssertThrow(param.calculation_of_time_step_size == CompNS::TimeStepCalculation::ConstTimeStepUserSpecified ||
                param.calculation_of_time_step_size == CompNS::TimeStepCalculation::ConstTimeStepCFLAndDiffusion,
...
calculate_pressure()
{
  if((this->param.output_data.write_output == true && this->param.output_data.write_pressure == true) ||
...
calculate_velocity()
{
  if((this->param.output_data.write_output == true && this->param.output_data.write_velocity == true) ||
...
{
  if(this->param.output_data.write_output == true && this->param.output_data.write_temperature == true)
  {
...
calculate_vorticity()
{
  if(this->param.output_data.write_output == true && this->param.output_data.write_vorticity == true)
  {
    AssertThrow(this->param.calculate_velocity == true,
        ExcMessage("The velocity field has to be computed in order to calculate vorticity."));
...
postprocessing()
{
  ...
  postprocessor->do_postprocessing(solution_n,
                                   velocity,
                                   pressure,
...
solve_timestep()
{
  ...
  if(this->time_step_number%this->param.output_solver_info_every_timesteps == 0)
  {
    pcout << std::endl
          ...
          << "t_n = " << time << " -> t_n+1 = " << time + time_step << std::endl
          << "______________________________________________________________________" << std::endl;
  }
  ...
  {
    pcout << std::endl
          << "Solve time step explicitly: Wall time in [s] = " << std::scientific << wall_time << std::endl;
...
analyze_computing_times() const
{
  ...
  double time_operator_evaluation = comp_navier_stokes_operation->get_wall_time_operator_evaluation();
  Utilities::MPI::MinMaxAvg data_op = Utilities::MPI::min_max_avg (time_operator_evaluation, MPI_COMM_WORLD);
  pcout << std::scientific << std::setprecision(4)
        << " Wall time operator eval. in [s] = " << data_op.avg << " -> "
        << std::fixed << std::setprecision(2) << data_op.avg/data.avg*100. << " %" << std::endl;

  // time spent with postprocessing
  Utilities::MPI::MinMaxAvg data_post = Utilities::MPI::min_max_avg (this->time_postprocessing, MPI_COMM_WORLD);
  pcout << std::scientific << std::setprecision(4)
        << " Wall time postprocessing in [s] = " << data_post.avg << " -> "
        << std::fixed << std::setprecision(2) << data_post.avg/data.avg*100. << " %" << std::endl;
