...
template<int dim>
void write_output(CompNS::OutputDataCompNavierStokes const      &output_data,
                  ...
                  unsigned int const                            output_counter)
{
  ...
  solution_component_interpretation(dim+2, DataComponentInterpretation::component_is_part_of_vector);
  ...
  data_out.add_data_vector (dof_handler, solution_conserved, solution_names_conserved, solution_component_interpretation);

  // additional solution fields
  for(typename std::vector<SolutionField<dim,double> >::const_iterator
...
public:
  ...
  void evaluate(parallel::distributed::Vector<double> const   &solution_conserved,
                std::vector<SolutionField<dim,double> > const &additional_fields,
                double const                                  &time,
