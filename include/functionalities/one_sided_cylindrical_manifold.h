...
public:
  ...
  Point<dim> pull_back(const Point<dim> &x) const
  {
    ...
    Assert(xi[0] >= 0.0 && xi[0] <= 1.0, ExcMessage("Pull back operation generated invalid xi[0] values."));
    Assert(xi[1] >= 0.0 && xi[1] <= 1.0, ExcMessage("Pull back operation generated invalid xi[1] values."));
    Assert(xi[2] >= 0.0 && xi[2] <= 1.0, ExcMessage("Pull back operation generated invalid xi[2] values."));
...
public:
  MyCylindricalManifold(const Point<spacedim> center_in)
    :
    ChartManifold<dim,spacedim,spacedim>(MyCylindricalManifold<dim,spacedim>::get_periodicity()),
    center(center_in)
  {}

  Tensor<1,spacedim> get_periodicity()
  {
  ...
  Point<spacedim> push_forward(const Point<spacedim> &ref_point) const
  {
    ...
    if (radius > 1e-10)
    {
      AssertThrow(spacedim ==2 || spacedim ==3, ExcMessage("Only implemented for 2D and 3D case."));
