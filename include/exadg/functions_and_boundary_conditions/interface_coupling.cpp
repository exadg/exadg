  ...
  double const                                               tolerance_)
{
  ...
  for(auto quad_index : interface_data_dst->get_quad_indices())
  {
    ...
    {
	  std::string const file_name = "interface_coupling_quad_index_" + dealii::Utilities::to_string(quad_index);
	  plot_points_and_triangulation(interface_data_dst->get_array_q_points(quad_index),
								    dof_handler_src_,
								    mapping_src_,
								    file_name);
	  AssertThrow(false, dealii::ExcMessage("Setup of InterfaceCoupling was not successful. Not all points have been found."));
    }
  }
}
...
template<int rank, int dim, typename Number>
void
InterfaceCoupling<rank, dim, Number>::plot_points_and_triangulation(std::vector<dealii::Point<dim>> const & points,
		...
		std::string const & file_name) const
{
  ...
  data_out.write_vtu_with_pvtu_record("./", file_name + "_grid", 0, dof_handler.get_communicator(), 4);
  ...
  particle_output.write_vtu_with_pvtu_record("./", file_name + "_points", 0, dof_handler.get_communicator());
