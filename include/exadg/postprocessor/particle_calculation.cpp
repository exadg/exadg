                                       ...
                                       ParticleData const &            data_in)
{
  if(data_in.time_control_data.is_active)
  {
    ...
    const auto my_bounding_box = dealii::GridTools::compute_mesh_predicate_bounding_box(
                                          ...
                                          bool const         print_output)
{
  ...
  while(particle != particle_handler.end())
  {
    ...
    const auto dh_cell = typename dealii::DoFHandler<dim>::cell_iterator(*cell, dof_handler);
