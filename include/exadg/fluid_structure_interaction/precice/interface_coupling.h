...
private:
  ...
  typedef std::map<dealii::types::boundary_id, std::shared_ptr<FunctionCached<rank, dim, double>>>
  ...
  typedef std::map<Id, dealii::types::global_dof_index> MapIndex;

  typedef std::pair<std::vector<dealii::types::global_dof_index>, std::vector<double>> Cache;

  typedef std::vector<dealii::Point<dim>> ArrayQuadraturePoints;
...
public:
   ...
   */
  void
  update_data(std::vector<dealii::Tensor<rank, dim, double>> & new_data) const
  {
    Assert(new_data.size() > 0, dealii::ExcInternalError());
