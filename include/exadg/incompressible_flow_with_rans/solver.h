...
#define INCLUDE_EXADG_INCOMPRESSIBLE_FLOW_WITH_RANS_SOLVER_H_
...
// application
#include <exadg/incompressible_flow_with_rans/user_interface/declare_get_application.h>
...
create_input_file(std::string const & input_file)
{
  ...
  NSRans::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)->add_parameters(prm);
...
run(std::string const & input_file, MPI_Comm const & mpi_comm, bool const is_test)
{
  ...
  std::shared_ptr<NSRans::ApplicationBase<dim, Number>> application =
    NSRans::get_application<dim, Number>(input_file, mpi_comm);

  std::shared_ptr<NSRans::Driver<dim, Number>> driver =
    std::make_shared<NSRans::Driver<dim, Number>>(mpi_comm, application, is_test);
...
#endif /* INCLUDE_EXADG_INCOMPRESSIBLE_FLOW_WITH_RANS_SOLVER_H_ */
