...
BoundaryMassOperator<dim, Number, n_components>::BoundaryMassOperator() : global_scaling_factor(1.0)
{
}
...
{
  return this->ids_to_parameters.size() > 0;
}
  ...
  BoundaryMassOperatorData<dim, Number> const & data)
{
  ...
  this->ids_to_parameters = data.ids_to_parameters;
}

template<int dim, typename Number, int n_components>
void
BoundaryMassOperator<dim, Number, n_components>::set_global_scaling_factor(
  Number const & factor) const
{
  this->global_scaling_factor = factor;
}

template<int dim, typename Number, int n_components>
void
BoundaryMassOperator<dim, Number, n_components>::set_ids_to_parameters(
  std::map<dealii::types::boundary_id, BoundaryMassParameters<Number>> const & ids_to_parameters_in)
  const
{
  this->ids_to_parameters = ids_to_parameters_in;
}
  ...
  Range const &                           range) const
{
  ...
  for(unsigned int face = range.first; face < range.second; face++)
  {
    ...
    // integrate over selected boundaries and *do not* `gather_evaluate()`/`integrate_scatter()` on
    // others
    if(auto it{this->ids_to_parameters.find(boundary_id)}; it != this->ids_to_parameters.end())
    {
      this->reinit_boundary_face(integrator_m, face);

      Number const scaled_coefficient    = it->second.scaling_factor_per_id * global_scaling_factor;
      bool const   use_normal_projection = it->second.use_normal_projection_per_id;

      integrator_m.reinit(face);
      integrator_m.gather_evaluate(src, this->integrator_flags.face_evaluate);

      this->do_boundary_mass_integral(integrator_m, scaled_coefficient, use_normal_projection);
...
template<int dim, typename Number, int n_components>
void
BoundaryMassOperator<dim, Number, n_components>::do_boundary_mass_integral(
  IntegratorFace & integrator_m,
  Number const &   scaled_coefficient,
  bool const       use_normal_projection) const
{
