...
public:
         ...
         std::string const          new_name = "")
  {
    ...
    AssertThrow(id == ids[0],
                ExcMessage("You want to insert a sub_tree with ids[0] = <" + ids[0] +
    ...
    if(found == false)
    {
      AssertThrow(remaining_id.size() == 0,
                  ExcMessage("Subtree can not be inserted since the specified identifier <" +
                             remaining_id[0] + "> does not exist in this tree."));

      std::shared_ptr<TimerTree> new_tree(new TimerTree());
      new_tree->copy_from(sub_tree);

      // The sub_tree's id can be replaced by a new name (if specified when calling this function)
      if(!new_name.empty())
      ...
      if(it != sub_trees.end()) // replace existing sub-tree if new sub-tree already exists
        *it = new_tree;
