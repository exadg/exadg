...
//      ++its;
//    }
//    AssertThrow(residual.norm() < 1e-12, dealii::ExcMessage("Newton for point did not
//    converge.")); return x;
//  }
//
//  std::unique_ptr<dealii::Manifold<dim>>
...
//
//  // the vertices need to be placed correctly according to the manifold description such that mesh
//  // refinements and high-order mappings are done correctly (which invoke pull-back and
//  push-forward
//  // operations)
//  std::vector<bool> vertex_touched(triangulation.n_vertices(), false);
//
...
private:
  ...
  double const GRID_STRETCH_FAC = 2.8; // 1.8;
  ...
  grid_transform_y(double const & eta) const
  {
    ...
    if(GRID_STRETCH_FAC >= 0)
      y = dimensions[1] / 2.0 * std::tanh(GRID_STRETCH_FAC * (2. * eta - 1.)) /
  ...
  inverse_grid_transform_y(double const & y) const
  {
    ...
    if(GRID_STRETCH_FAC >= 0)
      eta = (std::atanh(y * std::tanh(GRID_STRETCH_FAC) * 2.0 / dimensions[1]) / GRID_STRETCH_FAC +
             1.0) /
            2.0;
    else // use a negative GRID_STRETCH_FACTOR deactivate grid stretching
...
void
apply_boundary_layer_manifold(dealii::Triangulation<dim> &   triangulation,
                              dealii::Tensor<1, dim> const & dimensions)
{
  static BoundaryLayerManifold<dim> manifold(dimensions);
  triangulation.set_all_manifold_ids(1);
  triangulation.set_manifold(1, manifold);
}
