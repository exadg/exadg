...
#ifndef INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_TIME_INTEGRATION_CREATE_TIME_INTEGRATOR_H_
#define INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_TIME_INTEGRATION_CREATE_TIME_INTEGRATOR_H_

#include <exadg/incompressible_navier_stokes_for_rans/time_integration/time_int_bdf.h>
...
#include <exadg/incompressible_navier_stokes_for_rans/time_integration/time_int_interpolate_analytical_solution.h>
                       ...
                       bool const                                        is_test)
{
  ...
  if(parameters.temporal_discretization == TemporalDiscretization::BDFCoupledSolution)
  {
    ...
    time_integrator = std::make_shared<IncRANS::TimeIntBDFCoupled<dim, Number>>(
  ...
  else if(parameters.temporal_discretization == TemporalDiscretization::BDFDualSplittingScheme)
  {
    ...
    time_integrator = std::make_shared<IncRANS::TimeIntBDFDualSplitting<dim, Number>>(
  ...
  else if(parameters.temporal_discretization == TemporalDiscretization::BDFPressureCorrection)
  {
    ...
    time_integrator = std::make_shared<IncRANS::TimeIntBDFPressureCorrection<dim, Number>>(
  ...
  {
    time_integrator = std::make_shared<IncRANS::TimeIntInterpolateAnalyticalSolution<dim, Number>>(
