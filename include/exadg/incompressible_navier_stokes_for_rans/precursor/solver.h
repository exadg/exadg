...
#ifndef INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_SOLVER_PRECURSOR_H_
#define INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_SOLVER_PRECURSOR_H_
...
// driver
#include <exadg/incompressible_navier_stokes_for_rans/precursor/driver.h>

// utilities
#include <exadg/utilities/general_parameters.h>

// application
#include <exadg/incompressible_navier_stokes_for_rans/precursor/user_interface/declare_get_application.h>
...
create_input_file(std::string const & input_file)
{
  ...
  IncRANS::Precursor::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)->add_parameters(prm);
...
run(std::string const & input_file, MPI_Comm const & mpi_comm, bool const is_test)
{
  ...
  std::shared_ptr<IncRANS::Precursor::ApplicationBase<dim, Number>> application =
    IncRANS::Precursor::get_application<dim, Number>(input_file, mpi_comm);

  std::shared_ptr<IncRANS::Precursor::Driver<dim, Number>> driver =
    std::make_shared<IncRANS::Precursor::Driver<dim, Number>>(mpi_comm, application, is_test);
...
#endif /* INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_SOLVER_PRECURSOR_H_ */
