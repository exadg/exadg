...
#include <exadg/incompressible_navier_stokes_for_rans/preconditioners/multigrid_preconditioner_momentum.h>
...
OperatorCoupled<dim, Number>::initialize_preconditioner_pressure_block()
{
  ...
  else if(type == SchurComplementPreconditioner::PressureConvectionDiffusion)
  {
    ...
    setup_pressure_rans_equations_operator();
...
template<int dim, typename Number>
void
OperatorCoupled<dim, Number>::setup_pressure_rans_equations_operator()
{
  ...
  // equations.
  RANS::Operators::ConvectiveKernelData<dim> convective_kernel_data;
  convective_kernel_data.velocity_type      = RANS::TypeVelocityField::DoFVector;
  convective_kernel_data.dof_index_velocity = this->get_dof_index_velocity();
  convective_kernel_data.numerical_flux_formulation =
    RANS::NumericalFluxConvectiveOperator::LaxFriedrichsFlux;

  // diffusive operator:
  // take interior penalty factor of diffusivity of viscous operator, but use polynomial degree of
  // pressure shape functions.
  RANS::Operators::DiffusiveKernelData diffusive_kernel_data;
  ...
  RANS::CombinedOperatorData<dim> operator_data;
  ...
  pressure_conv_diff_operator = std::make_shared<RANS::CombinedOperator<dim, Number>>();
