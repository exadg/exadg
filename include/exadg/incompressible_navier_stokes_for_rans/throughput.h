...
#ifndef INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_THROUGHPUT_H_
#define INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_THROUGHPUT_H_
...
// driver
#include <exadg/incompressible_navier_stokes_for_rans/driver.h>
...
// application
#include <exadg/incompressible_navier_stokes_for_rans/user_interface/declare_get_application.h>
...
create_input_file(std::string const & input_file)
{
  ...
  ThroughputParameters<IncRANS::OperatorType> throughput;
  ...
  try
  {
    ...
    IncRANS::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)->add_parameters(prm);
  }
  catch(...)
  {
  }
    ...
    bool const                                        is_test)
{
  ...
  std::shared_ptr<IncRANS::Driver<dim, Number>> driver =
    std::make_shared<IncRANS::Driver<dim, Number>>(mpi_comm, application, is_test, true);
...
main(int argc, char ** argv)
{
  ...
  prm.enter_subsection("Throughput");
  {
    prm.add_parameter("PressureDegree",
                      ...
                      ExaDG::Patterns::Enum<ExaDG::IncRANS::PressureDegree>(),
                      true);
  }
  ...
  auto const lambda_get_dofs_per_element =
    [&](unsigned int const dim, unsigned int const degree, ExaDG::ElementType const element_type) {
      return ExaDG::IncRANS::get_dofs_per_element(
...
#endif /* INCLUDE_EXADG_INCOMPRESSIBLE_NAVIER_STOKES_FOR_RANS_THROUGHPUT_H_ */
