...
#ifndef INCLUDE_EXADG_RANS_EQUATIONS_THROUGHPUT_H_
#define INCLUDE_EXADG_RANS_EQUATIONS_THROUGHPUT_H_
...
// application
#include <exadg/rans_equations/user_interface/declare_get_application.h>
...
create_input_file(std::string const & input_file)
{
  ...
  ThroughputParameters<RANS::OperatorType> throughput;
  ...
  try
  {
    ...
    RANS::get_application<Dim, Number>(input_file, MPI_COMM_WORLD)->add_parameters(prm);
  }
  catch(...)
  {
  }
    ...
    bool const                                           is_test)
{
  ...
  std::shared_ptr<RANS::Driver<dim, Number>> driver =
    std::make_shared<RANS::Driver<dim, Number>>(mpi_comm, application, is_test, true);
...
#endif /* INCLUDE_EXADG_RANS_EQUATIONS_THROUGHPUT_H_ */
