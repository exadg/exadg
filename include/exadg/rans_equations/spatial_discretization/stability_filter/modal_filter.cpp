...
void ModalFilterOperator<dim, Number>::initialize(std::shared_ptr<dealii::MatrixFree<dim, Number> const>	matrix_free_in,
...
template<int dim, typename Number>
void ModalFilterOperator<dim, Number>::evaluate_critical_gradients(VectorType const & src)
{
	VectorType dummy;
	matrix_free->cell_loop(&This::cell_loop_calculate_critical_gradients,
			  this,
			  dummy,
			  src);
...
template<int dim, typename Number>
void ModalFilterOperator<dim, Number>::cell_loop_calculate_critical_gradients(dealii::MatrixFree<dim, Number> const & matrix_free_in,
									      VectorType &		dst,
									      VectorType const &	src,
									      Range const &		cell_range)
{
	IntegratorCell integrator(matrix_free_in,
			   dof_index,
			   quad_index);
...
template<int dim, typename Number>
void ModalFilterOperator<dim, Number>::apply_weighted_relaxation(VectorType const & src,
								 VectorType &      dst)
{
	matrix_free->cell_loop(&This::cell_loop_apply_weighted_relaxation,
			  this,
			  dst,
			  src);
}
...
template<int dim, typename Number>
void ModalFilterOperator<dim, Number>::cell_loop_apply_weighted_relaxation(dealii::MatrixFree<dim, Number> const & matrix_free_in,
					     VectorType &		dst,
					     VectorType const &	src,
					     Range const &		cell_range)
{
...
template<int dim, typename Number>
void ModalFilterOperator<dim, Number>::apply_modal_filter(VectorType const & src,
						      VectorType &       dst)
{
...
template<int dim, typename Number>
void
ModalFilterOperator<dim, Number>::convert_unit_cell_to_reference_cell(std::vector<dealii::Point<dim>> const & src,
