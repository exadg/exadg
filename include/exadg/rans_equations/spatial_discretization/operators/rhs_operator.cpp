...
template<int dim, typename Number, int n_components>
void
RHSOperator<dim, Number, n_components>::set_solution(VectorType const & src) const
{
  kernel->set_solution(src);
}
  ...
  Range const &                           cell_range) const
{
  ...
  for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
  {
    ...
    kernel->reinit_cell_velocity(cell);

    kernel->reinit_cell_solution(cell);
