                                                ...
                                                IntegratorFace & integrator_p) const
{
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    ...
    if(operator_data.diffusive_problem)
    {
      scalar gradient_flux = diffusive_kernel->calculate_gradient_flux(value_m, value_p, q);
                                                    ...
                                                    IntegratorFace & integrator_p) const
{
  ...
  for(unsigned int q = 0; q < integrator_p.n_q_points; ++q)
  {
    ...
    if(operator_data.diffusive_problem)
    {
      scalar gradient_flux = diffusive_kernel->calculate_gradient_flux(value_p, value_m, q);
  ...
  dealii::types::boundary_id const & boundary_id) const
{
  ...
  for(unsigned int q = 0; q < integrator_m.n_q_points; ++q)
  {
    ...
    if(operator_data.diffusive_problem)
    {
      ...
      value_flux += diffusive_kernel->calculate_boundary_value_flux(normal_gradient_m,
                                                            normal_gradient_p,
                                                            value_m,
                                                            value_p,
                                                            q);
    ...
    if(operator_data.diffusive_problem)
    {
      scalar gradient_flux = diffusive_kernel->calculate_boundary_gradient_flux(value_m, value_p, q);
      integrator_m.submit_normal_derivative(gradient_flux, q);
    }
  }
}
