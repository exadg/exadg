...
public:
  ...
  solve(VectorType & dst, VectorType const & rhs) const override
  {
    ...
    dealii::SolverSelector<VectorType> solver(solver_data.solver_name, solver_control);

    // Additional settings depending on requested solver type.
    if(solver_data.solver_name == "gmres")
    {
    ...
    else if(solver_data.solver_name == "fgmres")
    {
    ...
    this->timer_tree->insert({"Solver (" + solver_data.solver_name + ")"}, timer.wall_time());
  ...
  get_timings() const override
  {
    if(use_preconditioner)
    {
      this->timer_tree->insert({"Solver (" + solver_data.solver_name + ")"},
