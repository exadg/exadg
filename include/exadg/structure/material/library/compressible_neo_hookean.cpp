  ...
  unsigned int const                                                         cell) const
{
  ...
  for(unsigned int q = 0; q < integrator_lin->n_q_points; ++q)
  {
    ...
    if(check_type == 2)
    {
      ...
      if(update_J)
      {
	    AssertThrow(stable_formulation == false, dealii::ExcMessage("Storing F_old does not allow for a stable recovery of J."));
...
CompressibleNeoHookean<dim, Number>::second_piola_kirchhoff_stress_displacement_derivative(
  ...
  tensor const &     gradient_displacement, // ##+ I added this
  ...
  unsigned int const q) const
{
  ...
  if(cache_level == 0)
  {
	...
	scalar Jm1;
	tensor F;
	get_modified_F_Jm1(F, Jm1, gradient_displacement, check_type, true /* compute_J */, stable_formulation);
                                                      ...
                                                      bool const         force_evaluation) const
{
  ...
  if(cache_level < 2 or force_evaluation)
  {
    ...
    tensor F;
    get_modified_F_Jm1(
      F, Jm1, gradient_displacement, check_type, cache_level == 0 /* compute_J */, stable_formulation);
