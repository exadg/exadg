...
#ifndef INCLUDE_INVERSEMASSMATRIXXWALL_H_
#define INCLUDE_INVERSEMASSMATRIXXWALL_H_
...
template <int dim, int fe_degree, int fe_degree_xwall, int n_q_points_1d, typename Number, int n_components=dim>
...
class InverseMassMatrixXWallOperator: public InverseMassMatrixOperator<dim,fe_degree,value_type,n_components>
{
public:
                  ...
                  const unsigned int               quad_index)
  {
    ...
    this->mass_matrix_data_xwall.reset(new Threads::ThreadLocalStorage<InverseMassMatrixXWallData<dim,fe_degree,fe_degree_xwall,n_q_points_1d,value_type,n_components> >
            (InverseMassMatrixXWallData<dim,fe_degree,fe_degree_xwall,n_q_points_1d,value_type,n_components>(*(this->matrix_free_data), fe_param, dof_index, quad_index)));
  }
      ...
      if(mass_data.fe_eval_scalar[0].enriched)
      {
        ...
        for (unsigned int v = 0; v < data.n_components_filled(cell); ++v)
        {
          if (matrices[cell][v].m() != mass_data.fe_eval_scalar[0].dofs_per_cell)
            matrices[cell][v].reinit(mass_data.fe_eval_scalar[0].dofs_per_cell, mass_data.fe_eval_scalar[0].dofs_per_cell);// = onematrix;
          else
            matrices[cell][v]=0;
        }
...
#endif /* INCLUDE_INVERSEMASSMATRIXXWALL_H_ */
