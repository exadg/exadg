...
public:
  ...
  void
  reinit(const std::vector<Point<dim>> & control_points_in)
  {
    ...
    for(auto i : control_points_in)
      Assert(i.size() == nx,
             ExcInternalError("Expected constant size of control points along x, "
                              "got " +
                              std::to_string(i.size()) + " vs " + std::to_string(nx)));
  ...
  value(const double x, const double y) const
  {
    ...
    const unsigned int     ky =
      std::min(n_intervals_y - 1, static_cast<unsigned int>(y * n_intervals_y));
    const unsigned int k =
      std::min(n_intervals_x - 1, static_cast<unsigned int>(x * n_intervals_x));

    if(knot_vector_x[p + k] * (1 - 1e-13) > x || knot_vector_x[p + 1 + k] * (1 + 1e-13) < x)
      std::cout << "Could not identify x knot position " << x << " with k = " << k << " "
                << knot_vector_x[p + k] << " " << knot_vector_x[p + 1 + k] << std::endl;
    if(knot_vector_y[p + ky] * (1 - 1e-13) > y || knot_vector_y[p + 1 + ky] * (1 + 1e-13) < y)
      std::cout << "Could not identify y knot position " << y << " with k = " << ky << " "
                << knot_vector_y[p + ky] << " " << knot_vector_y[p + 1 + ky] << std::endl;
    ...
    for(unsigned int r = 1; r <= p; ++r)
    {
      for(unsigned int j = p; j >= r; --j)
      {
        const double alpha_j =
          (y - knot_vector_y[j + ky]) / (knot_vector_y[j + 1 + ky - r + p] - knot_vector_y[j + ky]);
        for(unsigned int i = 0; i <= p; ++i)
    ...
    for(unsigned int r = 1; r <= p; ++r)
    {
      ...
      {
        const double alpha_j =
          (x - knot_vector_x[i + k]) / (knot_vector_x[i + 1 + k - r + p] - knot_vector_x[i + k]);
  ...
  void
  write_to_file(std::ostream & file)
  {
  ...
  void
  read_from_file(std::istream & file)
  {
...
inline void
create_bspline_from_file(const std::string & filename)
{
